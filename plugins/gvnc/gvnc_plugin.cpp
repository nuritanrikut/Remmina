...
static void gvnc_plugin_keystroke( RemminaProtocolWidget *gp, const guint keystrokes[], const gint keylen )
{
    ...
    if( gpdata->vnc )
    {
        vnc_display_send_keys( VNC_DISPLAY( gpdata->vnc ), keystrokes, keylen );
    }
}
...
static void
gvnc_plugin_desktop_resize( GtkWidget *vncdisplay G_GNUC_UNUSED, int width, int height, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    GVncPluginData *gpdata = GET_PLUGIN_DATA( gp );
...
static void
gvnc_plugin_auth_unsupported( VncDisplay *vnc G_GNUC_UNUSED, unsigned int authType, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    GVncPluginData *gpdata = GET_PLUGIN_DATA( gp );
        ...
        for( i = 0; i < credArray->len; i++ )
        {
            ...
            switch( g_value_get_enum( cred ) )
            {
                case VNC_DISPLAY_CREDENTIAL_USERNAME:
                    if( !s_username
                        || vnc_display_set_credential(
                            VNC_DISPLAY( gpdata->vnc ), g_value_get_enum( cred ), s_username ) )
                    {
                ...
                case VNC_DISPLAY_CREDENTIAL_PASSWORD:
                    if( !s_password
                        || vnc_display_set_credential(
                            VNC_DISPLAY( gpdata->vnc ), g_value_get_enum( cred ), s_password ) )
                    {
                        g_debug( "Failed to set credential type %d", g_value_get_enum( cred ) );
                        vnc_display_close( VNC_DISPLAY( gpdata->vnc ) );
                    }
                    break;
                case VNC_DISPLAY_CREDENTIAL_CLIENTNAME:
                    if( vnc_display_set_credential( VNC_DISPLAY( gpdata->vnc ), g_value_get_enum( cred ), "remmina" ) )
                    {
                        g_debug( "Failed to set credential type %d", g_value_get_enum( cred ) );
                        vnc_display_close( VNC_DISPLAY( gpdata->vnc ) );
                    }
...
static void gvnc_plugin_init( RemminaProtocolWidget *gp )
{
    ...
    g_signal_connect( gpdata->vnc, "vnc-auth-credential", G_CALLBACK( gvnc_plugin_ask_auth ), gp );
    g_signal_connect( gpdata->vnc, "vnc-auth-failure", G_CALLBACK( gvnc_plugin_auth_failure ), gp );
    g_signal_connect( gpdata->vnc, "vnc-auth-unsupported", G_CALLBACK( gvnc_plugin_auth_unsupported ), gp );
    g_signal_connect( gpdata->vnc, "vnc-disconnected", G_CALLBACK( gvnc_plugin_disconnected ), gp );
    g_signal_connect( gpdata->vnc, "vnc-initialized", G_CALLBACK( gvnc_plugin_initialized ), gp );
    g_signal_connect( gpdata->vnc, "vnc-desktop-resize", G_CALLBACK( gvnc_plugin_desktop_resize ), gp );
    g_signal_connect( gpdata->vnc, "vnc-bell", G_CALLBACK( gvnc_plugin_on_bell ), gp );
    g_signal_connect( gpdata->vnc, "vnc-error", G_CALLBACK( gvnc_plugin_on_vnc_error ), gp );
    g_signal_connect( gpdata->vnc, "vnc-pointer-grab", G_CALLBACK( gvnc_plugin_mouse_grab ), gp );
    g_signal_connect( gpdata->vnc, "vnc-pointer-ungrab", G_CALLBACK( gvnc_plugin_mouse_ungrab ), gp );
    g_signal_connect( gpdata->vnc, "vnc-server-cut-text", G_CALLBACK( gvnc_plugin_cut_text ), gp );
...
static int gvnc_plugin_open_connection( RemminaProtocolWidget *gp )
{
    ...
    else
    {
        ...
        if( !tunnel )
            return FALSE;
        remmina_plugin_service->get_server_port( tunnel, GVNC_DEFAULT_PORT, &host, &port );
    }
...
static gpointer colordepth_list[] = { "0",
                                      N_( "Use server settings" ),
                                      "1",
                                      N_( "True colour (24 bits)" ),
                                      "2",
                                      N_( "High colour (16 bits)" ),
                                      "3",
                                      N_( "Low colour (8 bits)" ),
                                      "4",
                                      N_( "Ultra low colour (3 bits)" ),
                                      NULL };
...
static const RemminaProtocolSetting gvnc_plugin_basic_settings[] = {
    ...
    { REMMINA_PROTOCOL_SETTING_TYPE_CHECK,
      "lossy_encoding",
      N_( "Use JPEG Compression" ),
      TRUE,
      NULL,
      N_( "This might not work on all VNC servers" ),
      NULL,
      NULL },
...
static const RemminaProtocolSetting gvnc_plugin_advanced_settings[] = {
    ...
    { REMMINA_PROTOCOL_SETTING_TYPE_CHECK,
      "shared",
      N_( "Shared connection" ),
      TRUE,
      NULL,
      N_( "If the server should try to share the desktop by leaving other clients connected" ) },
...
static const RemminaProtocolFeature gvnc_plugin_features[] = {
    ...
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      GVNC_PLUGIN_FEATURE_TOOL_SENDCTRLALTDEL,
      N_( "Send Ctrl+Alt+_Del" ),
      NULL,
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      GVNC_PLUGIN_FEATURE_TOOL_SENDCTRLALTBACKSPACE,
      N_( "Send Ctrl+Alt+_Backspace" ),
      NULL,
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      GVNC_PLUGIN_FEATURE_TOOL_SENDCTRLALTF1,
      N_( "Send Ctrl+Alt+_F1" ),
      NULL,
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      GVNC_PLUGIN_FEATURE_TOOL_SENDCTRLALTF2,
      N_( "Send Ctrl+Alt+_F2" ),
      NULL,
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      GVNC_PLUGIN_FEATURE_TOOL_SENDCTRLALTF3,
      N_( "Send Ctrl+Alt+_F3" ),
      NULL,
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      GVNC_PLUGIN_FEATURE_TOOL_SENDCTRLALTF4,
      N_( "Send Ctrl+Alt+_F4" ),
      NULL,
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      GVNC_PLUGIN_FEATURE_TOOL_SENDCTRLALTF5,
      N_( "Send Ctrl+Alt+_F5" ),
      NULL,
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      GVNC_PLUGIN_FEATURE_TOOL_SENDCTRLALTF6,
      N_( "Send Ctrl+Alt+_F6" ),
      NULL,
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      GVNC_PLUGIN_FEATURE_TOOL_SENDCTRLALTF7,
      N_( "Send Ctrl+Alt+_F7" ),
      NULL,
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      GVNC_PLUGIN_FEATURE_TOOL_SENDCTRLALTF8,
      N_( "Send Ctrl+Alt+_F8" ),
      NULL,
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      GVNC_PLUGIN_FEATURE_TOOL_SENDCTRLALTF9,
      N_( "Send Ctrl+Alt+_F9" ),
      NULL,
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      GVNC_PLUGIN_FEATURE_TOOL_SENDCTRLALTF10,
      N_( "Send Ctrl+Alt+_F10" ),
      NULL,
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      GVNC_PLUGIN_FEATURE_TOOL_SENDCTRLALTF11,
      N_( "Send Ctrl+Alt+_F11" ),
      NULL,
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      GVNC_PLUGIN_FEATURE_TOOL_SENDCTRLALTF12,
      N_( "Send Ctrl+Alt+_F12" ),
      NULL,
      NULL },
#ifdef HAVE_VNC_POWER_CONTROL
    ...
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      GVNC_PLUGIN_FEATURE_TOOL_RESET,
      N_( "Reset remote host (hard reboot)" ),
      NULL,
      NULL },
