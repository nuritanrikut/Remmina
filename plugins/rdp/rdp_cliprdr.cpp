...
void remmina_rdp_cliprdr_get_target_types( UINT32 **formats, UINT16 *size, GdkAtom *types, int count )
{
    ...
    *formats = static_cast<UINT32 *>( realloc( *formats, sizeof( UINT32 ) * ( *size ) ) );
}
...
void remmina_rdp_cliprdr_send_client_format_list( RemminaProtocolWidget *gp )
{
    ...
    pFormatList = static_cast<CLIPRDR_FORMAT_LIST *>( remmina_rdp_event_queue_ui_sync_retptr( gp, ui ) );
                                                    ...
                                                    const CLIPRDR_FORMAT_LIST *formatList )
{
    ...
    REMMINA_PLUGIN_DEBUG(
        "gp=%p: Received a new ServerFormatList from server clipboard. Remmina version = %s", gp, VERSION );

    GtkTargetList *list = gtk_target_list_new( NULL, 0 );

    if( clipboard->srv_clip_data_wait == rfClipboard::SCDW_BUSY_WAIT )
    {
    ...
    if( n_targets == 0 )
    {
        REMMINA_PLUGIN_DEBUG( "gp=%p adding a dummy text target (empty text) for local clipboard, because we have no "
                              "interesting targets from the server. Putting it in the local clipboard cache." );
...
int timeval_diff(  timeval *start,  timeval *end )
{
                                                             ...
                                                             const CLIPRDR_FORMAT_DATA_RESPONSE *formatDataResponse )
{
    ...
    REMMINA_PLUGIN_DEBUG( "gp=%p %zu bytes transferred from server in %d ms. Speed is %d bytes/sec",
                          ...
                          (size_t)size,
                          mstrans,
                          mstrans != 0 ? (int)( (int64_t)size * 1000 / mstrans ) : 0 );
    ...
    if( size > 0 )
    {
        switch( rfi->clipboard.format )
        {
            case CF_UNICODETEXT:
            {
                ...
                crlf2lf( static_cast<UINT8 *>( output ), &size );
                break;
            }
    ...
    if( output != NULL )
        ...
        REMMINA_PLUGIN_DEBUG( "gp=%p: data from server is not valid (size=%zu format=%d), cannot load into cache",
                              gp,
                              size,
    ...
    else
    {
        ...
        REMMINA_PLUGIN_DEBUG( "gp=%p: clipboard transfer from server completed, but no local application is requesting "
                              "it. Data is on local cache now, try to paste later.",
                              gp );
    }
    clipboard->srv_clip_data_wait = rfClipboard::SCDW_NONE;
                                       ...
                                       RemminaProtocolWidget *gp )
{
    ...
    clipboard = &( rfi->clipboard );
    if( clipboard->srv_clip_data_wait != rfClipboard::SCDW_NONE )
    {
    ...
    if( clipboard->format != info || clipboard->srv_data == NULL )
    {
        ...
        clipboard->srv_clip_data_wait =
            rfClipboard::SCDW_BUSY_WAIT; // Annotate that we are waiting for ServerFormatDataResponse

        REMMINA_PLUGIN_DEBUG(
            "gp=%p Requesting clipboard data with format %d from the server via ServerFormatDataRequest",
            gp,
            clipboard->format );
		...
		*      In the meanwhile allow GTK event loop to proceed */
        ...
        rc = 100000;
        tlimit1s = tstart + 1;
        while( ( tnow = time( NULL ) ) < tlimit && rc != 0
               && clipboard->srv_clip_data_wait == rfClipboard::SCDW_BUSY_WAIT )
        {
            if( tnow >= tlimit1s )
            {
                REMMINA_PLUGIN_DEBUG(
                    "gp=%p, clipboard data is still not here after %u seconds", gp, (unsigned)( tnow - tstart ) );
        ...
        if( rc != 0 )
        {
            ...
            if( clipboard->srv_clip_data_wait == rfClipboard::SCDW_ABORTING )
            {
            ...
            {
                if( rc == ETIMEDOUT )
                    g_warning( "[RDP] gp=%p Clipboard data from the server is not available in %d seconds. No data "
    ...
    if( clipboard->srv_data != NULL )
    {
        ...
        {
            gtk_selection_data_set_pixbuf( selection_data, static_cast<GdkPixbuf *>( clipboard->srv_data ) );
        }
        else if( info == CB_FORMAT_HTML || info == clipboard->server_html_format_id )
        {
            REMMINA_PLUGIN_DEBUG( "gp=%p returning %zu bytes of HTML in clipboard to requesting application",
                                  gp,
                                  strlen( static_cast<char *>( clipboard->srv_data ) ) );
            GdkAtom atom = gdk_atom_intern( "text/html", TRUE );
            gtk_selection_data_set( selection_data,
                                    atom,
                                    8,
                                    static_cast<const guchar *>( clipboard->srv_data ),
                                    strlen( static_cast<char *>( clipboard->srv_data ) ) );
        }
        else
        {
            REMMINA_PLUGIN_DEBUG( "gp=%p returning %zu bytes of text in clipboard to requesting application",
                                  gp,
                                  strlen( static_cast<char *>( clipboard->srv_data ) ) );
            gtk_selection_data_set_text( selection_data, static_cast<char *>( clipboard->srv_data ), -1 );
        }
        clipboard->srv_clip_data_wait = rfClipboard::SCDW_NONE;
    }
    else
...
CLIPRDR_FORMAT_LIST *remmina_rdp_cliprdr_get_client_format_list( RemminaProtocolWidget *gp )
{
    ...
    if( result && loccount > 0 )
    {
        ...
        if( srvcount > 0 )
        {
            retp = ( retp_t *)malloc( sizeof(  retp_t ) + sizeof( CLIPRDR_FORMAT ) * srvcount );
        ...
        {
            retp = ( retp_t *)malloc( sizeof(  retp_t ) );
    ...
    {
        retp = ( retp_t *)malloc( sizeof(  retp_t ) + sizeof( CLIPRDR_FORMAT ) );
        retp->pFormatList.formats = NULL;
        retp->pFormatList.numFormats = 0;
    }
...
void remmina_rdp_cliprdr_set_clipboard_data( RemminaProtocolWidget *gp, RemminaPluginRdpUiObject *ui )
{
    ...
    if( gtkClipboard )
    {
        ...
        gtk_clipboard_set_with_owner( gtkClipboard,
                                      targets,
                                      n_targets,
                                      (GtkClipboardGetFunc)remmina_rdp_cliprdr_request_data,
                                      (GtkClipboardClearFunc)remmina_rdp_cliprdr_empty_clipboard,
                                      G_OBJECT( gp ) );
