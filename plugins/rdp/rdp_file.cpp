...
static void remmina_rdp_file_import_field( RemminaFile *remminafile, const char *key, const char *value )
{
    ...
    {
        remmina_plugin_service->file_set_int(
            remminafile, "gateway_usage", ( atoi( value ) == TSC_PROXY_MODE_DETECT ) );
    }
...
static RemminaFile *remmina_rdp_file_import_channel( GIOChannel *channel )
{
    ...
    remmina_plugin_service->file_set_string(
        remminafile, "name", remmina_plugin_service->file_get_string( remminafile, "server" ) );
