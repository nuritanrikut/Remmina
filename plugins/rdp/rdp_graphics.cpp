...
BOOL rf_Bitmap_New( rdpContext *context, rdpBitmap *bitmap )
{
    ...
    if( bitmap->data != NULL )
    {
        data = freerdp_image_convert(
            bitmap->data, NULL, bitmap->width, bitmap->height, rfi->srcBpp, rfi->bpp, rfi->clrconv );

        if( bitmap->ephemeral != TRUE )
        {
            image = XCreateImage( rfi->display,
                                  rfi->visual,
                                  rfi->depth,
                                  ZPixmap,
                                  0,
...
BOOL rf_Bitmap_Paint( rdpContext *context, rdpBitmap *bitmap )
{
    ...
    XPutImage( rfi->display, rfi->primary, rfi->gc, image, 0, 0, bitmap->left, bitmap->top, width, height );
...
BOOL rf_Glyph_New( rdpContext *context, const rdpGlyph *glyph )
{
    ...
    image =
        XCreateImage( rfi->display, rfi->visual, 1, ZPixmap, 0, (char *)glyph->aj, glyph->cx, glyph->cy, 8, scanline );
                                ...
                                BOOL fOpRedundant )
{
    ...
    bgcolor = ( rfi->clrconv->invert ) ? freerdp_color_convert_var_bgr( bgcolor, rfi->srcBpp, 32, rfi->clrconv )
                                       : freerdp_color_convert_var_rgb( bgcolor, rfi->srcBpp, 32, rfi->clrconv );

    fgcolor = ( rfi->clrconv->invert ) ? freerdp_color_convert_var_bgr( fgcolor, rfi->srcBpp, 32, rfi->clrconv )
...
static BOOL
rf_Glyph_EndDraw( rdpContext *context, INT32 x, INT32 y, INT32 width, INT32 height, UINT32 bgcolor, UINT32 fgcolor )
{
    TRACE_CALL( __func__ );
#ifdef RF_GLYPH
    rfContext *rfi = (rfContext *)context;
