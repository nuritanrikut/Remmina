...
int remmina_rdp_event_on_map( RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
...
int remmina_rdp_event_on_unmap( RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    rfContext *rfi = GET_PLUGIN_DATA( gp );
    rdpGdi *gdi;
...
static int remmina_rdp_event_on_focus_in( GtkWidget *widget, GdkEventKey *event, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    ...
    const char *wname = gtk_widget_get_name( gtk_widget_get_toplevel( widget ) );
...
void remmina_rdp_event_event_push( RemminaProtocolWidget *gp, const RemminaPluginRdpEvent *e )
{
    ...
    if( rfi->event_queue )
    {
        event = static_cast<RemminaPluginRdpEvent *>( g_memdup2( e, sizeof( RemminaPluginRdpEvent ) ) );
...
static void remmina_rdp_event_release_key( RemminaProtocolWidget *gp, RemminaPluginRdpEvent rdp_event )
{
    ...
    gint i;
    rfContext *rfi = GET_PLUGIN_DATA( gp );
    RemminaPluginRdpEvent rdp_event_2;
...
static void remmina_rdp_event_scale_area( RemminaProtocolWidget *gp, gint *x, gint *y, gint *w, gint *h )
{
    ...
    sx = MIN( MAX( 0, ( *x ) * rfi->scale_width / width - rfi->scale_width / width - 2 ), rfi->scale_width - 1 );

    sy = MIN( MAX( 0, ( *y ) * rfi->scale_height / height - rfi->scale_height / height - 2 ), rfi->scale_height - 1 );

    sw = MIN( rfi->scale_width - sx, ( *w ) * rfi->scale_width / width + rfi->scale_width / width + 4 );

    sh = MIN( rfi->scale_height - sy, ( *h ) * rfi->scale_height / height + rfi->scale_height / height + 4 );
...
static int remmina_rdp_event_on_draw( GtkWidget *widget, cairo_t *context, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    rfContext *rfi = GET_PLUGIN_DATA( gp );
    guint width, height;
    ...
    if( rfi->is_reconnecting )
    {
        ...
        /* Draw text */
        msg = g_strdup_printf(
            _( "Reconnection attempt %d of %d…" ), rfi->reconnect_nattempt, rfi->reconnect_maxattempts );
        ...
        cairo_move_to( context,
                       ( width - ( extents.width + extents.x_bearing ) ) / 2,
                       ( height - ( extents.height + extents.y_bearing ) ) / 2 );
...
static int remmina_rdp_event_delayed_monitor_layout( RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    rfContext *rfi = GET_PLUGIN_DATA( gp );
    ...
    if( rfi->dispcontext && rfi->dispcontext->SendMonitorLayout )
    {
        ...
        if( ( gpwidth != prevwidth || gpheight != prevheight ) && gpwidth >= 200 && gpheight >= 200 )
        {
            ...
            {
                const rdpMonitor *base =
                    static_cast<const rdpMonitor *>( freerdp_settings_get_pointer( rfi->settings, FreeRDP_MonitorDefArray ) );
                for( gint i = 0; i < freerdp_settings_get_uint32( rfi->settings, FreeRDP_MonitorCount ); ++i )
                {
                    ...
                    REMMINA_PLUGIN_DEBUG( "EVNT MON LAYOUT - physicalWidth: %i",
                                          rdp_event.monitor_layout.physicalWidth );
                    rdp_event.monitor_layout.physicalHeight = current->attributes.physicalHeight;
                    REMMINA_PLUGIN_DEBUG( "EVNT MON LAYOUT - PhysicalHeight: %i",
                                          rdp_event.monitor_layout.physicalHeight );
                    ...
                    REMMINA_PLUGIN_DEBUG( "EVNT MON LAYOUT - desktopOrientation: %i",
                    ...
                    REMMINA_PLUGIN_DEBUG( "EVNT MON LAYOUT - ScaleFactorflag: %i",
...
void remmina_rdp_event_send_delayed_monitor_layout( RemminaProtocolWidget *gp )
{
    ...
    if( rfi->scale == REMMINA_PROTOCOL_WIDGET_SCALE_MODE_DYNRES )
        rfi->delayed_monitor_layout_handler =
            g_timeout_add( 500, (GSourceFunc)remmina_rdp_event_delayed_monitor_layout, gp );
}

static int remmina_rdp_event_on_configure( GtkWidget *widget, GdkEventConfigure *event, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
...
static void remmina_rdp_event_translate_pos( RemminaProtocolWidget *gp, int ix, int iy, UINT16 *ox, UINT16 *oy )
{
    ...
    if( ( rfi->scale == REMMINA_PROTOCOL_WIDGET_SCALE_MODE_SCALED ) && ( rfi->scale_width >= 1 )
        && ( rfi->scale_height >= 1 ) )
    {
...
static void
remmina_rdp_event_reverse_translate_pos_reverse( RemminaProtocolWidget *gp, int ix, int iy, int *ox, int *oy )
{
    TRACE_CALL( __func__ );
    rfContext *rfi = GET_PLUGIN_DATA( gp );
...
static int remmina_rdp_event_on_motion( GtkWidget *widget, GdkEventMotion *event, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    RemminaPluginRdpEvent rdp_event;
...
static int remmina_rdp_event_on_button( GtkWidget *widget, GdkEventButton *event, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    gint flag;
    bool extended = FALSE;
...
static int remmina_rdp_event_on_scroll( GtkWidget *widget, GdkEventScroll *event, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    gint flag;
    RemminaPluginRdpEvent rdp_event;
...
static void remmina_rdp_event_init_keymap( rfContext *rfi, const char *strmap )
{
    long int v1, v2;
    const char *s;
    char *endptr;
...
static int remmina_rdp_event_on_key( GtkWidget *widget, GdkEventKey *event, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
        ...
        default:
			 ...
			 */
                if( event->keyval >= 0xfe00 ||         // Arrows, Shift, Alt, Fn, num keypad…
                    ...
                    ( event->state & ( GDK_MOD1_MASK | GDK_CONTROL_MASK | GDK_SUPER_MASK ) )
                        != 0 // A modifier not recognized by gdk_keyval_to_unicode()
                )
                {
...
int remmina_rdp_event_on_clipboard( GtkClipboard *gtkClipboard, GdkEvent *event, RemminaProtocolWidget *gp )
{
    ...
    if( new_owner != (GObject *)gp )
    {
        ...
        REMMINA_PLUGIN_DEBUG( "gp=%p owner-change: new owner is different than me: new=%p me=%p", gp, new_owner, gp );
...
void remmina_rdp_event_init( RemminaProtocolWidget *gp )
{
    ...
    disable_smooth_scrolling =
        remmina_plugin_service->file_get_int( remminafile, "disable-smooth-scrolling", disable_smooth_scrolling );
    ...
    gtk_widget_add_events( rfi->drawing_area,
                           GDK_POINTER_MOTION_MASK | GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK
                               | GDK_KEY_PRESS_MASK | GDK_KEY_RELEASE_MASK | GDK_SCROLL_MASK | GDK_FOCUS_CHANGE_MASK );
    ...
    if( rfi->use_client_keymap && rfi->keymap )
        fprintf( stderr,
                 "RDP profile error: you cannot define both rdp_map_hardware_keycode and have 'Use client keyboard "
                 "mapping' enabled\n" );

    g_signal_connect( G_OBJECT( rfi->drawing_area ), "draw", G_CALLBACK( remmina_rdp_event_on_draw ), gp );
    g_signal_connect(
        G_OBJECT( rfi->drawing_area ), "configure-event", G_CALLBACK( remmina_rdp_event_on_configure ), gp );
    g_signal_connect(
        G_OBJECT( rfi->drawing_area ), "motion-notify-event", G_CALLBACK( remmina_rdp_event_on_motion ), gp );
    g_signal_connect(
        G_OBJECT( rfi->drawing_area ), "button-press-event", G_CALLBACK( remmina_rdp_event_on_button ), gp );
    g_signal_connect(
        G_OBJECT( rfi->drawing_area ), "button-release-event", G_CALLBACK( remmina_rdp_event_on_button ), gp );
    g_signal_connect( G_OBJECT( rfi->drawing_area ), "scroll-event", G_CALLBACK( remmina_rdp_event_on_scroll ), gp );
    g_signal_connect( G_OBJECT( rfi->drawing_area ), "key-press-event", G_CALLBACK( remmina_rdp_event_on_key ), gp );
    g_signal_connect( G_OBJECT( rfi->drawing_area ), "key-release-event", G_CALLBACK( remmina_rdp_event_on_key ), gp );
    g_signal_connect(
        G_OBJECT( rfi->drawing_area ), "focus-in-event", G_CALLBACK( remmina_rdp_event_on_focus_in ), gp );
    /** Fixme: This comment
    ...
    if( !remmina_plugin_service->file_get_int( remminafile, "disableclipboard", FALSE ) )
    {
        ...
        rfi->clipboard.clipboard_handler =
            g_signal_connect( clipboard, "owner-change", G_CALLBACK( remmina_rdp_event_on_clipboard ), gp );
    }
    ...
    GdkVisual *visual = gdk_screen_get_system_visual( gdk_display_get_default_screen( rfi->display ) );
...
void remmina_rdp_event_uninit( RemminaProtocolWidget *gp )
{
    ...
    {
        g_signal_handler_disconnect( G_OBJECT( gtk_widget_get_clipboard( rfi->drawing_area, GDK_SELECTION_CLIPBOARD ) ),
                                     rfi->clipboard.clipboard_handler );
...
static void remmina_rdp_event_create_cairo_surface( rfContext *rfi )
{
    ...
    rfi->surface = cairo_image_surface_create_for_data(
        (unsigned char *)gdi->primary_buffer, rfi->cairo_format, gdi->width, gdi->height, stride );
}
...
void remmina_rdp_event_update_scale( RemminaProtocolWidget *gp )
{
    ...
    if( rfi->surface
        && ( cairo_image_surface_get_width( rfi->surface ) != gdi->width
    ...
    if( rfi->scale == REMMINA_PROTOCOL_WIDGET_SCALE_MODE_SCALED
        || rfi->scale == REMMINA_PROTOCOL_WIDGET_SCALE_MODE_DYNRES )
...
static BOOL remmina_rdp_event_create_cursor( RemminaProtocolWidget *gp, RemminaPluginRdpUiObject *ui )
{
    ...
    UINT8 *data = static_cast<UINT8 *>(malloc( pointer->width * pointer->height * 4 ));

    if( !freerdp_image_copy_from_pointer_data( (BYTE *)data,
                                               PIXEL_FORMAT_BGRA32,
                                               pointer->width * 4,
                                               0,
                                               0,
    ...
    surface =
        cairo_image_surface_create_for_data( data,
                                             ...
                                             pointer->width,
                                             pointer->height,
                                             cairo_format_stride_for_width( CAIRO_FORMAT_ARGB32, pointer->width ) );
    pixbuf = gdk_pixbuf_get_from_surface( surface, 0, 0, pointer->width, pointer->height );
    cairo_surface_destroy( surface );
    free( data );
    ( (rfPointer *)ui->cursor.pointer )->cursor =
        gdk_cursor_new_from_pixbuf( rfi->display, pixbuf, pointer->xPos, pointer->yPos );
...
static int remmina_rdp_event_process_ui_queue( RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
