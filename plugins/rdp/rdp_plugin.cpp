        ...
        switch( event->type )
        {
            ...
            case REMMINA_RDP_EVENT_TYPE_MOUSE:
                if( event->mouse_event.extended )
                    input->ExtendedMouseEvent(
                        input, event->mouse_event.flags, event->mouse_event.x, event->mouse_event.y );
                else
                    input->MouseEvent( input, event->mouse_event.flags, event->mouse_event.x, event->mouse_event.y );
                break;

            case REMMINA_RDP_EVENT_TYPE_CLIPBOARD_SEND_CLIENT_FORMAT_LIST:
                rfi->clipboard.context->ClientFormatList( rfi->clipboard.context,
            ...
            case REMMINA_RDP_EVENT_TYPE_CLIPBOARD_SEND_CLIENT_FORMAT_DATA_REQUEST:
                ...
                rfi->clipboard.context->ClientFormatDataRequest(
                    rfi->clipboard.context, event->clipboard_formatdatarequest.pFormatDataRequest );
            ...
            case REMMINA_RDP_EVENT_TYPE_SEND_MONITOR_LAYOUT:
                if( remmina_plugin_service->file_get_int( remminafile, "multimon", FALSE ) )
                {
                    ...
                    rfi->dispcontext->SendMonitorLayout(
                        rfi->dispcontext, freerdp_settings_get_uint32( rfi->settings, FreeRDP_MonitorCount ), dcml );
                    g_free( dcml );
                }
                else
                {
...
BOOL rf_auto_reconnect( rfContext *rfi )
{
    ...
    while( TRUE )
    {
        ...
        REMMINA_PLUGIN_DEBUG( "[%s] reconnection, attempt #%d of %d",
                              freerdp_settings_get_string( rfi->settings, FreeRDP_ServerHostname ),
                              rfi->reconnect_nattempt,
                              rfi->reconnect_maxattempts );
        ...
        else
        {
            if( freerdp_reconnect( rfi->instance ) )
            {
                ...
                REMMINA_PLUGIN_DEBUG( "[%s] reconnected.",
                                      freerdp_settings_get_string( rfi->settings, FreeRDP_ServerHostname ) );
                rfi->is_reconnecting = FALSE;
                return TRUE;
            }
...
static BOOL remmina_rdp_pre_connect( freerdp *instance )
{
    ...
    PubSub_SubscribeChannelDisconnected(
        instance->context->pubSub, (pChannelDisconnectedEventHandler)remmina_rdp_OnChannelDisconnectedEventHandler );
...
static BOOL remmina_rdp_authenticate( freerdp *instance, char **username, char **password, char **domain )
{
    ...
    if( ret == GTK_RESPONSE_OK )
    {
        ...
        if( s_username )
            freerdp_settings_set_string( rfi->settings, FreeRDP_Username, s_username );

        s_password = remmina_plugin_service->protocol_plugin_init_get_password( gp );
        if( s_password )
            freerdp_settings_set_string( rfi->settings, FreeRDP_Password, s_password );

        s_domain = remmina_plugin_service->protocol_plugin_init_get_domain( gp );
        if( s_domain )
            freerdp_settings_set_string( rfi->settings, FreeRDP_Domain, s_domain );
...
static BOOL remmina_rdp_gw_authenticate( freerdp *instance, char **username, char **password, char **domain )
{
    ...
    if( ret == GTK_RESPONSE_OK )
    {
        s_username = remmina_plugin_service->protocol_plugin_init_get_username( gp );
        if( s_username )
            freerdp_settings_set_string( rfi->settings, FreeRDP_GatewayUsername, s_username );

        s_password = remmina_plugin_service->protocol_plugin_init_get_password( gp );
        if( s_password )
            freerdp_settings_set_string( rfi->settings, FreeRDP_GatewayPassword, s_password );

        s_domain = remmina_plugin_service->protocol_plugin_init_get_domain( gp );
        if( s_domain )
            freerdp_settings_set_string( rfi->settings, FreeRDP_GatewayDomain, s_domain );
...
static DWORD remmina_rdp_verify_certificate( freerdp *instance,
                                             ...
                                             const char *issuer,
                                             const char *fingerprint,
                                             BOOL host_mismatch ) __attribute__( ( unused ) );
static DWORD remmina_rdp_verify_certificate( freerdp *instance,
                                             const char *common_name,
                                             const char *subject,
                                             const char *issuer,
...
static int remmina_rdp_main( RemminaProtocolWidget *gp )
{
    ...
    datapath = g_build_path( "/", remmina_plugin_service->file_get_user_datadir(), "RDP", NULL );
    ...
    freerdp_settings_set_bool(
        ...
        ( remmina_plugin_service->file_get_int( remminafile, "disableautoreconnect", FALSE ) ? FALSE : TRUE ) );
    ...
    freerdp_settings_set_uint32(
        rfi->settings, FreeRDP_ColorDepth, remmina_plugin_service->file_get_int( remminafile, "colordepth", 99 ) );
    ...
    {
        if( freerdp_settings_get_uint32( rfi->settings, FreeRDP_DesktopWidth ) < AVC_MIN_DESKTOP_WIDTH )
            freerdp_settings_set_uint32( rfi->settings, FreeRDP_DesktopWidth, AVC_MIN_DESKTOP_WIDTH );
        if( freerdp_settings_get_uint32( rfi->settings, FreeRDP_DesktopHeight ) < AVC_MIN_DESKTOP_HEIGHT )
            freerdp_settings_set_uint32( rfi->settings, FreeRDP_DesktopHeight, AVC_MIN_DESKTOP_HEIGHT );
    }
    ...
    if( remmina_plugin_service->file_get_string( remminafile, "username" ) )
        freerdp_settings_set_string(
            rfi->settings, FreeRDP_Username, remmina_plugin_service->file_get_string( remminafile, "username" ) );

    if( remmina_plugin_service->file_get_string( remminafile, "domain" ) )
        freerdp_settings_set_string(
            rfi->settings, FreeRDP_Domain, remmina_plugin_service->file_get_string( remminafile, "domain" ) );

    s = remmina_plugin_service->file_get_string( remminafile, "password" );
    if( s )
        freerdp_settings_set_string( rfi->settings, FreeRDP_Password, s );
    ...
    if( proxy_type && proxy_hostname )
    {
        ...
        REMMINA_PLUGIN_DEBUG( "ProxyType set to: %" PRIu32,
    ...
    {
        freerdp_settings_set_string(
            rfi->settings, FreeRDP_GatewayDomain, freerdp_settings_get_string( rfi->settings, FreeRDP_Domain ) );
        freerdp_settings_set_string(
            rfi->settings, FreeRDP_GatewayUsername, freerdp_settings_get_string( rfi->settings, FreeRDP_Username ) );
        freerdp_settings_set_string(
            rfi->settings, FreeRDP_GatewayPassword, freerdp_settings_get_string( rfi->settings, FreeRDP_Password ) );
    }
    ...
    freerdp_settings_set_uint32( rfi->settings,
                                 ...
                                 remmina_plugin_service->file_get_int(
                                     ...
                                     freerdp_settings_get_uint32( rfi->settings, FreeRDP_AuthenticationLevel ) ) );
    ...
    freerdp_settings_set_bool( rfi->settings,
                               ...
                               remmina_plugin_service->file_get_int( remminafile, "relax-order-checks", 0 ) );
    freerdp_settings_set_uint32( rfi->settings,
    ...
    if( remmina_plugin_service->file_get_string( remminafile, "exec" ) )
        freerdp_settings_set_string(
            rfi->settings, FreeRDP_AlternateShell, remmina_plugin_service->file_get_string( remminafile, "exec" ) );
    ...
    sm = g_strdup_printf( "rdp_quality_%i",
                          remmina_plugin_service->file_get_int( remminafile, "quality", DEFAULT_QUALITY_0 ) );
    ...
    {
        freerdp_settings_set_string(
            rfi->settings, FreeRDP_KeyboardRemappingList, remmina_plugin_service->pref_get_value( "rdp_kbd_remap" ) );
        REMMINA_PLUGIN_DEBUG( "rdp_keyboard_remapping_list: %s", rfi->settings->KeyboardRemappingList );
    }
#endif
    ...
    if( remmina_plugin_service->file_get_string( remminafile, "pth" ) )
    {
        ...
        freerdp_settings_set_bool( rfi->settings, FreeRDP_RestrictedAdminModeRequired, TRUE );
        freerdp_settings_set_string(
            rfi->settings, FreeRDP_PasswordHash, remmina_plugin_service->file_get_string( remminafile, "pth" ) );
    ...
    freerdp_settings_set_bool( rfi->settings,
                               ...
                               remmina_plugin_service->file_get_int( remminafile, "disableclipboard", FALSE ) ? FALSE
                                                                                                              : TRUE );
    ...
    if( remmina_plugin_service->file_get_int( remminafile, "multimon", FALSE ) )
    {
        ...
        if( monitorids != NULL && monitorids[0] != '\0' )
        {
            ...
            REMMINA_PLUGIN_DEBUG( "NumMonitorIds: %d",
                                  freerdp_settings_get_uint32( rfi->settings, FreeRDP_NumMonitorIds ) );
        ...
        {
            REMMINA_PLUGIN_DEBUG( "Cannot set Desktop Size, we are using the previously set values: %dx%d",
                                  freerdp_settings_get_uint32( rfi->settings, FreeRDP_DesktopWidth ),
                                  freerdp_settings_get_uint32( rfi->settings, FreeRDP_DesktopHeight ) );
        }
        remmina_plugin_service->protocol_plugin_set_width(
        ...
        if( !orphaned )
        {
            ...
            switch( e )
            {
                ...
                case FREERDP_ERROR_CONNECT_FAILED:
                    remmina_plugin_service->protocol_plugin_set_error(
                        gp,
                        _( "Lost connection to the RDP server “%s”." ),
                        freerdp_settings_get_string( rfi->settings, FreeRDP_ServerHostname ) );
                    break;
                case FREERDP_ERROR_DNS_NAME_NOT_FOUND:
                    remmina_plugin_service->protocol_plugin_set_error(
                        gp,
                        _( "Could not find the address for the RDP server “%s”." ),
                        freerdp_settings_get_string( rfi->settings, FreeRDP_ServerHostname ) );
                    break;
                case FREERDP_ERROR_TLS_CONNECT_FAILED:
                    remmina_plugin_service->protocol_plugin_set_error(
                        gp,
                        _( "Could not connect to the RDP server “%s” via TLS. See the DEBUG traces from a terminal for "
                           "more information." ),
                        freerdp_settings_get_string( rfi->settings, FreeRDP_ServerHostname ) );
                    break;
                case FREERDP_ERROR_SECURITY_NEGO_CONNECT_FAILED:
                    ...
                    remmina_plugin_service->protocol_plugin_set_error(
                        gp,
                        _( "Unable to establish a connection to the RDP server “%s”. Check “Security protocol "
                           "negotiation”." ),
                        freerdp_settings_get_string( rfi->settings, FreeRDP_ServerHostname ) );
                    break;
                ...
                case FREERDP_ERROR_SERVER_DENIED_CONNECTION:
                    remmina_plugin_service->protocol_plugin_set_error(
                        gp,
                        _( "The “%s” server refused the connection." ),
                        freerdp_settings_get_string( rfi->settings, FreeRDP_ServerHostname ) );
                    break;
#endif
                case 0x800759DB:
                    ...
                    remmina_plugin_service->protocol_plugin_set_error(
                        gp,
                        _( "The Remote Desktop Gateway “%s” denied the user “%s\\%s” access due to policy." ),
                        freerdp_settings_get_string( rfi->settings, FreeRDP_GatewayHostname ),
                        freerdp_settings_get_string( rfi->settings, FreeRDP_GatewayDomain ),
                        freerdp_settings_get_string( rfi->settings, FreeRDP_GatewayUsername ) );
                    break;
...
static gpointer remmina_rdp_main_thread( gpointer data )
{
    ...
    do
        remmina_rdp_main( gp );
    while( !remmina_plugin_service->protocol_plugin_has_error( gp ) && rfi->attempt_interactive_authentication == TRUE
           && !rfi->user_cancelled );
...
static int remmina_rdp_open_connection( RemminaProtocolWidget *gp )
{
    ...
    {
        remmina_plugin_service->protocol_plugin_set_error( gp, "%s", "Could not start pthread." );
...
static void remmina_rdp_call_feature( RemminaProtocolWidget *gp, const RemminaProtocolFeature *feature )
{
    ...
    switch( feature->id )
    {
        ...
        case REMMINA_RDP_FEATURE_TOOL_REFRESH:
            if( rfi )
                gtk_widget_queue_draw_area( rfi->drawing_area,
                                            0,
                                            0,
...
static const RemminaProtocolSetting remmina_rdp_basic_settings[] = {
    ...
    { REMMINA_PROTOCOL_SETTING_TYPE_CHECK,
      "span",
      N_( "Span screen over multiple monitors" ),
      TRUE,
      NULL,
      NULL,
      NULL,
...
static const RemminaProtocolSetting remmina_rdp_advanced_settings[] = {
    ...
    { REMMINA_PROTOCOL_SETTING_TYPE_SELECT,
      "security",
      N_( "Security protocol negotiation" ),
      FALSE,
      security_list,
      NULL },
    ...
    { REMMINA_PROTOCOL_SETTING_TYPE_TEXT,
      "audio-output",
      N_( "Redirect local audio output" ),
      TRUE,
      NULL,
      audio_tooltip },
    { REMMINA_PROTOCOL_SETTING_TYPE_TEXT,
      "microphone",
      N_( "Redirect local microphone" ),
      TRUE,
      NULL,
      microphone_tooltip },
    ...
    { REMMINA_PROTOCOL_SETTING_TYPE_TEXT,
      "gateway_username",
      N_( "Remote Desktop Gateway username" ),
      FALSE,
      NULL,
      NULL },
    { REMMINA_PROTOCOL_SETTING_TYPE_PASSWORD,
      "gateway_password",
      N_( "Remote Desktop Gateway password" ),
      FALSE,
      NULL,
      NULL },
    ...
    { REMMINA_PROTOCOL_SETTING_TYPE_CHECK,
      "serialpermissive",
      N_( "(SELinux) permissive mode for serial ports" ),
      TRUE,
      NULL,
      NULL },
    ...
    { REMMINA_PROTOCOL_SETTING_TYPE_CHECK,
      "gateway_usage",
      N_( "Server detection using Remote Desktop Gateway" ),
      TRUE,
      NULL,
      NULL },
#if defined( PROXY_TYPE_IGNORE )
    ...
    { REMMINA_PROTOCOL_SETTING_TYPE_CHECK,
      ...
      N_( "Turn off automatic reconnection" ),
      TRUE,
      NULL,
      NULL },
...
static RemminaFilePlugin remmina_rdpf = { REMMINA_PLUGIN_TYPE_FILE,       // Type
                                          "RDPF",                         // Name
