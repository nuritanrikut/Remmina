...
static void remmina_rdp_settings_kbd_init()
{
    ...
    rdp_keyboard_remapping_list = g_strdup( remmina_plugin_service->pref_get_value( "rdp_kbd_remap" ) );
...
#define REMMINA_RDPSET_GRID( obj ) \
    ( G_TYPE_CHECK_INSTANCE_CAST( ( obj ), REMMINA_TYPE_PLUGIN_RDPSET_GRID, RemminaPluginRdpsetGrid ) )
#define REMMINA_RDPSET_GRID_CLASS( klass ) \
    ( G_TYPE_CHECK_CLASS_CAST( ( klass ), REMMINA_TYPE_PLUGIN_RDPSET_GRID, RemminaPluginRdpsetGridClass ) )
#define REMMINA_IS_PLUGIN_RDPSET_GRID( obj ) ( G_TYPE_CHECK_INSTANCE_TYPE( ( obj ), REMMINA_TYPE_PLUGIN_RDPSET_GRID ) )
#define REMMINA_IS_PLUGIN_RDPSET_GRID_CLASS( klass ) \
    ( G_TYPE_CHECK_CLASS_TYPE( ( klass ), REMMINA_TYPE_PLUGIN_RDPSET_GRID ) )
...
static void remmina_rdp_settings_grid_destroy( GtkWidget *widget, gpointer data )
{
    ...
    remmina_plugin_service->pref_set_value(
        "rdp_use_client_keymap",
        gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( grid->use_client_keymap_check ) ) ? "1" : "0" );

    remmina_plugin_service->pref_set_value(
        "rdp_disable_smooth_scrolling",
    ...
    remmina_plugin_service->pref_set_value( "rdp_kbd_remap", gtk_entry_get_text( GTK_ENTRY( grid->kbd_remap ) ) );
...
static void remmina_rdp_settings_quality_option_on_toggled( GtkToggleButton *togglebutton,
                                                            RemminaPluginRdpsetGrid *grid )
{
    TRACE_CALL( __func__ );
    guint v;
...
static void remmina_rdp_settings_grid_init( RemminaPluginRdpsetGrid *grid )
{
    ...
    gtk_toggle_button_set_active( GTK_TOGGLE_BUTTON( widget ), s && s[0] == '1' ? TRUE : FALSE );
    g_free( s );
    ...
    gtk_widget_set_tooltip_text( widget,
                                 _( "List of key=value,… pairs to remap scancodes. E.g. 0x56=0x29,0x29=0x56" ) );
#else
    ...
    gtk_cell_layout_pack_start( GTK_CELL_LAYOUT( widget ), renderer, TRUE );
    gtk_cell_layout_add_attribute( GTK_CELL_LAYOUT( widget ), renderer, "text", 1 );
    g_signal_connect( G_OBJECT( widget ), "changed", G_CALLBACK( remmina_rdp_settings_quality_on_changed ), grid );
    ...
    g_signal_connect(
        G_OBJECT( widget ), "toggled", G_CALLBACK( remmina_rdp_settings_quality_option_on_toggled ), grid );
    ...
    g_signal_connect( G_OBJECT( widget ), "changed", G_CALLBACK( remmina_rdp_settings_appscale_on_changed ), grid );
    ...
    gtk_widget_set_tooltip_text( widget,
                                 _( "The maximum number of reconnect attempts upon an RDP disconnect (default: 20)" ) );
