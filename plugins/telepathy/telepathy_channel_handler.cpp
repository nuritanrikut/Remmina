...
static void remmina_tp_channel_handler_connect( RemminaTpChannelHandler *chandler )
{
    ...
    chandler->proto_widget = remmina_plugin_telepathy_service->open_connection(
        remminafile, G_CALLBACK( remmina_tp_channel_handler_on_disconnect ), chandler, &chandler->disconnect_handler );
}
...
remmina_tp_channel_handler_on_response( GtkDialog *dialog, gint response_id, RemminaTpChannelHandler *chandler )
{
    ...
    if( response_id == GTK_RESPONSE_YES )
    {
        ...
        tp_cli_channel_type_stream_tube_call_accept( chandler->channel,
                                                     -1,
...
remmina_tp_channel_handler_channel_ready( TpChannel *channel, const GError *channel_error, gpointer user_data )
{
    ...
    if( tp_cli_channel_connect_to_closed(
            channel, remmina_tp_channel_handler_channel_closed, chandler, NULL, NULL, &error )
        == NULL )
    {
    ...
    tp_connection_get_contacts_by_handle( chandler->connection,
                                          1,
                                          &handle,
                                                         ...
                                                         gpointer user_data )
{
    ...
    chandler->channel =
        tp_channel_new_from_properties( connection, chandler->channel_path, chandler->channel_properties, &error );
                                     ...
                                     DBusGMethodInvocation *context )
{
    ...
    tp_g_hash_table_update( chandler->channel_properties,
                            channel_properties,
