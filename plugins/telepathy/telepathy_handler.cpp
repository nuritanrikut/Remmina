...
G_DEFINE_TYPE_WITH_CODE( RemminaTpHandler,
                         remmina_tp_handler,
                         G_TYPE_OBJECT,
...
static void remmina_tp_handler_handle_channels( TpSvcClientHandler *handler,
...
static int remmina_tp_handler_register( RemminaTpHandler *handler )
{
    ...
    dbus_g_connection_register_g_object(
        tp_proxy_get_dbus_connection( TP_PROXY( bus ) ), REMMINA_TP_OBJECT_PATH, G_OBJECT( handler ) );
    g_object_unref( bus );
    g_print( "%s: bus_name " REMMINA_TP_BUS_NAME " object_path " REMMINA_TP_OBJECT_PATH "\n", __func__ );
    return TRUE;
}
