...
#    include <jsc/jsc.h>
#endif
#include "www_utils.hpp"
#include "www_plugin.hpp"
                                          ...
                                          RemminaProtocolWidget *gp )
{
    ...
    g_signal_connect(
        G_OBJECT( download ), "notify::response", G_CALLBACK( remmina_plugin_www_response_received ), gp );
    g_signal_connect( download, "created-destination", G_CALLBACK( remmina_plugin_www_notify_download ), gp );
}
...
static int remmina_plugin_www_decide_policy_cb( WebKitWebView *webview,
...
void remmina_plugin_www_decide_nav( WebKitPolicyDecision *decision, RemminaProtocolWidget *gp )
{
    ...
    switch( webkit_navigation_action_get_navigation_type( a ) )
    {
        case WEBKIT_NAVIGATION_TYPE_LINK_CLICKED:
            ...
            url = webkit_uri_request_get_uri( webkit_navigation_action_get_request( a ) );
        ...
        default:
            ...
            if( webkit_navigation_policy_decision_get_frame_name( WEBKIT_NAVIGATION_POLICY_DECISION( decision ) ) )
            {
                webkit_policy_decision_ignore( decision );
            }
            else
...
int remmina_plugin_www_decide_resource( WebKitPolicyDecision *decision, RemminaProtocolWidget *gp )
{
    ...
    ;
    const char *request_uri = webkit_uri_response_get_uri( response );
...
static int remmina_www_query_feature( RemminaProtocolWidget *gp, const RemminaProtocolFeature *feature )
{
    TRACE_CALL( __func__ );
    return TRUE;
}

static int remmina_plugin_www_load_failed_tls_cb( WebKitWebView *webview,
                                                  char *failing_uri,
                                                  GTlsCertificate *certificate,
                                                  GTlsCertificateFlags errors,
                                                  RemminaProtocolWidget *gp ) __attribute__( ( unused ) );
static int remmina_plugin_www_load_failed_tls_cb( WebKitWebView *webview,
                                                  char *failing_uri,
                                                  GTlsCertificate *certificate,
                                                  GTlsCertificateFlags errors,
...
static void remmina_plugin_www_init( RemminaProtocolWidget *gp )
{
    ...
    datapath = g_build_path(
        "/", g_path_get_dirname( remmina_plugin_service->file_get_path( remminafile ) ), PLUGIN_NAME, NULL );
    cache_dir = g_build_path( "/", datapath, "cache", NULL );
    REMMINA_PLUGIN_DEBUG( "WWW data path is %s", datapath );

    if( datapath )
    {
        char *indexeddb_dir = g_build_filename( datapath, "indexeddb", NULL );
        char *local_storage_dir = g_build_filename( datapath, "local_storage", NULL );
        char *applications_dir = g_build_filename( datapath, "applications", NULL );
        char *websql_dir = g_build_filename( datapath, "websql", NULL );
        gpdata->data_mgr = webkit_website_data_manager_new( "disk-cache-directory",
                                                            cache_dir,
                                                            "indexeddb-directory",
                                                            indexeddb_dir,
    ...
    if( remmina_plugin_service->file_get_string( remminafile, "server" ) )
        ...
        gpdata->url = strdup( "https://remmina.org" );
    ...
    {
        char *useragent = g_strdup( remmina_plugin_service->file_get_string( remminafile, "user-agent" ) );
    ...
    if( remmina_plugin_service->file_get_int( remminafile, "ignore-tls-errors", FALSE ) )
    {
        ...
        webkit_website_data_manager_set_tls_errors_policy( gpdata->data_mgr, WEBKIT_TLS_ERRORS_POLICY_IGNORE );
#else
    ...
    {
        char *proxyurl = g_strdup( remmina_plugin_service->file_get_string( remminafile, "proxy-url" ) );
    ...
    g_signal_connect(
        G_OBJECT( gpdata->context ), "download-started", G_CALLBACK( remmina_plugin_www_download_started ), gp );
}

static int
remmina_plugin_www_on_auth( WebKitWebView *webview, WebKitAuthenticationRequest *request, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    ...
    if( ret == GTK_RESPONSE_OK )
    {
        ...
        if( request )
        {
            gpdata->credentials = webkit_credential_new(
                g_strdup( s_username ), g_strdup( s_password ), WEBKIT_CREDENTIAL_PERSISTENCE_FOR_SESSION );
...
static void
remmina_plugin_www_form_auth( WebKitWebView *webview, WebKitLoadEvent load_event, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    char *s_js;
    GString *jsstr;
                ...
                if( www_js_file )
                {
                    ...
                    if( g_file_get_contents( www_js_file, &s_js, NULL, &error ) )
                    {
                        ...
                        else
                        {
                            ...
                            webkit_web_view_run_javascript( webview, s_js, NULL, remmina_www_web_view_js_finished, gp );
                            g_free( s_js );
                        }
                    }
                    else
...
static int remmina_plugin_www_close_connection( RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    RemminaPluginWWWData *gpdata;
    gpdata = (RemminaPluginWWWData *)g_object_get_data( G_OBJECT( gp ), "plugin-data" );
...
static int remmina_plugin_www_open_connection( RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    ...
    g_object_connect( G_OBJECT( gpdata->webview ),
                      "signal::create",
                      G_CALLBACK( remmina_plugin_www_on_create ),
                      gp,
                      "signal::load-changed",
                      G_CALLBACK( remmina_plugin_www_form_auth ),
                      gp,
                      "signal::authenticate",
                      G_CALLBACK( remmina_plugin_www_on_auth ),
                      gp,
                      "signal::decide-policy",
                      G_CALLBACK( remmina_plugin_www_decide_policy_cb ),
                      gp,
                      NULL );
...
static void remmina_plugin_www_save_snapshot( GObject *object, GAsyncResult *result, RemminaProtocolWidget *gp )
{
    ...
    www_utils_string_replace_all( pngstr, "%p", remmina_plugin_service->file_get_string( remminafile, "name" ) );
    www_utils_string_replace_all( pngstr, "%h", "URL" );
    www_utils_string_replace_all( pngstr, "%Y", g_strdup_printf( "%d", g_date_time_get_year( date ) ) );
    www_utils_string_replace_all( pngstr, "%m", g_strdup_printf( "%d", g_date_time_get_month( date ) ) );
    www_utils_string_replace_all( pngstr, "%d", g_strdup_printf( "%d", g_date_time_get_day_of_month( date ) ) );
    www_utils_string_replace_all( pngstr, "%H", g_strdup_printf( "%d", g_date_time_get_hour( date ) ) );
    www_utils_string_replace_all( pngstr, "%M", g_strdup_printf( "%d", g_date_time_get_minute( date ) ) );
    www_utils_string_replace_all( pngstr, "%S", g_strdup_printf( "%f", g_date_time_get_seconds( date ) ) );
...
static int remmina_plugin_www_get_snapshot( RemminaProtocolWidget *gp, RemminaPluginScreenshotData *rpsd )
{
    TRACE_CALL( __func__ );
    RemminaPluginWWWData *gpdata;
    gpdata = (RemminaPluginWWWData *)g_object_get_data( G_OBJECT( gp ), "plugin-data" );
...
static const RemminaProtocolSetting remmina_plugin_www_basic_settings[] = {
    { REMMINA_PROTOCOL_SETTING_TYPE_TEXT,
      ...
      FALSE,
      NULL,
      static_cast<void *>( const_cast<char *>( N_( "http://address or https://address" ) ) ),
      NULL,
      NULL },
...
static const RemminaProtocolSetting remmina_plugin_www_advanced_settings[] = {
    ...
    { REMMINA_PROTOCOL_SETTING_TYPE_TEXT,
      ...
      FALSE,
      NULL,
      static_cast<void *>( const_cast<char *>( N_( "E.g. https://example.org, socks://mysocks:1080" ) ) ) },
    ...
    { REMMINA_PROTOCOL_SETTING_TYPE_CHECK,
      "enable-smooth-scrolling",
      N_( "Turn on smooth scrolling" ),
      TRUE,
      NULL,
      NULL },
    { REMMINA_PROTOCOL_SETTING_TYPE_CHECK,
      "enable-spatial-navigation",
      N_( "Turn on spatial navigation" ),
      TRUE,
      NULL,
      NULL },
