...
remmina_plugin_spice_channel_open_fd( SpiceChannel *channel, int with_tls G_GNUC_UNUSED, RemminaProtocolWidget *gp )
{
    ...
    REMMINA_PLUGIN_DEBUG(
        "Opening channel %p %s %d with FD: %d", channel, g_type_name( G_OBJECT_TYPE( channel ) ), id, gpdata->fd );
...
static void remmina_plugin_spice_init( RemminaProtocolWidget *gp )
{
    ...
    g_signal_connect( gpdata->session, "channel-new", G_CALLBACK( remmina_plugin_spice_channel_new_cb ), gp );
    ...
    g_object_set( gpdata->session,
                  ...
                  "enable-audio",
                  remmina_plugin_service->file_get_int( remminafile, "enableaudio", FALSE ),
                  "enable-smartcard",
                  remmina_plugin_service->file_get_int( remminafile, "sharesmartcard", FALSE ),
                  "shared-dir",
                  remmina_plugin_service->file_get_string( remminafile, "sharefolder" ),
                  NULL );
...
static gboolean remmina_plugin_spice_open_connection( RemminaProtocolWidget *gp )
{
	...
	*/
    ...
    remmina_plugin_service->get_server_port( tunnel, XSPICE_DEFAULT_PORT, &host, &port );
...
static void
remmina_plugin_spice_channel_new_cb( SpiceSession *session, SpiceChannel *channel, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
	...
	*/
    ...
    if( SPICE_IS_MAIN_CHANNEL( channel ) )
    {
        ...
        g_signal_connect( channel, "channel-event", G_CALLBACK( remmina_plugin_spice_main_channel_event_cb ), gp );
        g_signal_connect(
            channel, "main-agent-update", G_CALLBACK( remmina_plugin_spice_agent_connected_event_cb ), gp );
#ifdef SPICE_GTK_CHECK_VERSION
#    if SPICE_GTK_CHECK_VERSION( 0, 31, 0 )
        g_signal_connect( channel, "new-file-transfer", G_CALLBACK( remmina_plugin_spice_file_transfer_new_cb ), gp );
#    endif /* SPICE_GTK_CHECK_VERSION(0, 31, 0) */
#endif     /* SPICE_GTK_CHECK_VERSION */
    }
    ...
    if( SPICE_IS_DISPLAY_CHANNEL( channel ) )
    {
        ...
        g_signal_connect( gpdata->display, "notify::ready", G_CALLBACK( remmina_plugin_spice_display_ready_cb ), gp );
        ...
        {
            g_signal_connect(
                channel, "gst-video-overlay", G_CALLBACK( remmina_plugin_spice_disable_gst_overlay ), gp );
        }
    }
...
static void
remmina_plugin_spice_main_channel_event_cb( SpiceChannel *channel, SpiceChannelEvent event, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    ...
    switch( event )
    {
        case SPICE_CHANNEL_CLOSED:
            remmina_plugin_service->get_server_port(
                remmina_plugin_service->file_get_string( remminafile, "server" ), XSPICE_DEFAULT_PORT, &server, &port );
            remmina_plugin_service->protocol_plugin_set_error(
                gp, _( "Disconnected from the SPICE server “%s”." ), server );
            g_free( server );
...
#        if SPICE_GTK_CHECK_VERSION( 0, 38, 0 )
            ...
            GArray *preferred_codecs =
                g_array_sized_new( FALSE, FALSE, sizeof( gint ), ( SPICE_VIDEO_CODEC_TYPE_ENUM_END - 1 ) );
...
#        elif SPICE_GTK_CHECK_VERSION( 0, 35, 0 )
            spice_display_channel_change_preferred_video_codec_type( SPICE_CHANNEL( gpdata->display_channel ),
                                                                     videocodec );
#        else
            spice_display_change_preferred_video_codec_type( SPICE_CHANNEL( gpdata->display_channel ), videocodec );
#        endif
        }
#    endif
#endif
...
#        if SPICE_GTK_CHECK_VERSION( 0, 35, 0 )
            spice_display_channel_change_preferred_compression( SPICE_CHANNEL( gpdata->display_channel ),
                                                                imagecompression );
#        else
            spice_display_change_preferred_compression( SPICE_CHANNEL( gpdata->display_channel ), imagecompression );
#        endif
        }
#    endif
#endif
...
static void remmina_plugin_spice_keystroke( RemminaProtocolWidget *gp, const guint keystrokes[], const gint keylen )
{
    ...
    if( gpdata->display )
    {
        spice_display_send_keys( gpdata->display, keystrokes, keylen, SPICE_DISPLAY_KEY_EVENT_CLICK );
    }
}
...
static void remmina_plugin_spice_update_scale_mode( RemminaProtocolWidget *gp )
{
    ...
    else
    {
        ...
        g_object_get( gpdata->display_channel, "width", &width, "height", &height, NULL );
...
#    if SPICE_GTK_CHECK_VERSION( 0, 35, 0 )
    ...
    { REMMINA_PROTOCOL_SETTING_TYPE_CHECK,
      "disablegstvideooverlay",
      N_( "Turn off GStreamer overlay" ),
      FALSE,
      NULL,
...
#    if SPICE_GTK_CHECK_VERSION( 0, 31, 0 )
    { REMMINA_PROTOCOL_SETTING_TYPE_SELECT,
      "imagecompression",
      N_( "Preferred image compression" ),
      FALSE,
