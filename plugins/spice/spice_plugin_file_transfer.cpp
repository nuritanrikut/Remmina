...
static void
remmina_plugin_spice_file_transfer_finished_cb( SpiceFileTransferTask *, GError *, RemminaProtocolWidget * );
static void remmina_plugin_spice_file_transfer_progress_cb( GObject *, GParamSpec *, RemminaProtocolWidget * );

struct RemminaPluginSpiceXferWidgets
{
    GtkWidget *vbox;
    GtkWidget *hbox;
    GtkWidget *progress;
...
void remmina_plugin_spice_file_transfer_new_cb( SpiceMainChannel *main_channel,
                                                ...
                                                RemminaProtocolWidget *gp )
{
    ...
    {
        gpdata->file_transfers = g_hash_table_new_full(
            g_direct_hash, g_direct_equal, g_object_unref, (GDestroyNotify)remmina_plugin_spice_xfer_widgets_free );
    }
    ...
    if( !gpdata->file_transfer_dialog )
    {
		 ...
		 */
        gpdata->file_transfer_dialog = gtk_dialog_new_with_buttons(
            _( "File Transfers" ), NULL, static_cast<GtkDialogFlags>( 0 ), _( "_Cancel" ), GTK_RESPONSE_CANCEL, NULL );
...
static void
remmina_plugin_spice_file_transfer_dialog_response_cb( GtkDialog *dialog, gint response, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
...
static void
remmina_plugin_spice_file_transfer_progress_cb( GObject *task, GParamSpec *param_spec, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
...
static void
remmina_plugin_spice_file_transfer_finished_cb( SpiceFileTransferTask *task, GError *error, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    ...
    else
    {
        ...
        notification_message = g_strdup_printf( _( "The %s file has been transferred" ), filename );
    }

    g_notification_set_body( notification, notification_message );
    g_application_send_notification(
        g_application_get_default(), "remmina-plugin-spice-file-transfer-finished", notification );
