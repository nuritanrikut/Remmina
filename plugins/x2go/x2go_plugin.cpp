...
#define FEATURE_AVAILABLE( gpdata, feature ) \
    gpdata->available_features \
        ? ( g_list_find_custom( gpdata->available_features, feature, (GCompareFunc)g_strcmp0 ) ? TRUE : FALSE ) \
        : FALSE

#define FEATURE_NOT_AVAIL_STR( feature ) \
    g_strdup_printf( _( "The command-line feature '%s' is not available! Attempting " \
                        "to start PyHoca-CLI without using this feature…" ), \
                     feature )
...
#define SET_RESUME_SESSION( gp, resume_data ) \
    g_object_set_data_full( G_OBJECT( gp ), "resume-session-data", resume_data, g_free )

#define GET_RESUME_SESSION( gp ) (char *)g_object_get_data( G_OBJECT( gp ), "resume-session-data" )

// A session is selected if the returning value is something other than 0.
#define IS_SESSION_SELECTED( gp ) g_object_get_data( G_OBJECT( gp ), "session-selected" ) ? TRUE : FALSE
...
#define SET_SESSION_SELECTED( gp, is_session_selected ) \
    g_object_set_data_full( G_OBJECT( gp ), "session-selected", is_session_selected, NULL )
// -------------------

#define REMMINA_PLUGIN_INFO( fmt, ... ) rm_plugin_service->_remmina_info( "[%s] " fmt, PLUGIN_NAME, ##__VA_ARGS__ )

#define REMMINA_PLUGIN_MESSAGE( fmt, ... ) \
    rm_plugin_service->_remmina_message( "[%s] " fmt, PLUGIN_NAME, ##__VA_ARGS__ )

#define REMMINA_PLUGIN_DEBUG( fmt, ... ) \
    rm_plugin_service->_remmina_debug( __func__, "[%s] " fmt, PLUGIN_NAME, ##__VA_ARGS__ )

#define REMMINA_PLUGIN_WARNING( fmt, ... ) \
    rm_plugin_service->_remmina_warning( __func__, "[%s] " fmt, PLUGIN_NAME, ##__VA_ARGS__ )

#define REMMINA_PLUGIN_ERROR( fmt, ... ) \
    rm_plugin_service->_remmina_error( __func__, "[%s] " fmt, PLUGIN_NAME, ##__VA_ARGS__ )

#define REMMINA_PLUGIN_CRITICAL( fmt, ... ) \
    rm_plugin_service->_remmina_critical( __func__, "[%s] " fmt, PLUGIN_NAME, ##__VA_ARGS__ )

#define GET_PLUGIN_STRING( value ) g_strdup( rm_plugin_service->file_get_string( remminafile, value ) )

#define GET_PLUGIN_PASSWORD( value ) GET_PLUGIN_STRING( value )

#define GET_PLUGIN_INT( value, default_value ) rm_plugin_service->file_get_int( remminafile, value, default_value )

#define GET_PLUGIN_BOOLEAN( value ) rm_plugin_service->file_get_int( remminafile, value, FALSE )

static RemminaPluginService *rm_plugin_service = NULL;

struct RemminaPluginX2GoData
{
    GtkWidget *socket;
    gint socket_id;
 ...
 */
str2int_errno str2int( gint *out, char *s, gint base )
{
    char *end;
 ...
 * static int rmplugin_x2go_test_callback(RemminaProtocolWidget *gp, gint response_id, \n
 *					       GtkDialog *self) { \n
 ...
 *				dialog_data -> dialog data (_DialogData*)
 ...
 */
static int rmplugin_x2go_open_dialog( X2GoCustomUserData *custom_data )
{
    REMMINA_PLUGIN_DEBUG( "Function entry." );

    if( !custom_data || !custom_data->gp || !custom_data->dialog_data )
    {
        REMMINA_PLUGIN_CRITICAL(
            "%s", g_strdup_printf( _( "Internal error: %s" ), _( "Parameter 'custom_data' is not initialized!" ) ) );

        return G_SOURCE_REMOVE;
    }

    RemminaProtocolWidget *gp = (RemminaProtocolWidget *)custom_data->gp;
    _DialogData *ddata = (_DialogData *)custom_data->dialog_data;
    ...
    {
        widget_gtk_dialog =
            gtk_message_dialog_new( ddata->parent, ddata->flags, ddata->type, ddata->buttons, "%s", ddata->title );

        gtk_message_dialog_format_secondary_text( GTK_MESSAGE_DIALOG( widget_gtk_dialog ), "%s", ddata->message );
    }
    ...
    if( ddata->callbackfunc )
    {
        g_signal_connect_swapped(
            G_OBJECT( widget_gtk_dialog ), "response", G_CALLBACK( ddata->callbackfunc ), custom_data );
    }
    else
    {
        g_signal_connect( G_OBJECT( widget_gtk_dialog ), "response", G_CALLBACK( gtk_widget_destroy ), NULL );
    }
 ...
 * @return GtkWidget*
 */
static GtkWidget *rmplugin_x2go_find_child( GtkWidget *parent, const char *name )
{
    const char *parent_name = gtk_widget_get_name( (GtkWidget *)parent );
    if( g_ascii_strcasecmp( parent_name, (char *)name ) == 0 )
    {
        return parent;
    }
 ...
 */
static int rmplugin_x2go_session_chooser_row_activated( GtkTreeView *treeview,
                                                        GtkTreePath *path,
                                                        ...
                                                        X2GoCustomUserData *custom_data )
{
    ...
    {
        gtk_tree_model_get( GTK_TREE_MODEL( model ), &iter, SESSION_SESSION_ID, &session_id, -1 );
        ...
        // stored, a session is selected. So we use the gpointer as a bool.
 ...
 * @return char* Translated display name. (Can be NULL, if session_property is invalid!)
 */
static char *rmplugin_x2go_session_property_to_string( guint session_property )
{
    ...
    switch( session_property )
    {
        ...
        case SESSION_DISPLAY:
            return_char = g_strdup( _( "X Display" ) );
            break;
        case SESSION_STATUS:
            return_char = g_strdup( _( "Status" ) );
            break;
        case SESSION_SESSION_ID:
            return_char = g_strdup( _( "Session ID" ) );
            break;
        case SESSION_CREATE_DATE:
            return_char = g_strdup( _( "Create date" ) );
            break;
        case SESSION_SUSPENDED_SINCE:
            return_char = g_strdup( _( "Suspended since" ) );
            break;
        case SESSION_AGENT_PID:
            return_char = g_strdup( _( "Agent PID" ) );
            break;
        case SESSION_USERNAME:
            return_char = g_strdup( _( "Username" ) );
            break;
        case SESSION_HOSTNAME:
            return_char = g_strdup( _( "Hostname" ) );
            break;
        case SESSION_COOKIE:
            return_char = g_strdup( _( "Cookie" ) );
            break;
        case SESSION_GRAPHIC_PORT:
            return_char = g_strdup( _( "Graphic port" ) );
            break;
        case SESSION_SND_PORT:
            return_char = g_strdup( _( "SND port" ) );
            break;
        case SESSION_SSHFS_PORT:
            return_char = g_strdup( _( "SSHFS port" ) );
            break;
        case SESSION_DIALOG_IS_VISIBLE:
            return_char = g_strdup( _( "Visible" ) );
            break;
    }
...
static GtkWidget *rmplugin_x2go_choose_session_dialog_factory( X2GoCustomUserData *custom_data, GList *sessions_list )
{
    ...
    if( !custom_data || !custom_data->gp || !custom_data->dialog_data || !custom_data->connect_data )
    {
        REMMINA_PLUGIN_CRITICAL(
            "%s", g_strdup_printf( _( "Internal error: %s" ), _( "Parameter 'custom_data' is not initialized!" ) ) );
    ...
    if( !ddata || !sessions_list || !ddata->title )
    {
        REMMINA_PLUGIN_CRITICAL( "%s",
                                 _( "Couldn't retrieve valid `DialogData` or "
                                    "`sessions_list`! Aborting…" ) );
        return NULL;
    }

    GtkWidget *widget_gtk_dialog = NULL;
    widget_gtk_dialog = gtk_dialog_new_with_buttons( ddata->title,
                                                     ddata->parent,
                                                     ddata->flags,
    ...
    GtkWidget *button =
        gtk_dialog_get_widget_for_response( GTK_DIALOG( widget_gtk_dialog ), SESSION_CHOOSER_RESPONSE_TERMINATE );
...
#define DEFAULT_DIALOG_HEIGHT ( DEFAULT_DIALOG_WIDTH * 9 ) / 16
    ...
    gtk_widget_set_size_request( GTK_WIDGET( widget_gtk_dialog ), DEFAULT_DIALOG_WIDTH, DEFAULT_DIALOG_HEIGHT );
    gtk_window_set_default_size( GTK_WINDOW( widget_gtk_dialog ), DEFAULT_DIALOG_WIDTH, DEFAULT_DIALOG_HEIGHT );
    ...
    gtk_box_pack_start( GTK_BOX( gtk_dialog_get_content_area( GTK_DIALOG( widget_gtk_dialog ) ) ),
    ...
    for( guint i = 0; i < SESSION_NUM_PROPERTIES; ++i )
    {
        ...
        if( !header_title )
        {
            REMMINA_PLUGIN_WARNING(
                "%s",
                g_strdup_printf( _( "Internal error: %s" ), g_strdup_printf( _( "Unknown property '%i'" ), i ) ) );
    ...
    for( elem = sessions_list; elem; elem = elem->next )
    {
        ...
        for( gint i = 0; i < SESSION_NUM_PROPERTIES; i++ )
        {
            ...
            if( i == SESSION_DIALOG_IS_VISIBLE )
            {
                ...
                g_assert( G_VALUE_HOLDS_BOOLEAN( &a )
                          && "GValue does not "
                             "hold a boolean!" );
    ...
    g_signal_connect(
        tree_view, "row-activated", G_CALLBACK( rmplugin_x2go_session_chooser_row_activated ), custom_data );
...
static GtkTreeModelFilter *rmplugin_x2go_session_chooser_get_filter_model( GtkWidget *dialog, GtkTreeView *treeview )
{
    ...
    if( !treeview && dialog )
    {
        GtkWidget *treeview_new = rmplugin_x2go_find_child( GTK_WIDGET( dialog ), "session_chooser_treeview" );

        if( !treeview_new )
        {
            REMMINA_PLUGIN_CRITICAL( "%s",
                                     g_strdup_printf( _( "Internal error: %s" ),
...
static GtkTreePath *rmplugin_x2go_session_chooser_get_selected_row( GtkWidget *dialog )
{
    ...
    GtkWidget *treeview = rmplugin_x2go_find_child( GTK_WIDGET( dialog ), "session_chooser_treeview" );
    if( !treeview )
    {
        REMMINA_PLUGIN_CRITICAL( "%s",
                                 g_strdup_printf( _( "Internal error: %s" ),
    ...
    {
        REMMINA_PLUGIN_CRITICAL(
            "%s", g_strdup_printf( _( "Internal error: %s" ), _( "Couldn't get currently selected row (session)!" ) ) );
        return NULL;
    }

    GtkTreeModelFilter *filter = rmplugin_x2go_session_chooser_get_filter_model( NULL, GTK_TREE_VIEW( treeview ) );
    GtkTreeModel *model = gtk_tree_model_filter_get_model( filter );
    if( !model )
        return NULL; // error message was already handled.
    ...
    if( selected_rows_num != 1 )
    {
        REMMINA_PLUGIN_CRITICAL(
            "%s",
            g_strdup_printf( _( "Internal error: %s" ),
                             g_strdup_printf( _( "Exactly one session should be selectable but '%i' rows "
...
static GValue rmplugin_x2go_session_chooser_get_property( GtkWidget *dialog, gint property_index, GtkTreePath *row )
{
    ...
    if( !row )
    {
        ...
        if( !selected_row )
            return ret_value; // error message was already handled.
        row = selected_row;
    }
    ...
    GtkTreeIter iter;
    bool success = gtk_tree_model_get_iter( model, &iter, row );
    if( !success )
    {
        REMMINA_PLUGIN_CRITICAL( "%s",
                                 g_strdup_printf( _( "Internal error: %s" ), _( "Failed to fill 'GtkTreeIter'." ) ) );
						  ...
						  GtkTreeIter *iter, GtkWidget *dialog)
{
	...
	for (guint i = 0; i < SESSION_NUM_PROPERTIES; i++) {
		...
		char* display_name = rmplugin_x2go_session_property_to_string(i);
 ...
 */
static char *rmplugin_x2go_spawn_pyhoca_process( guint argc, char *argv[], GError **error, char **env )
{
    REMMINA_PLUGIN_DEBUG( "Function entry." );

    if( !argv )
    {
        char *errmsg = g_strdup_printf( _( "Internal error: %s" ), _( "parameter 'argv' is 'NULL'." ) );
    ...
    if( !error )
    {
        ...
        REMMINA_PLUGIN_CRITICAL( "%s",
                                 g_strdup_printf( _( "Internal error: %s" ), _( "parameter 'error' is 'NULL'." ) ) );
        return NULL;
    }
    ...
    {
        char *errmsg =
            g_strdup_printf( _( "Internal error: %s" ), _( "parameter 'env' is either invalid or uninitialized." ) );
        REMMINA_PLUGIN_CRITICAL( "%s", errmsg );
        g_set_error( error, 1, 1, "%s", errmsg );
        return NULL;
    }
    ...
    bool success_ret = g_spawn_sync(
        NULL, argv, env, G_SPAWN_SEARCH_PATH, NULL, NULL, &standard_out, &standard_err, &exit_code, error );
	...
	*/
    ...
    {
        if( g_str_has_prefix( standard_err,
                              "pyhoca-cli: error: a socket error "
                              "occured while establishing the connection:" ) )
        {
            // Log error into GUI.
            char *errmsg = g_strdup_printf( _( "The necessary PyHoca-CLI process has encountered a "
        ...
        {
            char *errmsg = g_strdup_printf( _( "Could not start "
                                               "PyHoca-CLI:\n%s" ),
                                            standard_err );
            REMMINA_PLUGIN_CRITICAL( "%s", errmsg );
            g_set_error( error, 1, 1, "%s", errmsg );
    ...
    else if( !success_ret || ( *error ) || strlen( standard_out ) <= 0 || exit_code )
    {
        if( !( *error ) )
        {
            char *errmsg = g_strdup_printf( _( "An unknown error occured while trying "
        ...
        {
            char *errmsg = g_strdup_printf( _( "An unknown error occured while trying to start "
 ...
 * @param value TRUE = row is visible & FALSE = row is invisible (bool)
 * @param dialog Session-Chooser-Dialog (GtkDialog*)
 * @return Returns TRUE if successful. (bool)
 */
static int rmplugin_x2go_session_chooser_set_row_visible( GtkTreePath *path, bool value, GtkDialog *dialog )
{
    REMMINA_PLUGIN_DEBUG( "Function entry." );
    ...
    GtkTreeModelFilter *filter = rmplugin_x2go_session_chooser_get_filter_model( GTK_WIDGET( dialog ), NULL );
    GtkTreeModel *model = gtk_tree_model_filter_get_model( filter );
    ...
    if( !gtk_tree_model_get_iter( GTK_TREE_MODEL( model ), &iter, path ) )
    {
        REMMINA_PLUGIN_CRITICAL(
            "%s",
            g_strdup_printf( _( "Internal error: %s" ), _( "GtkTreePath 'path' describes a non-existing row!" ) ) );
        return FALSE;
    }

    // Make session either visible or invisible.
    gtk_list_store_set( GTK_LIST_STORE( model ), &iter, SESSION_DIALOG_IS_VISIBLE, value, -1 );
    ...
    GtkWidget *term_button =
        gtk_dialog_get_widget_for_response( GTK_DIALOG( dialog ), SESSION_CHOOSER_RESPONSE_TERMINATE );
    GtkWidget *resume_button =
        gtk_dialog_get_widget_for_response( GTK_DIALOG( dialog ), SESSION_CHOOSER_RESPONSE_CHOOSE );
 ...
 */
static int rmplugin_x2go_pyhoca_terminate_session( X2GoCustomUserData *custom_data )
{
    REMMINA_PLUGIN_DEBUG( "Function entry." );
        ...
        || !custom_data->opt1 || !custom_data->opt2 )
    {
        REMMINA_PLUGIN_CRITICAL(
            "%s",
            g_strdup_printf( _( "Internal error: %s" ), _( "Parameter 'custom_data' is not fully initialized!" ) ) );
    ...
    //_DialogData *ddata = (_DialogData*) custom_data->dialog_data;
    _ConnectionData *connect_data = (_ConnectionData *)custom_data->connect_data;
    ...
    if( !connect_data || !connect_data->host || !connect_data->username || !connect_data->password
        || strlen( connect_data->host ) <= 0 || strlen( connect_data->username ) <= 0 )
    // Allow empty passwords. Maybe the user wants to connect via public key?
    {
        REMMINA_PLUGIN_CRITICAL( "%s",
                                 g_strdup_printf( _( "Internal error: %s" ), _( "'Invalid connection data.'" ) ) );
    ...
    GValue value = rmplugin_x2go_session_chooser_get_property( GTK_WIDGET( dialog ), SESSION_SESSION_ID, selected_row );
    // error message was handled already.
    if( !G_VALUE_HOLDS_STRING( &value ) )
        return G_SOURCE_REMOVE;
    const char *session_id = g_value_get_string( &value );
    ...
    if( error )
    {
        char *err_msg = g_strdup_printf(
            _( "An error occured while trying to terminate X2Go session '%s':\n%s" ), session_id, error->message );

        REMMINA_PLUGIN_CRITICAL( "%s", err_msg );

        _DialogData *err_ddata = g_new0( _DialogData, 1 );
		...
		*/
        ...
        if( !rmplugin_x2go_session_chooser_set_row_visible( selected_row, TRUE, dialog ) )
        {
            // error message was already handled.
            return G_SOURCE_REMOVE;
        }
 ...
 *
 * @return bool, #G_SOURCE_CONTINUE and #G_SOURCE_REMOVE are more memorable
 *	   names for the return value. See GLib docs. \n
 *	   https://docs.gtk.org/glib/const.SOURCE_REMOVE.html
 */
static int rmplugin_x2go_session_chooser_callback( X2GoCustomUserData *custom_data, gint response_id, GtkDialog *self )
{
    REMMINA_PLUGIN_DEBUG( "Function entry." );
    ...
    if( response_id == SESSION_CHOOSER_RESPONSE_NEW )
    {
        ...
        // than 0 stored, a session is selected. So we use the gpointer as a bool.
        SET_SESSION_SELECTED( gp, (gpointer)TRUE );
    ...
    GValue value = rmplugin_x2go_session_chooser_get_property( GTK_WIDGET( self ),
                                                               SESSION_SESSION_ID,
    ...
    char *session_id = (char *)g_value_get_string( &value );

    if( response_id == SESSION_CHOOSER_RESPONSE_CHOOSE )
    {
        if( !session_id || strlen( session_id ) <= 0 )
        {
            REMMINA_PLUGIN_DEBUG( "%s", _( "Couldn't get session ID from session chooser dialog." ) );
            SET_RESUME_SESSION( gp, NULL );
        }
        else
        {
            ...
            REMMINA_PLUGIN_MESSAGE( "%s", g_strdup_printf( _( "Resuming session: '%s'" ), session_id ) );
        }
    }
    else if( response_id == SESSION_CHOOSER_RESPONSE_TERMINATE )
    {
        ...
        else
        {
            ...
            REMMINA_PLUGIN_MESSAGE( "%s", g_strdup_printf( _( "Terminating session: '%s'" ), session_id ) );
        }

        GtkTreePath *path = rmplugin_x2go_session_chooser_get_selected_row( GTK_WIDGET( self ) );
        // error message was already handled.
        if( !path )
            return G_SOURCE_REMOVE;
        ...
        g_thread_new( "terminate-session-thread", (GThreadFunc)rmplugin_x2go_pyhoca_terminate_session, custom_data );
...
static int onMainThread_cb(  onMainThread_cb_data *d )
{
    TRACE_CALL( __func__ );
    if( !d->cancelled )
    {
...
{
    TRACE_CALL( __func__ );
     onMainThread_cb_data *d = data;
    d->cancelled = TRUE;
}

static void onMainThread_schedule_callback_and_wait(  onMainThread_cb_data *d )
{
    TRACE_CALL( __func__ );
    d->cancelled = FALSE;
...
static void onMainThread_gtk_socket_add_id( GtkSocket *sk, Window w )
{
    TRACE_CALL( __func__ );
     ...
     onMainThread_cb_data *d;

    d = g_new0(  onMainThread_cb_data, 1 );
    d->func = onMainThread_cb_data::FUNC_GTK_SOCKET_ADD_ID;
    d->sk = sk;
    d->w = w;
...
static void rmplugin_x2go_remove_window_id( Window window_id )
{
    ...
    for( i = 0; i < remmina_x2go_window_id_array->len; i++ )
    {
        ...
        {
            already_seen = TRUE;
            REMMINA_PLUGIN_DEBUG( "Window of X2Go Agent with ID [0x%lx] seen already.", window_id );
            break;
        }
    }
    ...
    if( already_seen )
    {
        ...
        REMMINA_PLUGIN_DEBUG( "Forgetting about window of X2Go Agent with ID [0x%lx]…", window_id );
    }
 ...
 *	     names for the return value.
 */
static int rmplugin_x2go_cleanup( RemminaProtocolWidget *gp )
{
    REMMINA_PLUGIN_DEBUG( "Function entry." );
...
static int rmplugin_x2go_close_connection( RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    RemminaPluginX2GoData *gpdata = GET_PLUGIN_DATA( gp );
...
static void rmplugin_x2go_pyhoca_cli_exited( GPid pid, gint status, RemminaProtocolWidget *gp )
{
    ...
    REMMINA_PLUGIN_CRITICAL( "%s",
                             _( "PyHoca-CLI exited unexpectedly. "
                                "This connection will now be closed." ) );

    _DialogData *ddata = g_new0( _DialogData, 1 );
 ...
 */
static int rmplugin_x2go_save_credentials( RemminaFile *remminafile, char *s_username, char *s_password, char *errmsg )
{
    ...
    if( s_password && s_username )
    {
        ...
        rm_plugin_service->file_set_string( remminafile, "password", s_password );
        rm_plugin_service->file_set_string( remminafile, "username", s_username );
    }
    else
    {
        g_strlcpy( errmsg, g_strdup_printf( _( "Internal error: %s" ), _( "Could not save new credentials." ) ), 512 );

        REMMINA_PLUGIN_CRITICAL( "%s",
                                 _( "An error occured while trying to save "
 ...
 */
static int
rmplugin_x2go_get_auth( RemminaProtocolWidget *gp, char **errmsg, char **default_username, char **default_password )
{
    REMMINA_PLUGIN_DEBUG( "Function entry." );
    ...
    {
        ( *errmsg ) =
            g_strdup_printf( _( "Internal error: %s" ), _( "Parameter 'default_username' is uninitialized." ) );
        REMMINA_PLUGIN_CRITICAL( "%s", errmsg );
        return FALSE;
    }
    ...
    gint ret;
    bool save;
    bool disable_password_storing;
    RemminaFile *remminafile;

    remminafile = rm_plugin_service->protocol_plugin_get_file( gp );

    disable_password_storing = rm_plugin_service->file_get_int( remminafile, "disablepasswordstoring", FALSE );

    ret = rm_plugin_service->protocol_plugin_init_auth(
        gp,
        ( disable_password_storing ? 0
                                   : REMMINA_MESSAGE_PANEL_FLAG_SAVEPASSWORD | REMMINA_MESSAGE_PANEL_FLAG_USERNAME ),
    ...
    if( ret == GTK_RESPONSE_OK )
    {
        ...
        if( save )
        {
            if( !rmplugin_x2go_save_credentials( remminafile, s_username, s_password, ( *errmsg ) ) )
            {
                return FALSE;
            }
        }
 ...
 * @param connect_data _ConnectionData* which stores all necessary information
 ...
 */
static char *
rmplugin_x2go_get_pyhoca_sessions( RemminaProtocolWidget *gp, GError **error, _ConnectionData *connect_data )
{
    REMMINA_PLUGIN_DEBUG( "Function entry." );
    RemminaPluginX2GoData *gpdata = GET_PLUGIN_DATA( gp );
    ...
    {
        g_set_error(
            error, 1, 1, "%s", g_strdup_printf( _( "Internal error: %s" ), _( "'Invalid connection data.'" ) ) );
        return NULL;
    }
    else
    {
    ...
    char **envp = g_get_environ();
    char *envp_splitted = g_strjoinv( ";", envp );
	...
	*/
    ...
    char *std_out = rmplugin_x2go_spawn_pyhoca_process( argc, argv, error, envp );
    g_strfreev( envp );
 ...
 */
static GList *
rmplugin_x2go_parse_pyhoca_sessions( RemminaProtocolWidget *gp, GError **error, _ConnectionData *connect_data )
{
    REMMINA_PLUGIN_DEBUG( "Function entry." );

    char *pyhoca_output = NULL;
    ...
    char **lines_list = g_strsplit( pyhoca_output, "\n", -1 );
    ...
    if( lines_list == NULL || lines_list[0] == NULL || lines_list[1] == NULL )
    {
        g_set_error( error,
                     ...
                     1,
                     "%s",
                     _( "Couldn't parse the output of PyHoca-CLI's "
    ...
    {
        char *current_line = lines_list[i];
        ...
        if( g_str_has_prefix( current_line, "Session Name: " ) )
        {
            ...
            char **line_list = g_strsplit( current_line, ": ", 0 );

            if( line_list == NULL || line_list[0] == NULL || line_list[1] == NULL || strlen( line_list[0] ) <= 0
            ...
            session = malloc( sizeof( char * ) * ( SESSION_NUM_PROPERTIES + 1 ) );
            if( !session )
            {
                REMMINA_PLUGIN_CRITICAL( "%s",
                                         _( "Couldn't allocate "
            ...
            REMMINA_PLUGIN_INFO( "%s",
                                 g_strdup_printf( _( "Found already existing X2Go session with ID: '%s'" ),
 ...
 * @return char* ID of session. Can be 'NULL' but then 'error' is set.
 */
static char *
rmplugin_x2go_ask_session( RemminaProtocolWidget *gp, GError **error, _ConnectionData *connect_data )
{
    if( !connect_data || !connect_data->host || !connect_data->username || !connect_data->password
        || strlen( connect_data->host ) <= 0 || strlen( connect_data->username ) <= 0 )
    // Allow empty passwords. Maybe the user wants to connect via public key?
    ...
    // int factory(X2GoCustomUserData*, gpointer)
    // 	X2GoCustomUserData*:
    //		gp -> gp (RemminaProtocolWidget*)
    //		dialog_data -> dialog data (_DialogData*)
    //		connect_data -> connection data (_ConnectionData*)
    ...
    char *chosen_resume_session = GET_RESUME_SESSION( gp );
...
static int rmplugin_x2go_exec_x2go( char *host,
                                    gint sshport,
                                    char *username,
                                    char *password,
                                    char *command,
                                    ...
                                    char *errmsg )
{
    ...
    _ConnectionData *connect_data = g_new0( _ConnectionData, 1 );
    connect_data->host = host;
    connect_data->username = username;
    connect_data->password = password;

    GError *session_error = NULL;
    char *resume_session_id = rmplugin_x2go_ask_session( gp, &session_error, connect_data );
    ...
    if( !resume_session_id || session_error || strlen( resume_session_id ) <= 0 )
    {
        ...
        REMMINA_PLUGIN_WARNING( "%s",
                                g_strdup_printf( _( "A non-critical error happened: %s" ), session_error->message ) );
    }
    else
    {
        REMMINA_PLUGIN_INFO( "%s",
                             g_strdup_printf( _( "User chose to resume session with ID: '%s'" ), resume_session_id ) );
    }
    ...
    if( FEATURE_AVAILABLE( gpdata, "DPI" ) )
    {
        ...
        {
            g_strlcpy( errmsg,
                       _( "DPI setting is out of bounds. Please adjust "
    ...
    GError *error = NULL;
    char **envp = g_get_environ();
    bool success = g_spawn_async_with_pipes( NULL,
                                             argv,
                                             envp,
    ...
    if( !success || error )
    {
        ...
        if( !error )
            error = g_error_new( 0, 0, _( "Internal error." ) );

        char *error_title = _( "An error occured while "
        ...
        ddata->message = g_strdup_printf( _( "Could not start PyHoca-CLI (%i): '%s'" ), error->code, error->message );
        // We don't need the response.
        ddata->callbackfunc = NULL;
        // We don't need a custom dialog either.
        ddata->dialog_factory_func = NULL;
    ...
    g_child_watch_add( gpdata->pidx2go, (GChildWatchFunc)rmplugin_x2go_pyhoca_cli_exited, gp );
...
#define AMOUNT_FEATURES 43
    char *features[AMOUNT_FEATURES] = { "ADD_TO_KNOWN_HOSTS",
                                        "AUTH_ATTEMPTS",
                                        "BROKER_PASSWORD",
                                        "BROKER_URL",
                                        "CLEAN_SESSIONS",
...
static GList *rmplugin_x2go_populate_available_features_list()
{
    ...
    char **envp = g_get_environ();
    char *features_string = rmplugin_x2go_spawn_pyhoca_process( argc, argv, &error, envp );
    g_strfreev( envp );
    ...
    {
        char **features_list = g_strsplit( features_string, "\n", 0 );

        if( features_list == NULL )
        {
            char *error_msg = _( "Could not parse PyHoca-CLI's command-line "
        ...
        REMMINA_PLUGIN_INFO( "%s",
                             _( "Retrieved the following PyHoca-CLI "
                                "command-line features:" ) );

        for( int k = 0; features_list[k] != NULL; k++ )
        {
            // Filter out empty strings
            if( strlen( features_list[k] ) <= 0 )
                continue;

            REMMINA_PLUGIN_INFO( "%s", g_strdup_printf( _( "Available feature[%i]: '%s'" ), k + 1, features_list[k] ) );
...
static int rmplugin_x2go_on_plug_removed( GtkSocket *socket, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    REMMINA_PLUGIN_DEBUG( "Function entry." );
    rmplugin_x2go_close_connection( gp );
...
static void rmplugin_x2go_init( RemminaProtocolWidget *gp )
{
    ...
    g_signal_connect( G_OBJECT( gpdata->socket ), "plug-added", G_CALLBACK( rmplugin_x2go_on_plug_added ), gp );
    g_signal_connect( G_OBJECT( gpdata->socket ), "plug-removed", G_CALLBACK( rmplugin_x2go_on_plug_removed ), gp );
    gtk_container_add( GTK_CONTAINER( gp ), gpdata->socket );
}

static int rmplugin_x2go_try_window_id( Window window_id )
{
    TRACE_CALL( __func__ );
    gint i;
    bool already_seen = FALSE;
...
static int rmplugin_x2go_start_create_notify( RemminaProtocolWidget *gp, char *errmsg )
{
    TRACE_CALL( __func__ );
    RemminaPluginX2GoData *gpdata = GET_PLUGIN_DATA( gp );
...
static int rmplugin_x2go_monitor_create_notify( RemminaProtocolWidget *gp, const char *cmd, char *errmsg )
{
    TRACE_CALL( __func__ );
    RemminaPluginX2GoData *gpdata;

    bool agent_window_found = FALSE;
    Atom atom;
    XEvent xev;
    Window w;
    Atom type;
    ...
    while( wait_amount > 0 )
    {
        ...
        if( XGetWindowProperty(
                gpdata->display, w, atom, 0, 255, False, AnyPropertyType, &type, &format, &nitems, &rest, &data )
            != Success )
        {
        ...
        if( data && g_strrstr( (char *)data, cmd ) && rmplugin_x2go_try_window_id( w ) )
        {
            gpdata->window_id = w;
    ...
    {
        g_strlcpy( errmsg,
                   _( "No X2Go session window appeared. "
...
static int rmplugin_x2go_start_session( RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    REMMINA_PLUGIN_DEBUG( "Function entry." );
    ...
    ;
    RemminaFile *remminafile;
    const char errmsg[512] = { 0 };
    bool ret = TRUE;

    char *servstr, *host, *username, *password, *command, *kbdlayout, *kbdtype, *audio, *clipboard, *res;
    gint sshport, dpi;
    ...
    const char *default_dsp_name = gdk_display_get_name( default_dsp );
    ...
    if( ret )
        ret = rmplugin_x2go_start_create_notify( gp, (char *)&errmsg );

    /* trigger the session start, session window should appear soon after this */
    if( ret )
        ret = rmplugin_x2go_exec_x2go( host,
                                       sshport,
                                       username,
                                       password,
                                       command,
    ...
    if( ret )
        ret = rmplugin_x2go_monitor_create_notify( gp, "x2goagent", (char *)&errmsg );
...
static int rmplugin_x2go_main( RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    RemminaPluginX2GoData *gpdata = GET_PLUGIN_DATA( gp );
    bool ret = FALSE;
...
static gpointer rmplugin_x2go_main_thread( RemminaProtocolWidget *gp )
{
    ...
    {
        REMMINA_PLUGIN_CRITICAL(
            "%s", g_strdup_printf( _( "Internal error: %s" ), _( "RemminaProtocolWidget* gp is 'NULL'!" ) ) );
        return NULL;
    }
...
static int rmplugin_x2go_open_connection( RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    RemminaPluginX2GoData *gpdata = GET_PLUGIN_DATA( gp );
    ...
    if( !rm_plugin_service->gtksocket_available() )
    {
        rm_plugin_service->protocol_plugin_set_error( gp,
                                                      _( "The %s protocol is "
...
static int rmplugin_x2go_query_feature( RemminaProtocolWidget *gp, const RemminaProtocolFeature *feature )
{
    TRACE_CALL( __func__ );
    return TRUE;
}
 ...
 */
static char *rmplugin_x2go_enumeration_prettifier( const guint max_elements,
 ...
 *
 */
static GError *rmplugin_x2go_string_setting_validator( char *key, char *value, char *data )
{
    GError *error = NULL;

    if( !data )
    {
        char *error_msg = _( "Invalid validation data in ProtocolSettings array!" );
        REMMINA_PLUGIN_CRITICAL( "%s", error_msg );
        g_set_error( &error, 1, 1, "%s", error_msg );
        return error;
    }

    char **elements_list = g_strsplit( data, ",", 0 );

    guint elements_amount = 0;
    elements_amount = g_strv_length( elements_list );

    if( elements_list == NULL || elements_list[0] == NULL || strlen( elements_list[0] ) <= 0 )
    {
        char *error_msg = _( "Validation data in ProtocolSettings array is invalid!" );
        REMMINA_PLUGIN_CRITICAL( "%s", error_msg );
        g_set_error( &error, 1, 1, "%s", error_msg );
        return error;
    }
    ...
    for( guint i = 0; elements_list[i] != NULL; i++ )
    {
        ...
        char *element = elements_list[i] ? elements_list[i] : "";
        ...
        data_str = rmplugin_x2go_enumeration_prettifier( elements_amount, i, element, data_str );
    }
 ...
 *
 */
static GError *rmplugin_x2go_int_setting_validator( char *key, gpointer value, char *data )
{
    GError *error = NULL;

    char **integer_list = g_strsplit( data, ";", 0 );

    if( integer_list == NULL || integer_list[0] == NULL || integer_list[1] == NULL || strlen( integer_list[0] ) <= 0
    ...
    if( err == STR2INT_INCONVERTIBLE )
    {
        g_set_error( &error,
                     ...
                     1,
                     "%s",
                     g_strdup_printf( _( "Internal error: %s" ), _( "The lower limit is not a valid integer!" ) ) );
    }
    else if( err == STR2INT_OVERFLOW )
    {
        g_set_error(
            &error, 1, 1, "%s", g_strdup_printf( _( "Internal error: %s" ), _( "The lower limit is too high!" ) ) );
    }
    else if( err == STR2INT_UNDERFLOW )
    {
        g_set_error(
            &error, 1, 1, "%s", g_strdup_printf( _( "Internal error: %s" ), _( "The lower limit is too low!" ) ) );
    }
    else if( err == STR2INT_INVALID_DATA )
    {
        g_set_error(
            &error, 1, 1, "%s", g_strdup_printf( _( "Internal error: %s" ), _( "Something unknown went wrong." ) ) );
    }

    if( error )
    {
        REMMINA_PLUGIN_CRITICAL( "%s",
                                 _( "Please check the RemminaProtocolSetting "
    ...
    if( err == STR2INT_INCONVERTIBLE )
    {
        g_set_error( &error,
                     ...
                     1,
                     "%s",
                     g_strdup_printf( _( "Internal error: %s" ), _( "The upper limit is not a valid integer!" ) ) );
    }
    else if( err == STR2INT_OVERFLOW )
    {
        g_set_error(
            &error, 1, 1, "%s", g_strdup_printf( _( "Internal error: %s" ), _( "The upper limit is too high!" ) ) );
    }
    else if( err == STR2INT_UNDERFLOW )
    {
        g_set_error(
            &error, 1, 1, "%s", g_strdup_printf( _( "Internal error: %s" ), _( "The upper limit is too low!" ) ) );
    }
    else if( err == STR2INT_INVALID_DATA )
    {
        g_set_error(
    ...
    {
        g_set_error( &error, 1, 1, _( "Input must be a number between %i and %i." ), minimum, maximum );
    }
    else if( err == STR2INT_INVALID_DATA )
    {
    ...
    /*REMMINA_PLUGIN_DEBUG("Key:  \t%s", (char*) key);
	REMMINA_PLUGIN_DEBUG("Value:\t%s", (char*) value);
