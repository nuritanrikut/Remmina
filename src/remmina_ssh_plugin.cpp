 ...
 **/
const GdkRGBA linux_palette[PALETTE_SIZE] = { { 0, 0, 0, 1 },
                                              { 0.666667, 0, 0, 1 },
                                              { 0, 0.666667, 0, 1 },
...
const GdkRGBA tango_palette[PALETTE_SIZE] = { { 0, 0, 0, 1 },
                                              { 0.8, 0, 0, 1 },
...
const GdkRGBA solarized_dark_palette[PALETTE_SIZE] = { { 0.027451, 0.211765, 0.258824, 1 },
...
const GdkRGBA solarized_light_palette[PALETTE_SIZE] = { { 0.933333, 0.909804, 0.835294, 1 },
                                                        { 0.862745, 0.196078, 0.184314, 1 },
                                                        { 0.521569, 0.600000, 0.000000, 1 },
                                                        { 0.709804, 0.537255, 0.000000, 1 },
                                                        { 0.149020, 0.545098, 0.823529, 1 },
...
const GdkRGBA xterm_palette[PALETTE_SIZE] = { { 0, 0, 0, 1 },
                                              { 0.803922, 0, 0, 1 },
                                              { 0, 0.803922, 0, 1 },
...
static int remmina_plugin_ssh_on_size_allocate( GtkWidget *widget, GtkAllocation *alloc, RemminaProtocolWidget *gp );

static int valid_color( GdkRGBA const *color )
{
    return color->red >= 0. && color->red <= 1. && color->green >= 0. && color->green <= 1. && color->blue >= 0.
           && color->blue <= 1. && color->alpha >= 0. && color->alpha <= 1.;
}
...
static gpointer remmina_plugin_ssh_main_thread( gpointer data )
{
    ...
    if( ssh )
    {
        ...
        REMMINA_DEBUG( "tunnel_entrance_host: %s, tunnel_entrance_port: %d",
        ...
        if( remmina_ssh_init_session( REMMINA_SSH( shell ) )
            ...
            && remmina_ssh_shell_open(
                shell, (RemminaSSHExitFunc)remmina_plugin_service->protocol_plugin_signal_connection_closed, gp ) )
            cont = TRUE;
    }
    else
    {
        ...
        while( 1 )
        {
            ...
            switch( ret )
            {
                case REMMINA_SSH_AUTH_SUCCESS:
                    ...
                    if( !remmina_ssh_shell_open(
                            shell,
                            (RemminaSSHExitFunc)remmina_plugin_service->protocol_plugin_signal_connection_closed,
                            gp ) )
                    {
                        remmina_plugin_service->protocol_plugin_set_error( gp, "%s", ssh->error );
                        break;
...
BREAK:
    ...
    char *charset = REMMINA_SSH( shell )->charset;

    remmina_plugin_ssh_vte_terminal_set_encoding_and_pty(
        VTE_TERMINAL( gpdata->vte ), charset, shell->master, shell->slave );
...
static int remmina_plugin_ssh_on_focus_in( GtkWidget *widget, GdkEventFocus *event, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
...
{
    TRACE_CALL( __func__ );
    vte_terminal_set_font_scale( VTE_TERMINAL( vte ),
                                 vte_terminal_get_font_scale( VTE_TERMINAL( vte ) ) - SCALE_FACTOR );
}
...
void remmina_plugin_ssh_vte_save_session( GtkMenuItem *menuitem, RemminaProtocolWidget *gp )
{
    ...
    if( err != NULL )
    {
        ...
        widget = gtk_message_dialog_new(
            NULL, GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, _( "Error: %s" ), err->message );
...
#    if VTE_CHECK_VERSION( 0, 38, 0 )
        vte_terminal_write_contents_sync(
            VTE_TERMINAL( gpdata->vte ), G_OUTPUT_STREAM( stream ), VTE_WRITE_DEFAULT, NULL, &err );
#    else
        vte_terminal_write_contents(
            VTE_TERMINAL( gpdata->vte ), G_OUTPUT_STREAM( stream ), VTE_TERMINAL_WRITE_DEFAULT, NULL, &err );
#    endif

    if( err == NULL )
    {
        remmina_public_send_notification(
            "remmina-terminal-saved", _( "Terminal content saved in" ), g_file_get_path( gpdata->vte_session_file ) );
    }
...
static void remmina_ssh_keystroke( RemminaProtocolWidget *gp, const guint keystrokes[], const gint keylen )
{
    ...
    remmina_plugin_service->protocol_plugin_send_keys_signals(
        gpdata->vte, keystrokes, keylen, static_cast<GdkEventType>( GDK_KEY_PRESS | GDK_KEY_RELEASE ) );
    return;
}
...
static void jit_regex( VteRegex *regex, char const *pattern )
{
    ...
    if( !vte_regex_jit( regex, PCRE2_JIT_COMPLETE, &error ) || !vte_regex_jit( regex, PCRE2_JIT_PARTIAL_SOFT, &error ) )
...
static VteRegex *compile_regex_for_search( char const *pattern, bool caseless, GError **error )
{
    TRACE_CALL( __func__ );
...
{
    TRACE_CALL( __func__ );
    bool can_search = search_widget->has_regex;
...
static void remmina_search_widget_update_regex( RemminaPluginSshData *gpdata )
{
    ...
    bool caseless = gtk_toggle_button_get_active( search_widget->match_case_checkbutton ) == FALSE;
    ...
    if( caseless == search_widget->regex_caseless && g_strcmp0( pattern, search_widget->regex_pattern ) == 0 )
        return;
...
GtkWidget *remmina_plugin_pop_search_new( GtkWidget *relative_to, RemminaProtocolWidget *gp )
{
    ...
    search_widget->builder =
        remmina_public_gtk_builder_new_from_resource( "/org/remmina/Remmina/src/../data/ui/remmina_search.glade" );
    ...
    g_signal_connect_swapped(
        search_widget->close_button, "clicked", G_CALLBACK( gtk_widget_destroy ), GTK_WIDGET( search_widget->window ) );

    g_object_bind_property( search_widget->reveal_button,
                            "active",
                            search_widget->revealer,
                            "reveal-child",
                            static_cast<GBindingFlags>( G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE ) );

    g_signal_connect_swapped(
        search_widget->search_entry, "next-match", G_CALLBACK( remmina_search_widget_search_forward ), gpdata );
    g_signal_connect_swapped(
        search_widget->search_entry, "previous-match", G_CALLBACK( remmina_search_widget_search_backward ), gpdata );
    g_signal_connect_swapped(
        search_widget->search_entry, "search-changed", G_CALLBACK( remmina_search_widget_update_regex ), gpdata );

    g_signal_connect_swapped(
        search_widget->search_next_button, "clicked", G_CALLBACK( remmina_search_widget_search_forward ), gpdata );
    g_signal_connect_swapped(
        search_widget->search_prev_button, "clicked", G_CALLBACK( remmina_search_widget_search_backward ), gpdata );

    g_signal_connect_swapped(
        search_widget->match_case_checkbutton, "toggled", G_CALLBACK( remmina_search_widget_update_regex ), gpdata );
    g_signal_connect_swapped(
        search_widget->entire_word_checkbutton, "toggled", G_CALLBACK( remmina_search_widget_update_regex ), gpdata );
    g_signal_connect_swapped(
        search_widget->regex_checkbutton, "toggled", G_CALLBACK( remmina_search_widget_update_regex ), gpdata );
    g_signal_connect_swapped(
        search_widget->match_case_checkbutton, "toggled", G_CALLBACK( remmina_search_widget_update_regex ), gpdata );

    g_signal_connect( search_widget->wrap_around_checkbutton,
                      "toggled",
                      G_CALLBACK( remmina_search_widget_wrap_around_toggled ),
                      gpdata );
...
void remmina_plugin_ssh_call_sftp( GtkMenuItem *menuitem, RemminaProtocolWidget *gp )
{
    ...
    remmina_protocol_widget_call_feature_by_type(
        gp, REMMINA_PROTOCOL_FEATURE_TYPE_TOOL, REMMINA_PROTOCOL_FEATURE_TOOL_SFTP );
}
...
void remmina_plugin_ssh_popup_ui( RemminaProtocolWidget *gp )
{
    ...
    g_signal_connect(
        G_OBJECT( gpdata->vte ), "button_press_event", G_CALLBACK( remmina_ssh_plugin_popup_menu ), menu );

    g_signal_connect(
        G_OBJECT( select_all ), "activate", G_CALLBACK( remmina_plugin_ssh_vte_select_all ), gpdata->vte );
    g_signal_connect( G_OBJECT( copy ), "activate", G_CALLBACK( remmina_plugin_ssh_vte_copy_clipboard ), gpdata->vte );
    g_signal_connect(
        G_OBJECT( paste ), "activate", G_CALLBACK( remmina_plugin_ssh_vte_paste_clipboard ), gpdata->vte );
    g_signal_connect( G_OBJECT( save ), "activate", G_CALLBACK( remmina_plugin_ssh_vte_save_session ), gp );
    g_signal_connect(
        G_OBJECT( font_incr ), "activate", G_CALLBACK( remmina_plugin_ssh_vte_increase_font ), gpdata->vte );
    g_signal_connect(
        G_OBJECT( font_decr ), "activate", G_CALLBACK( remmina_plugin_ssh_vte_decrease_font ), gpdata->vte );
    g_signal_connect( G_OBJECT( find_text ), "activate", G_CALLBACK( remmina_plugin_pop_search ), gp );
    g_signal_connect( G_OBJECT( sftp ), "activate", G_CALLBACK( remmina_plugin_ssh_call_sftp ), gp );

    gtk_widget_show_all( menu );
}

static int remmina_plugin_ssh_close_connection( RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    RemminaPluginSshData *gpdata = GET_PLUGIN_DATA( gp );
...
#    if VTE_CHECK_VERSION( 0, 38, 0 )
	 ...
	 */
    const char *color_name = remmina_plugin_service->file_get_string( remminafile, "ssh_color_scheme" );

    char *remmina_dir = g_build_path( "/", g_get_user_config_dir(), "remmina", "theme", NULL );
    char *remmina_colors_file = g_strdup_printf( "%s/%s.colors", remmina_dir, color_name );
    g_free( remmina_dir );
 ...
 */
static int remmina_plugin_ssh_open_connection( RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    RemminaPluginSshData *gpdata = GET_PLUGIN_DATA( gp );
    ...
    if( pthread_create( &gpdata->thread, NULL, remmina_plugin_ssh_main_thread, gp ) )
    {
        remmina_plugin_service->protocol_plugin_set_error(
            gp, "Failed to initialize pthread. Falling back to non-thread mode…" );
        gpdata->thread = 0;
        return FALSE;
    }
    else
 ...
 * @return Always TRUE
 */
static int remmina_plugin_ssh_query_feature( RemminaProtocolWidget *gp, const RemminaProtocolFeature *feature )
{
    TRACE_CALL( __func__ );
    return TRUE;
}
...
/** Charset list */
static const char *ssh_charset_list[] = { "",  "",
                                          "",  "ASCII",
                                          "",  "BIG5",
                                          "",  "CP437",
                                          "",  "CP720",
...
static const char *ssh_terminal_palette[] = { "0",
                                              "Linux",
                                              "1",
                                              "Tango",
                                              "2",
...
static RemminaProtocolFeature remmina_plugin_ssh_features[] = {
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      REMMINA_PLUGIN_SSH_FEATURE_TOOL_COPY,
      static_cast<void *>( const_cast<char *>( N_( "Copy" ) ) ),
      static_cast<void *>( const_cast<char *>( N_( "_Copy" ) ) ),
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      REMMINA_PLUGIN_SSH_FEATURE_TOOL_PASTE,
      static_cast<void *>( const_cast<char *>( N_( "Paste" ) ) ),
      static_cast<void *>( const_cast<char *>( N_( "_Paste" ) ) ),
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      REMMINA_PLUGIN_SSH_FEATURE_TOOL_SELECT_ALL,
      static_cast<void *>( const_cast<char *>( N_( "Select all" ) ) ),
      static_cast<void *>( const_cast<char *>( N_( "_Select all" ) ) ),
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      REMMINA_PLUGIN_SSH_FEATURE_TOOL_INCREASE_FONT,
      static_cast<void *>( const_cast<char *>( N_( "Increase font size" ) ) ),
      static_cast<void *>( const_cast<char *>( N_( "_Increase font size" ) ) ),
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      REMMINA_PLUGIN_SSH_FEATURE_TOOL_DECREASE_FONT,
      static_cast<void *>( const_cast<char *>( N_( "Decrease font size" ) ) ),
      static_cast<void *>( const_cast<char *>( N_( "_Decrease font size" ) ) ),
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      REMMINA_PLUGIN_SSH_FEATURE_TOOL_SEARCH,
      static_cast<void *>( const_cast<char *>( N_( "Find text" ) ) ),
      static_cast<void *>( const_cast<char *>( N_( "_Find text" ) ) ),
      NULL },
    { REMMINA_PROTOCOL_FEATURE_TYPE_TOOL,
      REMMINA_PROTOCOL_FEATURE_TOOL_SFTP,
      static_cast<void *>( const_cast<char *>( N_( "Open SFTP transfer…" ) ) ),
      static_cast<void *>( const_cast<char *>( "folder-remote" ) ),
      NULL },
...
static const RemminaProtocolSetting remmina_ssh_basic_settings[] = {
    { REMMINA_PROTOCOL_SETTING_TYPE_SERVER,
      ...
      NULL,
      FALSE,
      static_cast<void *>( const_cast<char *>( "_ssh._tcp" ) ),
      NULL,
      NULL,
      NULL },
...
static char log_tips[] = N_( "The filename can use the following placeholders:\n\n"
...
static const RemminaProtocolSetting remmina_ssh_advanced_settings[] = {
    ...
    { REMMINA_PROTOCOL_SETTING_TYPE_TEXT,
      "ssh_kex_algorithms",
      N_( "KEX (Key Exchange) algorithms" ),
      FALSE,
      NULL,
      NULL },
    ...
    { REMMINA_PROTOCOL_SETTING_TYPE_TEXT,
      "ssh_hostkeytypes",
      N_( "Preferred server host key types" ),
      FALSE,
      NULL,
      NULL },
    ...
    { REMMINA_PROTOCOL_SETTING_TYPE_CHECK,
      "sshlogenabled",
      N_( "Log SSH session when exiting Remmina" ),
      FALSE,
      NULL,
      NULL },
    { REMMINA_PROTOCOL_SETTING_TYPE_CHECK,
      "sshsavesession",
      N_( "Log SSH session asynchronously" ),
      FALSE,
      NULL,
      static_cast<void *>(
          const_cast<char *>( N_( "Saving the session asynchronously may have a notable performance impact" ) ) ) },
    ...
    { REMMINA_PROTOCOL_SETTING_TYPE_CHECK,
      ...
      N_( "Don't remember passwords" ),
      TRUE,
      NULL,
      NULL },
    { REMMINA_PROTOCOL_SETTING_TYPE_CHECK,
      "ssh_stricthostkeycheck",
      N_( "Strict host key checking" ),
      TRUE,
      NULL,
      NULL },
...
void remmina_ssh_plugin_load_terminal_palettes( gpointer *ssh_terminal_palette_new )
{
    ...
    for( i = 0; dirs[i] != NULL; ++i )
    {
        ...
        char *remmina_dir = g_build_path( "/", dirs[i], "remmina", "theme", NULL );
                ...
                while( ( filename = g_dir_read_name( system_data_dir ) ) )
                {
                    if( !g_file_test( filename, G_FILE_TEST_IS_DIR ) )
                    {
                        if( g_str_has_suffix( filename, ".colors" ) )
                        {
                            ...
                            char *menu_str = g_strndup( filename, len );
        ...
        if( user_data_dir )
        {
            ...
            while( ( filename = g_dir_read_name( user_data_dir ) ) )
            {
                if( !g_file_test( filename, G_FILE_TEST_IS_DIR ) )
                {
                    if( g_str_has_suffix( filename, ".colors" ) )
                    {
                        char *menu_str = static_cast<char *>( g_malloc( strlen( filename ) + 1 ) );
    ...
    gpointer *color_palette =
        static_cast<gpointer *>( g_malloc( ( preset_rec_size + rec_size ) * sizeof( gpointer ) ) );
    ...
    *ssh_terminal_palette_new = color_palette;
    ...
    {
        color_palette[field_idx] = static_cast<void *>( const_cast<char *>( ssh_terminal_palette[field_idx] ) );
    ...
    {
        char *menu_str = (char *)l_files->data;
...
void remmina_ssh_plugin_register()
{
    ...
    settings = static_cast<RemminaProtocolSettingOpt *>(
        g_memdup( remmina_ssh_advanced_settings, sizeof( remmina_ssh_advanced_settings ) ) );
