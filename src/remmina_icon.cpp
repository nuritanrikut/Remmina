...
static void remmina_icon_create_autostart_file()
{
    ...
    g_key_file_set_string(
        gkeyfile, "Desktop Entry", "Comment", _( "Connect to remote desktops through the applet menu" ) );
...
void remmina_icon_init()
{
    ...
    {
        //TRANSLATORS: %s is a placeholder for "StatusNotifier/Appindicator suppor in “DESKTOP NAME”: "
        REMMINA_INFO( _( "%s not supported natively by your Desktop Environment. libappindicator will try to fallback "
                         "to GtkStatusIcon/xembed" ),
                      msg );
    }
    ...
    {
        remmina_icon.icon =
            app_indicator_new( "remmina-icon", remmina_panel, APP_INDICATOR_CATEGORY_APPLICATION_STATUS );
    ...
    else if( remmina_icon.icon )
    {
        app_indicator_set_status( remmina_icon.icon,
                                  remmina_pref.disable_tray_icon ? APP_INDICATOR_STATUS_PASSIVE
...
#    endif
    ...
    {
        remmina_icon.autostart_file =
            g_strdup_printf( "%s/.config/autostart/remmina-applet.desktop", g_get_home_dir() );
    ...
    g_signal_connect(
        G_OBJECT( remmina_icon.icon ), "connection-changed", G_CALLBACK( remmina_icon_connection_changed_cb ), NULL );
