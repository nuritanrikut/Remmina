...
static GtkTargetEntry remmina_drop_types[] = { { const_cast<char *>( "text/uri-list" ), 0, 1 } };

static const char *quick_connect_plugin_list[] = { "RDP", "VNC", "SSH", "NX", "SPICE", "X2GO" };
...
static void remmina_main_save_expanded_group( void )
{
    ...
    if( GTK_IS_TREE_STORE( remminamain->priv->file_model ) )
    {
        ...
        gtk_tree_view_map_expanded_rows(
            remminamain->tree_files_list, (GtkTreeViewMappingFunc)remmina_main_save_expanded_group_func, NULL );
    }
}
 ...
 */
static int remmina_main_dexit( gpointer data )
{
    TRACE_CALL( __func__ );
...
int remmina_main_idle_destroy( gpointer data )
{
    TRACE_CALL( __func__ );
...
static void remmina_main_show_snap_welcome()
{
    ...
    bool need_snap_interface_connections = FALSE;
    GtkWidget *dsa;
    ...
    {
        dlgbuilder = remmina_public_gtk_builder_new_from_resource(
            "/org/remmina/Remmina/src/../data/ui/remmina_snap_info_dialog.glade" );
                                             ...
                                             gpointer user_data )
{
    ...
    gtk_tree_model_get( model,
                        &iter,
                        NAME_COLUMN,
                        &remminamain->priv->selected_name,
                        FILENAME_COLUMN,
                        &remminamain->priv->selected_filename,
                        -1 );
    ...
    {
        g_snprintf(
            buf, sizeof( buf ), "%s (%s)", remminamain->priv->selected_name, remminamain->priv->selected_filename );
...
static void remmina_main_load_file_list_callback( RemminaFile *remminafile, gpointer user_data )
{
    ...
    gtk_list_store_set( store,
                        &iter,
                        PROTOCOL_COLUMN,
                        remmina_file_get_icon_name( remminafile ),
                        NAME_COLUMN,
                        ...
                        datetime,
                        FILENAME_COLUMN,
                        remmina_file_get_filename( remminafile ),
                        -1 );
    g_free( datetime );
}

static int remmina_main_load_file_tree_traverse( GNode *node, GtkTreeStore *store, GtkTreeIter *parent )
{
    TRACE_CALL( __func__ );
    GtkTreeIter *iter;
...
static int remmina_main_load_file_tree_find( GtkTreeModel *tree, GtkTreeIter *iter, const char *match_group )
{
    TRACE_CALL( __func__ );
    bool ret, match;
    char *group, *filename;
...
static void remmina_main_load_file_tree_callback( RemminaFile *remminafile, gpointer user_data )
{
    ...
    if( gtk_tree_model_get_iter_first( GTK_TREE_MODEL( store ), &iter ) )
        found = remmina_main_load_file_tree_find(
            GTK_TREE_MODEL( store ), &iter, remmina_file_get_string( remminafile, "group" ) );
...
static int remmina_main_filter_visible_func( GtkTreeModel *model, GtkTreeIter *iter, gpointer user_data )
{
    TRACE_CALL( __func__ );
    char *text;
    char *protocol, *name, *group, *server, *plugin, *date, *s;
    bool result = TRUE;
    ...
    if( text && text[0] )
    {
        ...
        if( g_strcmp0( protocol, "folder-symbolic" ) != 0 )
        {
            ...
            g_free( date );
            date = s;
            result = ( strstr( name, text ) || strstr( group, text ) || strstr( server, text ) || strstr( plugin, text )
                       || strstr( date, text ) );
        }
        g_free( protocol );
        g_free( name );
...
static void remmina_main_select_file( const char *filename )
{
    TRACE_CALL( __func__ );
    GtkTreeIter iter;
    GtkTreePath *path;
    ...
    while( TRUE )
    {
        ...
        if( cmp == 0 )
        {
            gtk_tree_selection_select_iter( gtk_tree_view_get_selection( remminamain->tree_files_list ), &iter );
...
static void remmina_main_load_files()
{
    ...
    switch( view_file_mode )
    {
        case REMMINA_VIEW_FILE_TREE:
            ...
            /* Load files list */
            items_count =
                remmina_file_manager_iterate( (GFunc)remmina_main_load_file_tree_callback, (gpointer)newmodel );
            break;
        ...
        default:
            ...
            /* Load files list */
            items_count =
                remmina_file_manager_iterate( (GFunc)remmina_main_load_file_list_callback, (gpointer)newmodel );
            break;
    }
    ...
    gtk_tree_model_filter_set_visible_func( GTK_TREE_MODEL_FILTER( remminamain->priv->file_model_filter ),
                                            (GtkTreeModelFilterVisibleFunc)remmina_main_filter_visible_func,
                                            NULL,
                                            NULL );
    ...
    gtk_tree_sortable_set_sort_column_id( GTK_TREE_SORTABLE( remminamain->priv->file_model_sort ),
                                          ...
                                          static_cast<GtkSortType>( remmina_pref.main_sort_order ) );
    gtk_tree_view_set_model( remminamain->tree_files_list, remminamain->priv->file_model_sort );
    g_signal_connect( G_OBJECT( remminamain->priv->file_model_sort ),
                      "sort-column-changed",
...
static int remmina_main_search_key_event( GtkWidget *search_entry, GdkEventKey *event, gpointer user_data )
{
    TRACE_CALL( __func__ );
...
static int
remmina_main_tree_row_activated( GtkTreeView *tree, GtkTreePath *path, GtkTreeViewColumn *column, gpointer user_data )
{
    TRACE_CALL( __func__ );
...
void remmina_main_on_action_connection_delete( GSimpleAction *action, GVariant *param, gpointer data )
{
    ...
    dialog = gtk_message_dialog_new( remminamain->window,
                                     GTK_DIALOG_MODAL,
...
static void remmina_main_import_file_list( GSList *files )
{
    ...
    if( err->len > 0 )
    {
        ...
        dlg = gtk_message_dialog_new( remminamain->window,
                                      GTK_DIALOG_MODAL,
                                      GTK_MESSAGE_ERROR,
                                      GTK_BUTTONS_OK,
                                      _( "Unable to import:\n%s" ),
...
void remmina_main_on_action_tools_import( GSimpleAction *action, GVariant *param, gpointer data )
{
    ...
    dialog = gtk_file_chooser_dialog_new(
        _( "Import" ), remminamain->window, GTK_FILE_CHOOSER_ACTION_OPEN, "Import", GTK_RESPONSE_ACCEPT, NULL );
    gtk_file_chooser_set_select_multiple( GTK_FILE_CHOOSER( dialog ), TRUE );
    g_signal_connect(
        G_OBJECT( dialog ), "response", G_CALLBACK( remmina_main_action_tools_import_on_response ), NULL );
...
void remmina_main_on_action_tools_export( GSimpleAction *action, GVariant *param, gpointer data )
{
    ...
    if( plugin )
    {
        dialog = gtk_file_chooser_dialog_new( plugin->export_hints,
                                              remminamain->window,
...
static int remmina_main_quickconnect( void )
{
    ...
    char **strings = g_strsplit( server_trimmed, ":", 2 );
...
extern "C"
{
    int remmina_main_quickconnect_on_click( GtkWidget *widget, gpointer user_data )
    {
        TRACE_CALL( __func__ );
...
void remmina_main_on_action_search_toggle( GSimpleAction *action, GVariant *param, gpointer data )
{
    ...
    bool toggle_status = gtk_toggle_button_get_active( remminamain->search_toggle );
...
extern "C"
{
    ...
    int remmina_main_file_list_on_button_press( GtkWidget *widget, GdkEventButton *event, gpointer user_data )
    {
        TRACE_CALL( __func__ );
    ...
    int remmina_main_file_list_on_key_press( GtkWidget *widget, GdkEventKey *event, gpointer user_data )
    {
        TRACE_CALL( __func__ );
        if( event->keyval == GDK_KEY_Menu )
        {
#if GTK_CHECK_VERSION( 3, 22, 0 )
            gtk_menu_popup_at_widget( GTK_MENU( remminamain->menu_popup ),
                                      widget,
                                      GDK_GRAVITY_CENTER,
                                      GDK_GRAVITY_CENTER,
                                      (GdkEvent *)event );
...
extern "C"
{
                                             ...
                                             gpointer user_data )
    {
        ...
        uris = g_uri_list_extract_uris( (const char *)gtk_selection_data_get_data( data ) );
        for( i = 0; uris[i]; i++ )
        {
...
static int remmina_main_add_tool_plugin( char *name, RemminaPlugin *plugin, gpointer user_data )
{
    TRACE_CALL( __func__ );
...
extern "C"
{
    int remmina_main_on_window_state_event( GtkWidget *widget, GdkEventWindowState *event, gpointer user_data )
    {
        TRACE_CALL( __func__ );
        return FALSE;
    }
...
static void remmina_main_init( void )
{
    ...
    {
        const char *name = quick_connect_plugin_list[i];
        if( remmina_plugin_manager_get_plugin( REMMINA_PLUGIN_TYPE_PROTOCOL, name ) )
        {
            ...
            if( remmina_pref.last_quickconnect_protocol != NULL
                && strcmp( name, remmina_pref.last_quickconnect_protocol ) == 0 )
                qcp_actidx = qcp_idx;
            qcp_idx++;
        }
    }
    ...
    gtk_tree_selection_set_select_function(
        gtk_tree_view_get_selection( remminamain->tree_files_list ), remmina_main_selection_func, NULL, NULL );
    /** @todo Set entry_quick_connect_server as default search entry. Weirdly. This does not work yet. */
    gtk_tree_view_set_search_entry( remminamain->tree_files_list,
                                    GTK_ENTRY( remminamain->entry_quick_connect_server ) );
    if( remmina_pref.hide_searchbar )
        gtk_widget_grab_focus( GTK_WIDGET( remminamain->tree_files_list ) );
    /* Load the files list */
    remmina_main_load_files();
    ...
    /* Drag-n-drop support */
    gtk_drag_dest_set(
        GTK_WIDGET( remminamain->window ), GTK_DEST_DEFAULT_ALL, remmina_drop_types, 1, GDK_ACTION_COPY );
    ...
    g_object_set_data(
        G_OBJECT( remminamain->window ), "tag", static_cast<void *>( const_cast<char *>( "remmina-main-window" ) ) );
...
GtkWidget *remmina_main_new( void )
{
    ...
    remminamain->builder =
        remmina_public_gtk_builder_new_from_resource( "/org/remmina/Remmina/src/../data/ui/remmina_main.glade" );
    ...
    /* signals */
    g_signal_connect( remminamain->entry_quick_connect_server,
                      "key-release-event",
                      G_CALLBACK( remmina_main_search_key_event ),
                      NULL );
    g_signal_connect(
        remminamain->tree_files_list, "row-activated", G_CALLBACK( remmina_main_tree_row_activated ), NULL );
    /* Non widget objects */
    actions = g_simple_action_group_new();
    g_action_map_add_action_entries(
        G_ACTION_MAP( actions ), app_actions, G_N_ELEMENTS( app_actions ), remminamain->window );
    gtk_widget_insert_action_group( GTK_WIDGET( remminamain->window ), "app", G_ACTION_GROUP( actions ) );
    g_action_map_add_action_entries(
        G_ACTION_MAP( actions ), main_actions, G_N_ELEMENTS( main_actions ), remminamain->window );
