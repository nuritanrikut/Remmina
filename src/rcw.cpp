...
static guint rcw_signals[LAST_SIGNAL] = { 0 };

static RemminaConnectionWindow *rcw_create_scrolled( gint width, gint height, bool maximize );
static RemminaConnectionWindow *rcw_create_fullscreen( GtkWindow *old, gint view_mode );
static int rcw_hostkey_func( RemminaProtocolWidget *gp, guint keyval, bool release );
...
static const GtkTargetEntry dnd_targets_ftb[] = {
    { (char *)"text/x-remmina-ftb", GTK_TARGET_SAME_APP | GTK_TARGET_OTHER_WIDGET, 0 },
};

static const GtkTargetEntry dnd_targets_tb[] = {
    { (char *)"text/x-remmina-tb", GTK_TARGET_SAME_APP, 0 },
};
...
static void rcw_class_init( RemminaConnectionWindowClass *klass )
{
    ...
    rcw_signals[TOOLBARPLACE_SIGNAL] = g_signal_new( "toolbar-place",
                                                     G_TYPE_FROM_CLASS( klass ),
                                                     static_cast<GSignalFlags>( G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION ),
                                                     G_STRUCT_OFFSET( RemminaConnectionWindowClass, toolbar_place ),
                                                     NULL,
                                                     NULL,
...
static RemminaConnectionObject *rcw_get_cnnobj_at_page( RemminaConnectionWindow *cnnwin, gint npage )
{
    ...
    return static_cast<RemminaConnectionObject *>( g_object_get_data( G_OBJECT( po ), "cnnobj" ) );
}
...
static RemminaScaleMode
get_current_allowed_scale_mode( RemminaConnectionObject *cnnobj, bool *dynres_avail, bool *scale_avail )
{
    TRACE_CALL( __func__ );
    RemminaScaleMode scalemode;
    bool plugin_has_dynres, plugin_can_scale;
...
static int rcw_keyboard_grab_retry( gpointer user_data )
{
    TRACE_CALL( __func__ );
    ...
    if( keyboard != NULL )
    {
        ...
        if( ggs != GDK_GRAB_SUCCESS )
        {
            ...
            if( cnnwin->priv->grab_retry_eventsourceid == 0 )
                cnnwin->priv->grab_retry_eventsourceid =
                    g_timeout_add( 500, (GSourceFunc)rcw_keyboard_grab_retry, cnnwin );
        }
        else
        {
#if DEBUG_KB_GRABBING
    ...
    if( cnnwin->priv->on_delete_confirm_mode != RCW_ONDELETE_NOCONFIRM )
    {
        ...
        if( n > 1 )
        {
            dialog = gtk_message_dialog_new(
                ...
                GTK_BUTTONS_YES_NO,
                _( "Are you sure you want to close %i active connections in the current window?" ),
                n );
...
static int rcw_delete_event( GtkWidget *widget, GdkEvent *event, gpointer data )
{
    TRACE_CALL( __func__ );
...
int rcw_notify_widget_toolbar_placement( GtkWidget *widget, gpointer data )
{
    TRACE_CALL( __func__ );
    GType rcwtype;
...
static int rcw_tb_drag_failed( GtkWidget *widget, GdkDragContext *context, GtkDragResult result, gpointer user_data )
{
    TRACE_CALL( __func__ );
    RemminaConnectionWindowPriv *priv;
    RemminaConnectionWindow *cnnwin;
...
static int
rcw_tb_drag_drop( GtkWidget *widget, GdkDragContext *context, gint x, gint y, guint time, gpointer user_data )
{
    TRACE_CALL( __func__ );
    GtkAllocation wa;
...
void rcw_update_toolbar_opacity( RemminaConnectionWindow *cnnwin )
{
    ...
    priv->floating_toolbar_opacity =
        ( 1.0 - TOOLBAR_OPACITY_MIN ) / ( (gdouble)TOOLBAR_OPACITY_LEVEL )
            * ( (gdouble)( TOOLBAR_OPACITY_LEVEL
                           - remmina_file_get_int( cnnobj->remmina_file, "toolbar_opacity", 0 ) ) )
        + TOOLBAR_OPACITY_MIN;
    if( priv->floating_toolbar_widget )
        gtk_widget_set_opacity( GTK_WIDGET( priv->overlay_ftb_overlay ), priv->floating_toolbar_opacity );
}

static int rcw_floating_toolbar_make_invisible( gpointer data )
{
    TRACE_CALL( __func__ );
...
static void rcw_floating_toolbar_show( RemminaConnectionWindow *cnnwin, bool show )
{
    TRACE_CALL( __func__ );
    RemminaConnectionWindowPriv *priv = cnnwin->priv;
...
int rcw_toolbar_autofit_restore( RemminaConnectionWindow *cnnwin )
{
    TRACE_CALL( __func__ );
    ...
    if( cnnobj->connected && GTK_IS_SCROLLED_WINDOW( cnnobj->scrolled_container ) )
    {
        ...
        if( remmina_pref.toolbar_placement == TOOLBAR_PLACEMENT_LEFT
            ...
            gtk_window_resize( GTK_WINDOW( cnnobj->cnnwin ),
                               MAX( 1, dwidth + ta.width + nba.width - ca.width ),
                               MAX( 1, dheight + nba.height - ca.height ) );
        else
            gtk_window_resize( GTK_WINDOW( cnnobj->cnnwin ),
                               MAX( 1, dwidth + nba.width - ca.width ),
...
static void rcw_toolbar_autofit( GtkToolItem *toggle, RemminaConnectionWindow *cnnwin )
{
    ...
    if( GTK_IS_SCROLLED_WINDOW( cnnobj->scrolled_container ) )
    {
        if( ( gdk_window_get_state( gtk_widget_get_window( GTK_WIDGET( cnnwin ) ) ) & GDK_WINDOW_STATE_MAXIMIZED )
            != 0 )
        ...
        gtk_scrolled_window_set_policy(
            GTK_SCROLLED_WINDOW( cnnobj->scrolled_container ), GTK_POLICY_NEVER, GTK_POLICY_NEVER );
        ...
        case SCROLLED_WINDOW_MODE:
            if( remmina_file_get_int( cnnobj->remmina_file, "viewmode", UNDEFINED_MODE ) == UNDEFINED_MODE )
            {
                ...
                gtk_window_set_default_size( GTK_WINDOW( cnnobj->cnnwin ),
                                             MIN( rd_width, monitor_geometry.width ),
                                             MIN( rd_height, monitor_geometry.height ) );
...
static void rcw_set_tooltip( GtkWidget *item, const char *tip, guint key1, guint key2 )
{
    TRACE_CALL( __func__ );
    char *s1;
    char *s2;
    ...
    {
        if( key2 )
            s1 = g_strdup_printf( " (%s + %s,%s)",
                                  gdk_keyval_name( remmina_pref.hostkey ),
                                  gdk_keyval_name( gdk_keyval_to_upper( key1 ) ),
                                  gdk_keyval_name( gdk_keyval_to_upper( key2 ) ) );
        else if( key1 == remmina_pref.hostkey )
            s1 = g_strdup_printf( " (%s)", gdk_keyval_name( remmina_pref.hostkey ) );
        else
            s1 = g_strdup_printf(
                " (%s + %s)", gdk_keyval_name( remmina_pref.hostkey ), gdk_keyval_name( gdk_keyval_to_upper( key1 ) ) );
    }
    else
    {
        s1 = NULL;
...
static void remmina_protocol_widget_update_alignment( RemminaConnectionObject *cnnobj )
{
    ...
    else
    {
        ...
        if( scalemode == REMMINA_PROTOCOL_WIDGET_SCALE_MODE_SCALED
...
static void rcw_migrate( RemminaConnectionWindow *from, RemminaConnectionWindow *to )
{
    ...
    /* Migrate TAG */
    tag = g_strdup( (char *)g_object_get_data( G_OBJECT( from ), "tag" ) );
    ...
    if( from_notebook && GTK_IS_NOTEBOOK( from_notebook ) )
    {
        ...
        for( i = 0; i < np; i++ )
        {
            ...
            cnnobj = static_cast<RemminaConnectionObject *>( g_object_get_data( G_OBJECT( frompage ), "cnnobj" ) );
...
static void rcw_switch_viewmode( RemminaConnectionWindow *cnnwin, int newmode )
{
    ...
    {
        newwin = rcw_create_scrolled( cnnwin->priv->ss_width, cnnwin->priv->ss_height, cnnwin->priv->ss_maximized );
        rcw_migrate( cnnwin, newwin );
...
void rcw_toolbar_fullscreen_option( GtkToolItem *toggle, RemminaConnectionWindow *cnnwin )
{
    ...
    gtk_menu_popup_at_widget(
        GTK_MENU( menu ), GTK_WIDGET( toggle ), GDK_GRAVITY_SOUTH_WEST, GDK_GRAVITY_NORTH_WEST, NULL );
#else
    gtk_menu_popup( GTK_MENU( menu ),
                    NULL,
                    NULL,
                    remmina_public_popup_position,
                    cnnwin->priv->toolitem_fullscreen,
                    0,
...
static void rcw_toolbar_switch_page( GtkToolItem *toggle, RemminaConnectionWindow *cnnwin )
{
    ...
    g_signal_connect( G_OBJECT( menu ), "deactivate", G_CALLBACK( rcw_toolbar_switch_page_popdown ), cnnwin );
...
#else
    gtk_menu_popup(
        GTK_MENU( menu ), NULL, NULL, remmina_public_popup_position, widget, 0, gtk_get_current_event_time() );
#endif
}
...
static void rco_change_scalemode( RemminaConnectionObject *cnnobj, bool bdyn, bool bscale )
{
    RemminaScaleMode scalemode;
    RemminaConnectionWindowPriv *priv = cnnobj->cnnwin->priv;
    ...
    gtk_widget_set_sensitive( GTK_WIDGET( priv->scaler_option_button ),
                              scalemode == REMMINA_PROTOCOL_WIDGET_SCALE_MODE_SCALED );
    rco_update_toolbar_autofit_button( cnnobj );

    remmina_protocol_widget_call_feature_by_type(
        REMMINA_PROTOCOL_WIDGET( cnnobj->proto ), REMMINA_PROTOCOL_FEATURE_TYPE_SCALE, 0 );
...
static void rcw_toolbar_multi_monitor_mode( GtkToolItem *toggle, RemminaConnectionWindow *cnnwin )
{
    ...
    if( gtk_toggle_tool_button_get_active( GTK_TOGGLE_TOOL_BUTTON( toggle ) ) )
    {
        ...
        remmina_protocol_widget_call_feature_by_type(
            REMMINA_PROTOCOL_WIDGET( cnnobj->proto ), REMMINA_PROTOCOL_FEATURE_TYPE_MULTIMON, 0 );
...
static void rco_call_protocol_feature_radio( GtkMenuItem *menuitem, RemminaConnectionObject *cnnobj )
{
    ...
    if( gtk_check_menu_item_get_active( GTK_CHECK_MENU_ITEM( menuitem ) ) )
    {
        ...
        remmina_file_set_string( cnnobj->remmina_file, (const char *)feature->opt2, (const char *)value );
...
static void rco_call_protocol_feature_check( GtkMenuItem *menuitem, RemminaConnectionObject *cnnobj )
{
    ...
    remmina_file_set_int( cnnobj->remmina_file, (const char *)feature->opt2, value );
    remmina_protocol_widget_call_feature_by_ref( REMMINA_PROTOCOL_WIDGET( cnnobj->proto ), feature );
}
                                    ...
                                    bool enabled )
{
    ...
    group = NULL;
    value = remmina_file_get_string( remminafile, (const char *)feature->opt2 );
    list = (const char **)feature->opt3;
    for( i = 0; list[i]; i += 2 )
    {
        ...
        if( enabled )
        {
            ...
            g_signal_connect( G_OBJECT( menuitem ), "toggled", G_CALLBACK( rco_call_protocol_feature_radio ), cnnobj );
        }
        else
        {
            gtk_widget_set_sensitive( menuitem, FALSE );
                                    ...
                                    bool enabled )
{
    ...
    menuitem = gtk_check_menu_item_new_with_label( g_dgettext( domain, (const char *)feature->opt3 ) );
    gtk_widget_show( menuitem );
    gtk_menu_shell_append( GTK_MENU_SHELL( menu ), menuitem );
    ...
    if( enabled )
    {
        ...
        gtk_check_menu_item_set_active(
            ...
            remmina_file_get_int( cnnobj->remmina_file, (const char *)feature->opt2, FALSE ) );

        g_signal_connect( G_OBJECT( menuitem ), "toggled", G_CALLBACK( rco_call_protocol_feature_check ), cnnobj );
    }
    else
    {
        gtk_widget_set_sensitive( menuitem, FALSE );
...
static void rcw_toolbar_preferences( GtkToolItem *toggle, RemminaConnectionWindow *cnnwin )
{
    ...
    menu = gtk_menu_new();
    for( feature = remmina_protocol_widget_get_features( REMMINA_PROTOCOL_WIDGET( cnnobj->proto ) );
         ...
         feature++ )
    {
        ...
        switch( GPOINTER_TO_INT( feature->opt1 ) )
        {
            case REMMINA_PROTOCOL_FEATURE_PREF_RADIO:
                rcw_toolbar_preferences_radio( cnnobj, cnnobj->remmina_file, menu, feature, domain, enabled );
                separator = TRUE;
                break;
            case REMMINA_PROTOCOL_FEATURE_PREF_CHECK:
                rcw_toolbar_preferences_check( cnnobj, menu, feature, domain, enabled );
                break;
        }
    }
...
static void rcw_toolbar_tools( GtkToolItem *toggle, RemminaConnectionWindow *cnnwin )
{
         ...
         feature++ )
    {
        ...
        if( feature->opt1 )
            menuitem = gtk_menu_item_new_with_label( g_dgettext( domain, (const char *)feature->opt1 ) );
        if( feature->opt3 )
        ...
        if( enabled )
        {
            ...
            g_signal_connect(
                G_OBJECT( menuitem ), "activate", G_CALLBACK( rco_call_protocol_feature_activate ), cnnobj );
        }
        else
        {
            gtk_widget_set_sensitive( menuitem, FALSE );
...
static void rcw_toolbar_screenshot( GtkToolItem *toggle, RemminaConnectionWindow *cnnwin )
{
    ...
    char *denyclip = remmina_pref_get_value( "deny_screenshot_clipboard" );
    ...
    if( remmina_protocol_widget_plugin_screenshot( gp, &rpsd ) )
    {
        ...
        if( denyclip && ( g_strcmp0( denyclip, "true" ) ) )
            gtk_clipboard_set_image( c, gdk_pixbuf_get_from_surface( srcsurface, 0, 0, width, height ) );
    ...
    pngstr = g_string_new( g_strdup_printf( "%s/%s.png", remmina_pref.screenshot_path, remmina_pref.screenshot_name ) );
    remmina_utils_string_replace_all( pngstr, "%p", remmina_file_get_string( cnnobj->remmina_file, "name" ) );
    remmina_utils_string_replace_all( pngstr, "%h", remmina_file_get_string( cnnobj->remmina_file, "server" ) );
    remmina_utils_string_replace_all( pngstr, "%Y", g_strdup_printf( "%d", g_date_time_get_year( date ) ) );
    remmina_utils_string_replace_all( pngstr, "%m", g_strdup_printf( "%02d", g_date_time_get_month( date ) ) );
    remmina_utils_string_replace_all( pngstr, "%d", g_strdup_printf( "%02d", g_date_time_get_day_of_month( date ) ) );
    remmina_utils_string_replace_all( pngstr, "%H", g_strdup_printf( "%02d", g_date_time_get_hour( date ) ) );
    remmina_utils_string_replace_all( pngstr, "%M", g_strdup_printf( "%02d", g_date_time_get_minute( date ) ) );
    remmina_utils_string_replace_all( pngstr, "%S", g_strdup_printf( "%02d", g_date_time_get_second( date ) ) );
...
static GtkWidget *rcw_create_toolbar( RemminaConnectionWindow *cnnwin, gint mode )
{
    ...
    rcw_set_tooltip( GTK_WIDGET( toolitem ),
                     _( "Resize the window to fit in remote resolution" ),
    ...
    rcw_set_tooltip( GTK_WIDGET( toolitem ), _( "Toggle fullscreen mode" ), remmina_pref.shortcutkey_fullscreen, 0 );
    gtk_toolbar_insert( GTK_TOOLBAR( toolbar ), toolitem, -1 );
    gtk_widget_show( GTK_WIDGET( toolitem ) );
    ...
    if( n_monitors > 1 )
    {
        ...
        rcw_set_tooltip( GTK_WIDGET( toolitem ), _( "Multi monitor" ), remmina_pref.shortcutkey_multimon, 0 );
        gtk_toolbar_insert( GTK_TOOLBAR( toolbar ), toolitem, -1 );
        gtk_widget_show( GTK_WIDGET( toolitem ) );
    ...
    rcw_set_tooltip(
        GTK_WIDGET( toolitem ), _( "Toggle dynamic resolution update" ), remmina_pref.shortcutkey_dynres, 0 );
    gtk_toolbar_insert( GTK_TOOLBAR( toolbar ), toolitem, -1 );
    gtk_widget_show( GTK_WIDGET( toolitem ) );
    ...
    rcw_set_tooltip( GTK_WIDGET( toolitem ),
                     _( "Switch tab pages" ),
                     remmina_pref.shortcutkey_prevtab,
    ...
    rcw_set_tooltip( GTK_WIDGET( toolitem ), _( "Grab all keyboard events" ), remmina_pref.shortcutkey_grab, 0 );
    gtk_toolbar_insert( GTK_TOOLBAR( toolbar ), toolitem, -1 );
    gtk_widget_show( GTK_WIDGET( toolitem ) );
...
static void rco_update_toolbar( RemminaConnectionObject *cnnobj )
{
    ...
    RemminaConnectionWindowPriv *priv = cnnobj->cnnwin->priv;
    GtkToolItem *toolitem;
    bool bval, dynres_avail, scale_avail;
    bool test_floating_toolbar;
    RemminaScaleMode scalemode;
    ...
    {
        const char *str = remmina_file_get_string( cnnobj->remmina_file, "name" );
        const char *format;
        GdkRGBA rgba;
        char *bg;
...
static int rcw_floating_toolbar_on_enter( GtkWidget *widget, GdkEventCrossing *event, RemminaConnectionWindow *cnnwin )
{
    TRACE_CALL( __func__ );
...
static int rcw_on_enter_notify_event( GtkWidget *widget, GdkEventCrossing *event, gpointer user_data )
{
    TRACE_CALL( __func__ );
#if DEBUG_KB_GRABBING
...
static int rcw_on_leave_notify_event( GtkWidget *widget, GdkEventCrossing *event, gpointer user_data )
{
    TRACE_CALL( __func__ );
...
static int rco_leave_protocol_widget( GtkWidget *widget, GdkEventCrossing *event, RemminaConnectionObject *cnnobj )
{
    TRACE_CALL( __func__ );
    ...
    switch( event->detail )
    {
        case GDK_NOTIFY_ANCESTOR:
            printf( "GDK_NOTIFY_ANCESTOR" );
            break;
        case GDK_NOTIFY_VIRTUAL:
            printf( "GDK_NOTIFY_VIRTUAL" );
            break;
        case GDK_NOTIFY_NONLINEAR:
            printf( "GDK_NOTIFY_NONLINEAR" );
            break;
        case GDK_NOTIFY_NONLINEAR_VIRTUAL:
            printf( "GDK_NOTIFY_NONLINEAR_VIRTUAL" );
            break;
        case GDK_NOTIFY_UNKNOWN:
            printf( "GDK_NOTIFY_UNKNOWN" );
            break;
        case GDK_NOTIFY_INFERIOR:
            printf( "GDK_NOTIFY_INFERIOR" );
            break;
        default:
            printf( "unknown" );
    }
    ...
    switch( event->mode )
    {
        case GDK_CROSSING_NORMAL:
            printf( "GDK_CROSSING_NORMAL" );
            break;
        case GDK_CROSSING_GRAB:
            printf( "GDK_CROSSING_GRAB" );
            break;
        case GDK_CROSSING_UNGRAB:
            printf( "GDK_CROSSING_UNGRAB" );
            break;
        case GDK_CROSSING_GTK_GRAB:
            printf( "GDK_CROSSING_GTK_GRAB" );
            break;
        case GDK_CROSSING_GTK_UNGRAB:
            printf( "GDK_CROSSING_GTK_UNGRAB" );
            break;
        case GDK_CROSSING_STATE_CHANGED:
            printf( "GDK_CROSSING_STATE_CHANGED" );
            break;
        case GDK_CROSSING_TOUCH_BEGIN:
            printf( "GDK_CROSSING_TOUCH_BEGIN" );
            break;
        case GDK_CROSSING_TOUCH_END:
            printf( "GDK_CROSSING_TOUCH_END" );
            break;
        case GDK_CROSSING_DEVICE_SWITCH:
            printf( "GDK_CROSSING_DEVICE_SWITCH" );
            break;
...
int rco_enter_protocol_widget( GtkWidget *widget, GdkEventCrossing *event, RemminaConnectionObject *cnnobj )
{
    TRACE_CALL( __func__ );
    bool active;
...
static int focus_in_delayed_grab( RemminaConnectionWindow *cnnwin )
{
    TRACE_CALL( __func__ );
...
static void rcw_focus_out( RemminaConnectionWindow *cnnwin )
{
    ...
    if( cnnobj->proto && cnnobj->scrolled_container )
        remmina_protocol_widget_call_feature_by_type(
            REMMINA_PROTOCOL_WIDGET( cnnobj->proto ), REMMINA_PROTOCOL_FEATURE_TYPE_UNFOCUS, 0 );
}

static int rcw_floating_toolbar_hide( RemminaConnectionWindow *cnnwin )
{
    TRACE_CALL( __func__ );
    RemminaConnectionWindowPriv *priv = cnnwin->priv;
...
static int rcw_floating_toolbar_on_scroll( GtkWidget *widget, GdkEventScroll *event, RemminaConnectionWindow *cnnwin )
{
    TRACE_CALL( __func__ );
    RemminaConnectionObject *cnnobj;
...
static int rcw_after_configure_scrolled( gpointer user_data )
{
    TRACE_CALL( __func__ );
    gint width, height;
    GdkWindowState s;
    ...
    {
        RemminaConnectionObject *cnnobj = static_cast<RemminaConnectionObject *>( g_object_get_data(
            G_OBJECT( gtk_notebook_get_nth_page( GTK_NOTEBOOK( cnnwin->priv->notebook ), ipg ) ), "cnnobj" ) );
...
static int rcw_on_configure( GtkWidget *widget, GdkEventConfigure *event, gpointer data )
{
    TRACE_CALL( __func__ );
    RemminaConnectionWindow *cnnwin;
    RemminaConnectionObject *cnnobj;
    ...
    if( gtk_widget_get_window( GTK_WIDGET( cnnwin ) ) && cnnwin->priv->view_mode == SCROLLED_WINDOW_MODE )
...
static void rcw_update_pin( RemminaConnectionWindow *cnnwin )
{
    ...
    if( cnnwin->priv->pin_down )
        gtk_button_set_image(
            GTK_BUTTON( cnnwin->priv->pin_button ),
...
static void rcw_toolbar_place_signal( RemminaConnectionWindow *cnnwin, gpointer data )
{
    ...
    if( priv->toolbar && priv->grid )
    {
        ...
        rcw_place_toolbar( GTK_TOOLBAR( priv->toolbar ),
                           GTK_GRID( priv->grid ),
                           GTK_WIDGET( priv->notebook ),
                           remmina_pref.toolbar_placement );
...
static int rcw_focus_in_event( GtkWidget *widget, GdkEventWindowState *event, gpointer user_data )
{
    TRACE_CALL( __func__ );
#if DEBUG_KB_GRABBING
...
static int rcw_state_event( GtkWidget *widget, GdkEventWindowState *event, gpointer user_data )
{
    TRACE_CALL( __func__ );
...
static int rcw_map_event( GtkWidget *widget, GdkEvent *event, gpointer data )
{
    TRACE_CALL( __func__ );
...
static int rcw_unmap_event( GtkWidget *widget, GdkEvent *event, gpointer data )
{
    TRACE_CALL( __func__ );
...
static int rcw_map_event_fullscreen( GtkWidget *widget, GdkEvent *event, gpointer data )
{
    TRACE_CALL( __func__ );
    RemminaConnectionObject *cnnobj;
    gint target_monitor;
    ...
    if( remmina_pref.fullscreen_on_auto )
    {
        if( target_monitor == FULL_SCREEN_TARGET_MONITOR_UNDEFINED )
            ...
            gtk_window_fullscreen_on_monitor(
                GTK_WINDOW( widget ), gtk_window_get_screen( GTK_WINDOW( widget ) ), target_monitor );
    }
    else
    {
...
static RemminaConnectionWindow *rcw_new( bool fullscreen, int full_screen_target_monitor )
{
    TRACE_CALL( __func__ );
    RemminaConnectionWindow *cnnwin;
    ...
    if( fullscreen )
        ...
        g_signal_connect( G_OBJECT( cnnwin ),
                          "map-event",
                          G_CALLBACK( rcw_map_event_fullscreen ),
                          GINT_TO_POINTER( full_screen_target_monitor ) );
    else
...
void rco_closewin( RemminaProtocolWidget *gp )
{
    ...
    if( cnnobj && cnnobj->cnnwin )
    {
        ...
        if( page_to_remove )
        {
            gtk_notebook_remove_page( cnnobj->cnnwin->priv->notebook,
...
static GtkWidget *rco_create_tab_label( RemminaConnectionObject *cnnobj )
{
    ...
    g_signal_connect(
        G_OBJECT( cnnobj->proto ), "enter-notify-event", G_CALLBACK( rco_enter_protocol_widget ), cnnobj );
    g_signal_connect(
        G_OBJECT( cnnobj->proto ), "leave-notify-event", G_CALLBACK( rco_leave_protocol_widget ), cnnobj );
...
static int rcw_on_switch_page_finalsel( gpointer user_data )
{
    TRACE_CALL( __func__ );
    RemminaConnectionWindowPriv *priv;
    RemminaConnectionObject *cnnobj;
    ...
    if( GTK_IS_WIDGET( cnnobj->cnnwin ) )
    {
        ...
        if( !priv->hidetb_eventsource )
            priv->hidetb_eventsource =
                g_timeout_add( TB_HIDE_TIME_TIME, (GSourceFunc)rcw_floating_toolbar_hide, cnnobj->cnnwin );
...
static void
rcw_on_switch_page( GtkNotebook *notebook, GtkWidget *newpage, guint page_num, RemminaConnectionWindow *cnnwin )
{
    TRACE_CALL( __func__ );
    RemminaConnectionWindowPriv *priv = cnnwin->priv;
...
static void
rcw_on_page_added( GtkNotebook *notebook, GtkWidget *child, guint page_num, RemminaConnectionWindow *cnnwin )
{
...
static void
rcw_on_page_removed( GtkNotebook *notebook, GtkWidget *child, guint page_num, RemminaConnectionWindow *cnnwin )
{
    TRACE_CALL( __func__ );
...
rcw_on_notebook_create_window( GtkNotebook *notebook, GtkWidget *page, gint x, gint y, gpointer data )
{
    ...
    if( !dstcnnwin )
    {
        ...
        srctag = (char *)g_object_get_data( G_OBJECT( srccnnwin ), "tag" );
...
static RemminaConnectionWindow *rcw_create_scrolled( gint width, gint height, bool maximize )
{
    ...
    rcw_place_toolbar(
        GTK_TOOLBAR( toolbar ), GTK_GRID( grid ), GTK_WIDGET( notebook ), remmina_pref.toolbar_placement );

    gtk_container_add( GTK_CONTAINER( cnnwin ), grid );

    /* Add drag capabilities to the toolbar */
    gtk_drag_source_set( GTK_WIDGET( toolbar ),
                         GDK_BUTTON1_MASK,
                         dnd_targets_tb,
...
static void rcw_create_overlay_ftb_overlay( RemminaConnectionWindow *cnnwin )
{
    ...
    g_signal_connect( G_OBJECT( priv->overlay_ftb_overlay ),
                      "enter-notify-event",
                      G_CALLBACK( rcw_floating_toolbar_on_enter ),
                      cnnwin );
    g_signal_connect(
        G_OBJECT( priv->overlay_ftb_overlay ), "scroll-event", G_CALLBACK( rcw_floating_toolbar_on_scroll ), cnnwin );
    gtk_widget_add_events( GTK_WIDGET( priv->overlay_ftb_overlay ),
                           GDK_SCROLL_MASK
#if GTK_CHECK_VERSION( 3, 4, 0 )
    ...
    gtk_drag_source_set( GTK_WIDGET( priv->overlay_ftb_overlay ),
                         GDK_BUTTON1_MASK,
                         dnd_targets_ftb,
                         sizeof dnd_targets_ftb / sizeof *dnd_targets_ftb,
                         GDK_ACTION_MOVE );
    g_signal_connect_after(
        GTK_WIDGET( priv->overlay_ftb_overlay ), "drag-begin", G_CALLBACK( rcw_ftb_drag_begin ), cnnwin );
...
static int rcw_ftb_drag_drop( GtkWidget *widget,
...
#else
        full_screen_target_monitor =
            gdk_screen_get_monitor_at_window( gdk_screen_get_default(), gtk_widget_get_window( GTK_WIDGET( old ) ) );
#endif
    }
...
static int rcw_hostkey_func( RemminaProtocolWidget *gp, guint keyval, bool release )
{
    ...
    {
        rcw_toolbar_minimize( GTK_TOOL_ITEM( gp ), cnnobj->cnnwin );
    }
    else if( keyval == remmina_pref.shortcutkey_viewonly )
    {
        remmina_file_set_int( cnnobj->remmina_file,
                              "viewonly",
                              ( remmina_file_get_int( cnnobj->remmina_file, "viewonly", 0 ) == 0 ) ? 1 : 0 );
    }
    else if( keyval == remmina_pref.shortcutkey_screenshot )
    {
        rcw_toolbar_screenshot( GTK_TOOL_ITEM( gp ), cnnobj->cnnwin );
    }
...
int rcw_delayed_window_present( gpointer user_data )
{
    RemminaConnectionWindow *cnnwin = (RemminaConnectionWindow *)user_data;
...
void rco_on_connect( RemminaProtocolWidget *gp, RemminaConnectionObject *cnnobj )
{
    ...
    cnnobj->cnnwin->priv->dwp_eventsourceid =
        g_timeout_add( 200, rcw_delayed_window_present, (gpointer)cnnobj->cnnwin );
}
...
void rco_on_disconnect( RemminaProtocolWidget *gp, gpointer data )
{
    ...
    rcw_kp_ungrab( cnnobj->cnnwin );
    gtk_toggle_tool_button_set_active( GTK_TOGGLE_TOOL_BUTTON( priv->toolitem_grab ), FALSE );
    ...
    if( remmina_protocol_widget_has_error( gp ) )
    {
        ...
        remmina_message_panel_setup_message(
            mp, remmina_protocol_widget_get_error_message( gp ), cb_lasterror_confirmed, gp );
...
int rcw_open_from_filename( const char *filename )
{
    ...
    else
    {
        dialog = gtk_message_dialog_new( NULL,
                                         ...
                                         GTK_MESSAGE_ERROR,
                                         GTK_BUTTONS_CLOSE,
                                         _( "The file “%s” is corrupted, unreadable, or could not be found." ),
                                         filename );
        g_signal_connect( G_OBJECT( dialog ), "response", G_CALLBACK( gtk_widget_destroy ), NULL );
        gtk_widget_show( dialog );
...
static int open_connection_last_stage( gpointer user_data )
{
 ...
 */
static void
rcw_gtksocket_not_available_dialog_response( GtkDialog *self, gint response_id, RemminaConnectionObject *cnnobj )
{
    TRACE_CALL( __func__ );
...
GtkWidget *rcw_open_from_file_full( RemminaFile *remminafile, GCallback disconnect_cb, gpointer data, guint *handler )
{
    ...
    if( remmina_protocol_widget_has_error( (RemminaProtocolWidget *)cnnobj->proto ) )
    {
        ...
        dialog = gtk_message_dialog_new(
            wparent, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE, "%s", msg );
    ...
    cnnobj->plugin_can_scale =
        remmina_plugin_manager_query_feature_by_type( REMMINA_PLUGIN_TYPE_PROTOCOL,
    ...
    {
        char *title = _( "Warning: This plugin requires GtkSocket, but this "
        ...
        dialog = gtk_message_dialog_new(
            GTK_WINDOW( cnnobj->cnnwin ), GTK_DIALOG_MODAL, GTK_MESSAGE_WARNING, GTK_BUTTONS_OK, "%s", title );

        gtk_message_dialog_format_secondary_text( GTK_MESSAGE_DIALOG( dialog ), "%s", err_msg );
        gtk_dialog_add_button(
            GTK_DIALOG( dialog ), _( "Open in web browser" ), GTKSOCKET_NOT_AVAIL_RESPONSE_OPEN_BROWSER );

        REMMINA_CRITICAL( g_strdup_printf( "%s\n%s", title, err_msg ) );

        g_signal_connect(
            G_OBJECT( dialog ), "response", G_CALLBACK( rcw_gtksocket_not_available_dialog_response ), cnnobj );
    ...
    if( remmina_protocol_widget_has_error( (RemminaProtocolWidget *)cnnobj->proto ) )
    {
        printf( "OK, an error occurred in initializing the protocol plugin before connecting. The error is %s.\n"
                "TODO: Put this string as an error to show",
                remmina_protocol_widget_get_error_message( (RemminaProtocolWidget *)cnnobj->proto ) );
        return cnnobj->proto;
    }
    ...
    cnnobj->deferred_open_size_allocate_handler = g_signal_connect(
        G_OBJECT( cnnobj->proto ), "size-allocate", G_CALLBACK( rpw_size_allocated_on_connection ), NULL );
