...
#include "remmina_chat_window.hpp"
...
static void remmina_protocol_widget_class_init( RemminaProtocolWidgetClass *klass )
{
    ...
    remmina_protocol_widget_signals[UPDATE_ALIGN_SIGNAL] =
        g_signal_new( "update-align",
                      ...
                      G_TYPE_NONE,
                      0 );
    remmina_protocol_widget_signals[LOCK_DYNRES_SIGNAL] =
        g_signal_new( "lock-dynres",
                      G_TYPE_FROM_CLASS( klass ),
                      static_cast<GSignalFlags>( G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION ),
                      G_STRUCT_OFFSET( RemminaProtocolWidgetClass, lock_dynres ),
                      NULL,
                      NULL,
                      g_cclosure_marshal_VOID__VOID,
                      G_TYPE_NONE,
                      0 );
    remmina_protocol_widget_signals[UNLOCK_DYNRES_SIGNAL] =
        g_signal_new( "unlock-dynres",
                      G_TYPE_FROM_CLASS( klass ),
                      static_cast<GSignalFlags>( G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION ),
                      G_STRUCT_OFFSET( RemminaProtocolWidgetClass, unlock_dynres ),
                      NULL,
                      NULL,
                      g_cclosure_marshal_VOID__VOID,
                      G_TYPE_NONE,
...
void remmina_protocol_widget_open_connection_real( gpointer data )
{
    ...
    else
    {
        ...
        if( num_ssh )
        {
            ...
            feature->opt2 = static_cast<void *>( const_cast<char *>( "utilities-terminal" ) );
            feature->opt3 = NULL;
            feature++;
            ...
            feature->opt2 = static_cast<void *>( const_cast<char *>( "folder-remote" ) );
            feature->opt3 = NULL;
            feature++;
        }
...
static int conn_closed( gpointer data )
{
    TRACE_CALL( __func__ );
...
static int conn_opened( gpointer data )
{
    TRACE_CALL( __func__ );
    RemminaProtocolWidget *gp = (RemminaProtocolWidget *)data;
...
static int update_align( gpointer data )
{
    TRACE_CALL( __func__ );
    RemminaProtocolWidget *gp = (RemminaProtocolWidget *)data;
...
static int lock_dynres( gpointer data )
{
    TRACE_CALL( __func__ );
    RemminaProtocolWidget *gp = (RemminaProtocolWidget *)data;
...
static int unlock_dynres( gpointer data )
{
    TRACE_CALL( __func__ );
    RemminaProtocolWidget *gp = (RemminaProtocolWidget *)data;
...
static int desktop_resize( gpointer data )
{
    TRACE_CALL( __func__ );
    RemminaProtocolWidget *gp = (RemminaProtocolWidget *)data;
...
{
    TRACE_CALL( __func__ );
    char *keystrokes = static_cast<char *>( g_object_get_data( G_OBJECT( widget ), "keystrokes" ) );
    guint *keyvals;
    gint i;
    ...
    KeystrokeReplace keystrokes_replaces[] = {
        { const_cast<char *>( "\\n" ), const_cast<char *>( "\n" ), GDK_KEY_Return },
        { const_cast<char *>( "\\t" ), const_cast<char *>( "\t" ), GDK_KEY_Tab },
        { const_cast<char *>( "\\b" ), const_cast<char *>( "\b" ), GDK_KEY_BackSpace },
        { const_cast<char *>( "\\e" ), const_cast<char *>( "\e" ), GDK_KEY_Escape },
        { const_cast<char *>( "\\\\" ), const_cast<char *>( "\\" ), GDK_KEY_backslash },
        { NULL, NULL, 0 } };
    ...
    if( remmina_protocol_widget_plugin_receives_keystrokes( gp ) )
    {
        ...
        for( i = 0; keystrokes_replaces[i].replace; i++ )
        {
            ...
            keystrokes = g_strdup( remmina_public_str_replace_in_place(
                keystrokes, keystrokes_replaces[i].search, keystrokes_replaces[i].replace ) );
            REMMINA_DEBUG( "Keystrokes after replacement is \'%s\'", keystrokes );
        }
        char *iter = g_strdup( keystrokes );
 ...
 */
void remmina_protocol_widget_send_clip_strokes( GtkClipboard *clipboard, const char *clip_text, gpointer data )
{
    TRACE_CALL( __func__ );
    RemminaProtocolWidget *gp = REMMINA_PROTOCOL_WIDGET( data );
    char *text = g_strdup( clip_text );
    guint *keyvals;
    gint i;
    GdkKeymap *keymap = gdk_keymap_get_for_display( gdk_display_get_default() );
    gunichar character;
    ...
    if( remmina_protocol_widget_plugin_receives_keystrokes( gp ) )
    {
        if( text )
        {
            ...
            for( i = 0; text_replaces[i].replace; i++ )
            {
                ...
                text = g_strdup(
                    remmina_public_str_replace_in_place( text, text_replaces[i].search, text_replaces[i].replace ) );
                REMMINA_DEBUG( "Text clipboard after replacement is \'%s\'", text );
            }
            char *iter = g_strdup( text );
...
void remmina_protocol_widget_send_clipboard( RemminaProtocolWidget *gp, GtkMenuItem *widget )
{
	 ...
	 */
    gtk_clipboard_request_text( clipboard, remmina_protocol_widget_send_clip_strokes, gp );
}
...
int remmina_protocol_widget_query_feature_by_ref( RemminaProtocolWidget *gp, const RemminaProtocolFeature *feature )
{
    TRACE_CALL( __func__ );
...
void remmina_protocol_widget_call_feature_by_ref( RemminaProtocolWidget *gp, const RemminaProtocolFeature *feature )
{
    ...
    switch( feature->id )
    {
        ...
        case REMMINA_PROTOCOL_FEATURE_TOOL_SSH:
            ...
            {
                rcw_open_from_file_full( remmina_file_dup_temp_protocol( gp->priv->remmina_file, "SSH" ),
                                         NULL,
                                         (RemminaSSHTunnel *)gp->priv->ssh_tunnels->pdata[0],
                                         NULL );
                return;
            }
            break;
        ...
        case REMMINA_PROTOCOL_FEATURE_TOOL_SFTP:
            if( gp->priv->ssh_tunnels && gp->priv->ssh_tunnels->len > 0 )
            {
                rcw_open_from_file_full( remmina_file_dup_temp_protocol( gp->priv->remmina_file, "SFTP" ),
                                         NULL,
                                         gp->priv->ssh_tunnels->pdata[0],
                                         NULL );
                return;
            }
            break;
...
static int remmina_protocol_widget_on_key_press( GtkWidget *widget, GdkEventKey *event, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
...
static int remmina_protocol_widget_on_key_release( GtkWidget *widget, GdkEventKey *event, RemminaProtocolWidget *gp )
{
    TRACE_CALL( __func__ );
    if( gp->priv->hostkey_func )
...
void remmina_protocol_widget_register_hostkey( RemminaProtocolWidget *gp, GtkWidget *widget )
{
    ...
    g_signal_connect(
        G_OBJECT( widget ), "key-release-event", G_CALLBACK( remmina_protocol_widget_on_key_release ), gp );
}
...
static int remmina_protocol_widget_tunnel_destroy( RemminaSSHTunnel *tunnel, gpointer data )
{
    TRACE_CALL( __func__ );
    RemminaProtocolWidget *gp = REMMINA_PROTOCOL_WIDGET( data );
    guint idx;
...
char *remmina_protocol_widget_start_direct_tunnel( RemminaProtocolWidget *gp, gint default_port, bool port_plus )
{
    ...
    char *ssh_tunnel_host, *srv_host, *dest;
...
int remmina_protocol_widget_start_reverse_tunnel( RemminaProtocolWidget *gp, gint local_port )
{
    ...
    msg = g_strdup_printf( _( "Awaiting incoming SSH connection on port %i…" ),
                           remmina_file_get_int( gp->priv->remmina_file, "listenport", 0 ) );
    mp = remmina_protocol_widget_mpprogress( gp->cnnobj, msg, cancel_start_reverse_tunnel_cb, NULL );
    g_free( msg );

    if( !remmina_ssh_tunnel_reverse(
            tunnel, remmina_file_get_int( gp->priv->remmina_file, "listenport", 0 ), local_port ) )
    {
        remmina_ssh_tunnel_free( tunnel );
        remmina_protocol_widget_set_error( gp, REMMINA_SSH( tunnel )->error );
        return FALSE;
...
int remmina_protocol_widget_ssh_exec( RemminaProtocolWidget *gp, bool wait, const char *fmt, ... )
{
    ...
    if( ssh_channel_open_session( channel ) == SSH_OK && ssh_channel_request_exec( channel, cmd ) == SSH_OK )
    {
        if( wait )
        {
            ...
            switch( status )
            {
                ...
                case 127:
                    ...
                    remmina_ssh_set_application_error(
                        REMMINA_SSH( tunnel ), _( "The “%s” command is not available on the SSH server." ), cmd );
                    break;
                default:
                    // TRANSLATORS: “%s” is a place holder for a unix command path. “%i” is a placeholder for an error code number.
                    remmina_ssh_set_application_error(
                        REMMINA_SSH( tunnel ),
                        _( "Could not run the “%s” command on the SSH server (status = %i)." ),
                        cmd,
                        status );
                    break;
            }
...
#ifdef HAVE_LIBSSH
static int remmina_protocol_widget_xport_tunnel_init_callback( RemminaSSHTunnel *tunnel, gpointer data )
{
    TRACE_CALL( __func__ );
    RemminaProtocolWidget *gp = REMMINA_PROTOCOL_WIDGET( data );
    char *server;
    ...
    ret = ( (RemminaXPortTunnelInitFunc)gp->priv->init_func )(
        gp, tunnel->remotedisplay, ( tunnel->bindlocalhost ? "localhost" : server ), port );
    g_free( server );

    return ret;
}

static int remmina_protocol_widget_xport_tunnel_connect_callback( RemminaSSHTunnel *tunnel, gpointer data )
{
    TRACE_CALL( __func__ );
    return TRUE;
}

static int remmina_protocol_widget_xport_tunnel_disconnect_callback( RemminaSSHTunnel *tunnel, gpointer data )
{
    TRACE_CALL( __func__ );
    RemminaProtocolWidget *gp = REMMINA_PROTOCOL_WIDGET( data );
...
int remmina_protocol_widget_start_xport_tunnel( RemminaProtocolWidget *gp, RemminaXPortTunnelInitFunc init_func )
{
    ...
    msg = g_strdup_printf( _( "Connecting to %s via SSH…" ),
                           remmina_file_get_string( gp->priv->remmina_file, "server" ) );
    ...
    if( !remmina_ssh_tunnel_xport( tunnel, bindlocalhost ) )
    {
        remmina_protocol_widget_set_error(
            gp, "Could not open channel, %s", ssh_get_error( REMMINA_SSH( tunnel )->session ) );
        remmina_ssh_tunnel_free( tunnel );
        return FALSE;
    }
...
static void authpanel_mt_cb( void *user_data, int button )
{
    ...
    if( button == GTK_RESPONSE_OK )
    {
        ...
        {
            d->gp->priv->password = remmina_message_panel_field_get_string( d->gp->priv->auth_message_panel,
                                                                            REMMINA_MESSAGE_PANEL_PASSWORD );
            d->gp->priv->username = remmina_message_panel_field_get_string( d->gp->priv->auth_message_panel,
                                                                            REMMINA_MESSAGE_PANEL_USERNAME );
            d->gp->priv->domain =
                remmina_message_panel_field_get_string( d->gp->priv->auth_message_panel, REMMINA_MESSAGE_PANEL_DOMAIN );
            d->gp->priv->save_password = remmina_message_panel_field_get_switch_state(
                d->gp->priv->auth_message_panel, REMMINA_MESSAGE_PANEL_FLAG_SAVEPASSWORD );
        }
        else if( d->dtype == RPWDT_AUTHX509 )
        {
            d->gp->priv->cacert = remmina_message_panel_field_get_filename( d->gp->priv->auth_message_panel,
                                                                            REMMINA_MESSAGE_PANEL_CACERTFILE );
            d->gp->priv->cacrl = remmina_message_panel_field_get_filename( d->gp->priv->auth_message_panel,
                                                                           REMMINA_MESSAGE_PANEL_CACRLFILE );
            d->gp->priv->clientcert = remmina_message_panel_field_get_filename( d->gp->priv->auth_message_panel,
                                                                                REMMINA_MESSAGE_PANEL_CLIENTCERTFILE );
            d->gp->priv->clientkey = remmina_message_panel_field_get_filename( d->gp->priv->auth_message_panel,
    ...
    if( d->dtype == RPWDT_AUTH )
    {
        ...
        if( d->pflags & REMMINA_MESSAGE_PANEL_FLAG_SAVEPASSWORD )
            remmina_message_panel_field_set_switch(
                ...
                ( d->default_password == NULL || d->default_password[0] == 0 ) ? FALSE : TRUE );
    }
...
{
    return remmina_protocol_widget_dialog(
        RPWDT_QUESTIONYESNO, gp, static_cast<RemminaMessagePanelFlags>( 0 ), msg, NULL, NULL, NULL, NULL );
}
...
{
    TRACE_CALL( __func__ );
    return remmina_protocol_widget_dialog( RPWDT_AUTH,
                                           gp,
                                           pflags,
                                           title,
                                           default_username,
                                           default_password,
                                           default_domain,
                                           password_prompt == NULL ? _( "Password" ) : password_prompt );
}
                                                           ...
                                                           bool allow_password_saving )
{
    ...
    if( remmina_file_get_filename( remminafile ) != NULL && !remminafile->prevent_saving && allow_password_saving )
    ...
    return remmina_protocol_widget_dialog( RPWDT_AUTH,
                                           ...
                                           _( "Type in SSH username and password." ),
                                           username,
                                           password,
                                           NULL,
                                           _( "Password" ) );
...
/*
 * gint remmina_protocol_widget_panel_authpwd(RemminaProtocolWidget* gp, RemminaAuthpwdType authpwd_type, bool allow_password_saving)
 * {
 *      TRACE_CALL(__func__);
 *      unsigned pflags;
 *      RemminaFile* remminafile = gp->priv->remmina_file;
...
gint remmina_protocol_widget_panel_authx509( RemminaProtocolWidget *gp )
{
    ...
    return remmina_protocol_widget_dialog(
        RPWDT_AUTHX509, gp, static_cast<RemminaMessagePanelFlags>( 0 ), NULL, NULL, NULL, NULL, NULL );
}
                                                    ...
                                                    const char *fingerprint )
{
    ...
    if( remmina_pref_get_boolean( "trust_all" ) )
    {
        ...
        remmina_public_send_notification(
            "remmina-security-trust-all-id", _( "Fingerprint automatically accepted" ), fingerprint );
        rc = GTK_RESPONSE_OK;
        return rc;
    }
    ...
    rc = remmina_protocol_widget_dialog(
        RPWDT_QUESTIONYESNO, gp, static_cast<RemminaMessagePanelFlags>( 0 ), s, NULL, NULL, NULL, NULL );
    g_free( s );
                                                        ...
                                                        const char *old_fingerprint )
{
    ...
    if( remmina_pref_get_boolean( "trust_all" ) )
    {
        /* For compatibility with plugin API: The plugin expects GTK_RESPONSE_OK when user confirms new cert */
        remmina_public_send_notification(
            "remmina-security-trust-all-id", _( "Fingerprint automatically accepted" ), new_fingerprint );
        rc = GTK_RESPONSE_OK;
        return rc;
    }
    // For markup see https://developer.gnome.org/pygtk/stable/pango-markup-language.html
                                        ...
                                        void ( *on_destroy )( RemminaProtocolWidget *gp ) )
{
    ...
    {
        gp->priv->chat_window =
            remmina_chat_window_new( GTK_WINDOW( gtk_widget_get_toplevel( GTK_WIDGET( gp ) ) ), name );
        g_signal_connect_swapped( G_OBJECT( gp->priv->chat_window ), "send", G_CALLBACK( on_send ), gp );
        g_signal_connect_swapped(
            G_OBJECT( gp->priv->chat_window ), "destroy", G_CALLBACK( remmina_protocol_widget_chat_on_destroy ), gp );
                                    ...
                                    RemminaConnectionObject *cnnobj )
{
    ...
    plugin = (RemminaProtocolPlugin *)remmina_plugin_manager_get_plugin(
        REMMINA_PLUGIN_TYPE_PROTOCOL, remmina_file_get_string( remminafile, "protocol" ) );

    if( !plugin || !plugin->init || !plugin->open_connection )
    {
        // TRANSLATORS: “%s” is a placeholder for a protocol name, like “RDP”.
        remmina_protocol_widget_set_error(
            gp, _( "Install the %s protocol plugin first." ), remmina_file_get_string( remminafile, "protocol" ) );
        gp->priv->plugin = NULL;
        return;
    }
    ...
    gp->priv->scalemode =
        static_cast<RemminaScaleMode>( remmina_file_get_int( gp->priv->remmina_file, "scale", FALSE ) );
...
void remmina_protocol_widget_update_remote_resolution( RemminaProtocolWidget *gp )
{
    ...
    res_mode = static_cast<RemminaProtocolWidgetResolutionMode>(
        remmina_file_get_int( gp->priv->remmina_file, "resolution_mode", RES_INVALID ) );
    scalemode = static_cast<RemminaScaleMode>(
        remmina_file_get_int( gp->priv->remmina_file, "scale", REMMINA_PROTOCOL_WIDGET_SCALE_MODE_NONE ) );
