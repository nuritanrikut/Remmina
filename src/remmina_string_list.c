...
void remmina_string_list_on_action_add( GtkWidget *widget, gpointer user_data )
{
    ...
    gtk_tree_view_set_cursor_on_cell( string_list->treeview_items,
                                      path,
                                      string_list->treeviewcolumn_item,
                                      GTK_CELL_RENDERER( string_list->priv->two_columns
                                                             ? string_list->cellrenderertext_item1
                                                             : string_list->cellrenderertext_item2 ),
                                      TRUE );
    gtk_tree_path_free( path );
...
void remmina_string_list_set_text( const gchar *text, const gboolean clear_data )
{
    ...
    for( i = 0; i < g_strv_length( items ); i++ )
    {
        ...
        if( g_strv_length( values ) > 1 )
        {
            /* Two columns data */
            gtk_list_store_set(
                string_list->liststore_items, &iter, COLUMN_DESCRIPTION, values[0], COLUMN_VALUE, values[1], -1 );
        }
        else
        {
            /* Single column data */
            gtk_list_store_set( string_list->liststore_items, &iter, COLUMN_VALUE, values[0], -1 );
        }
        g_strfreev( values );
    }
    g_strfreev( items );
...
gchar *remmina_string_list_get_text( void )
{
    ...
    while( ret )
    {
        gtk_tree_model_get( GTK_TREE_MODEL( string_list->liststore_items ),
                            &iter,
                            COLUMN_DESCRIPTION,
                            &item_description,
                            COLUMN_VALUE,
...
GtkDialog *remmina_string_list_new( gboolean two_columns, const gchar *fields_separator )
{
    ...
    string_list->builder =
        remmina_public_gtk_builder_new_from_resource( "/org/remmina/Remmina/src/../data/ui/remmina_string_list.glade" );
