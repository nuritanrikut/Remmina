 ...
 **/
static inline PyObject *pygobject_init( int req_major, int req_minor, int req_micro )
{
...
#    if PY_VERSION_HEX < 0x03000000
            PyErr_Format(
                PyExc_ImportError, "could not import gobject (error was: %s)", PyString_AsString( py_orig_exc ) );
#    else
            {
                 ...
                 */
                PyObject *errmsg = PyUnicode_FromFormat( "could not import gobject (error was: %U)", py_orig_exc );
...
#    endif
        ...
        {
            PyErr_SetString( PyExc_ImportError, "could not import gobject (no error given)" );
        }
        return NULL;
    }
...
#    endif
    ...
    {
        PyErr_SetString( PyExc_ImportError, "could not import gobject (could not find _PyGObject_API object)" );
        Py_DECREF( gobject );
        return NULL;
    }
    ...
    if( req_major != -1 )
    {
        ...
        if( !version )
        {
            PyErr_SetString( PyExc_ImportError, "could not import gobject (version too old)" );
            Py_DECREF( gobject );
            return NULL;
        }
        if( !PyArg_ParseTuple( version, "iii", &found_major, &found_minor, &found_micro ) )
        {
            PyErr_SetString( PyExc_ImportError, "could not import gobject (version has invalid format)" );
            Py_DECREF( version );
            Py_DECREF( gobject );
            return NULL;
        }
 ...
 */
#    define PYLIST_FROMGLIBLIST( type, prefix, py_list, list, item_convert_func, list_free, list_item_free ) \
        G_STMT_START \
        { \
            gint i, len; \
            PyObject *item; \
 ...
 *
 */
#    define PYLIST_FROMGLIST( py_list, list, item_convert_func, list_free, list_item_free ) \
        PYLIST_FROMGLIBLIST( GList, g_list, py_list, list, item_convert_func, list_free, list_item_free )
 ...
 *
 */
#    define PYLIST_FROMGSLIST( py_list, list, item_convert_func, list_free, list_item_free ) \
        PYLIST_FROMGLIBLIST( GSList, g_slist, py_list, list, item_convert_func, list_free, list_item_free )
 ...
 * macro(s) created from this macro.
 */
#    define PYLIST_ASGLIBLIST( \
        type, prefix, py_list, list, check_func, convert_func, child_free_func, errormsg, errorreturn ) \
        G_STMT_START \
        { \
            Py_ssize_t i, n_list; \
 ...
 * freed.
 */
#    define PYLIST_ASGLIST( py_list, list, check_func, convert_func, child_free_func, errormsg, errorreturn ) \
        PYLIST_ASGLIBLIST( \
            GList, g_list, py_list, list, check_func, convert_func, child_free_func, errormsg, errorreturn )
 ...
 * freed.
 */
#    define PYLIST_ASGSLIST( py_list, list, check_func, convert_func, child_free_func, errormsg, errorreturn ) \
        PYLIST_ASGLIBLIST( \
