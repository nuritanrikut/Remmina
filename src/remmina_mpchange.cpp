...
static int remmina_mpchange_fieldcompare( const char *needle, const char *haystack, int *matchcount )
{
    TRACE_CALL( __func__ );
...
static void remmina_mpchange_file_list_callback( RemminaFile *remminafile, gpointer user_data )
{
    ...
    gtk_list_store_set( store,
                        ...
                        s,
                        COL_FILENAME,
                        remminafile->filename,
                        -1 );
    g_free( s );
}

static void remmina_mpchange_checkbox_toggle( GtkCellRendererToggle *cell, char *path_string, gpointer user_data )
{
    TRACE_CALL( __func__ );
    GtkTreeIter iter;
...
static int remmina_mpchange_searchfield_changed_to( gpointer user_data )
{
    ...
    mpcp->store =
        gtk_list_store_new( NUM_COLS, G_TYPE_BOOLEAN, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING );
...
static void remmina_mpchange_searchfield_changed( GtkSearchEntry *se, gpointer user_data )
{
    ...
    mpcp->searchentrychange_timeout_source_id =
        g_timeout_add( 500, remmina_mpchange_searchfield_changed_to, user_data );
}
...
static int remmina_file_multipasswd_changer_mt( gpointer d )
{
    TRACE_CALL( __func__ );
    mpchanger_params *mpcp = static_cast<mpchanger_params*>(d);
    GtkBuilder *bu;
    GtkDialog *dialog;
    ...
    if( initerror )
    {
        ...
        msgDialog = gtk_message_dialog_new(
            mainwindow, GTK_DIALOG_MODAL, GTK_MESSAGE_WARNING, GTK_BUTTONS_OK, "%s", initerror );
        gtk_dialog_run( GTK_DIALOG( msgDialog ) );
        gtk_widget_destroy( msgDialog );
        return FALSE;
    }
    ...
    g_signal_connect(
        G_OBJECT( mpcp->eGroup ), "changed", G_CALLBACK( remmina_mpchange_searchfield_changed ), (gpointer)mpcp );
    g_signal_connect(
        G_OBJECT( mpcp->eGroup ), "stop-search", G_CALLBACK( remmina_mpchange_stopsearch ), (gpointer)mpcp );

    mpcp->eUsername = GTK_ENTRY( GET_DIALOG_OBJECT( "usernameEntry" ) );
    gtk_entry_set_text( mpcp->eUsername, mpcp->username );
    g_signal_connect(
        G_OBJECT( mpcp->eUsername ), "changed", G_CALLBACK( remmina_mpchange_searchfield_changed ), (gpointer)mpcp );

    mpcp->eDomain = GTK_ENTRY( GET_DIALOG_OBJECT( "domainEntry" ) );
    gtk_entry_set_text( mpcp->eDomain, mpcp->domain );
    g_signal_connect(
        G_OBJECT( mpcp->eDomain ), "changed", G_CALLBACK( remmina_mpchange_searchfield_changed ), (gpointer)mpcp );
    ...
    if( mpcp->changed_passwords_count )
    {
        GtkWidget *msgDialog;
        msgDialog = gtk_message_dialog_new(
            GTK_WINDOW( mpcp->dialog ),
            GTK_DIALOG_DESTROY_WITH_PARENT,
            GTK_MESSAGE_INFO,
            GTK_BUTTONS_OK,
            ngettext( "%d password changed.", "%d passwords changed.", mpcp->changed_passwords_count ),
            mpcp->changed_passwords_count );
        gtk_dialog_run( GTK_DIALOG( msgDialog ) );
        gtk_widget_destroy( msgDialog );
    }
                                ...
                                const char *password )
{
    ...
    mpcp = static_cast<mpchanger_params*>(g_malloc0( sizeof( mpchanger_params ) ));
