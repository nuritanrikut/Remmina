...
static void remmina_sftp_client_class_init( RemminaSFTPClientClass *klass )
{
    TRACE_CALL( __func__ );
}
...
#    define GET_SFTPATTR_TYPE( a, type ) \
        ...
        { \
            type = \
                ( a->type == SSH_FILEXFER_TYPE_DIRECTORY ? REMMINA_FTP_FILE_TYPE_DIR : REMMINA_FTP_FILE_TYPE_FILE ); \
        }

/* ------------------------ The Task Thread routines ----------------------------- */

static int remmina_sftp_client_refresh( RemminaSFTPClient *client );

#    define THREAD_CHECK_EXIT ( !client->taskid || client->thread_abort )

static int remmina_sftp_client_thread_update_task( RemminaSFTPClient *client, RemminaFTPTask *task )
{
    TRACE_CALL( __func__ );
...
static void
remmina_sftp_client_thread_set_error( RemminaSFTPClient *client, RemminaFTPTask *task, const char *error_format, ... )
{
    TRACE_CALL( __func__ );
    va_list args;
...
static int remmina_sftp_client_thread_download_file( RemminaSFTPClient *client,
                                                          RemminaSFTP *sftp,
                                                          RemminaFTPTask *task,
                                                          ...
                                                          guint64 *donesize )
{
            ...
            case GTK_RESPONSE_ACCEPT:
                ...
                if( !local_file )
                {
                    ...
                    remmina_sftp_client_thread_set_error(
                        client, task, _( "Could not create the file “%s”." ), local_path );
                    return FALSE;
                }
                size = 0;
                break;
    ...
    if( !remote_file )
    {
        ...
        remmina_sftp_client_thread_set_error( client,
                                              task,
                                              _( "Could not open the file “%s” on the server. %s" ),
                                              remote_path,
    ...
    if( size > 0 )
    {
        if( sftp_seek64( remote_file, size ) < 0 )
        {
            ...
            remmina_sftp_client_thread_set_error( client,
                                                  task,
                                                  "Could not download the file “%s”. %s",
                                                  remote_path,
                                                  ssh_get_error( REMMINA_SSH( client->sftp )->session ) );
            return FALSE;
        }
...
static int remmina_sftp_client_thread_recursive_dir( RemminaSFTPClient *client,
                                                          RemminaSFTP *sftp,
                                                          RemminaFTPTask *task,
                                                          ...
                                                          GPtrArray *array )
{
    ...
    if( !sftpdir )
    {
        remmina_sftp_client_thread_set_error( client,
                                              task,
                                              _( "Could not open the folder “%s”. %s" ),
                                              dir_path,
                                              ssh_get_error( REMMINA_SSH( client->sftp )->session ) );
        g_free( dir_path );
        return FALSE;
    ...
    {
        if( g_strcmp0( sftpattr->name, "." ) != 0 && g_strcmp0( sftpattr->name, ".." ) != 0 )
        {
...
static int remmina_sftp_client_thread_recursive_localdir( RemminaSFTPClient *client,
                                                               RemminaFTPTask *task,
                                                               const char *rootdir_path,
                                                               const char *subdir_path,
                                                               GPtrArray *array )
...
static int remmina_sftp_client_thread_mkdir( RemminaSFTPClient *client,
                                                  RemminaSFTP *sftp,
                                                  RemminaFTPTask *task,
                                                  const char *path )
{
    ...
    if( sftp_mkdir( sftp->sftp_sess, path, 0755 ) < 0 )
    {
        remmina_sftp_client_thread_set_error( client,
                                              task,
                                              _( "Could not create the folder “%s” on the server. %s" ),
                                              path,
                                              ssh_get_error( REMMINA_SSH( client->sftp )->session ) );
        return FALSE;
    }
...
static int remmina_sftp_client_thread_upload_file( RemminaSFTPClient *client,
                                                        RemminaSFTP *sftp,
                                                        RemminaFTPTask *task,
                                                        const char *remote_path,
                                                        const char *local_path,
                                                        guint64 *donesize )
{
    ...
    if( !remote_file )
    {
        remmina_sftp_client_thread_set_error( client,
                                              task,
                                              _( "Could not create the file “%s” on the server. %s" ),
                                              remote_path,
                                              ssh_get_error( REMMINA_SSH( client->sftp )->session ) );
        return FALSE;
    }
    ...
    while( !THREAD_CHECK_EXIT && ( len = fread( buf, 1, sizeof( buf ), local_file ) ) > 0 )
    {
        ...
        if( sftp_write( remote_file, buf, len ) < len )
        {
            ...
            remmina_sftp_client_thread_set_error( client,
                                                  task,
                                                  _( "Could not write to the file “%s” on the server. %s" ),
                                                  remote_path,
                                                  ssh_get_error( REMMINA_SSH( client->sftp )->session ) );
            return FALSE;
        }
...
static gpointer remmina_sftp_client_thread_main( gpointer data )
{
    ...
    while( task )
    {
        ...
        if( !sftp )
        {
            ...
            if( remmina_ssh_auth( REMMINA_SSH( sftp ), REMMINA_SSH( sftp )->password, client->gp, NULL )
        ...
        switch( task->tasktype )
        {
            case REMMINA_FTP_TASK_TYPE_DOWNLOAD:
                switch( task->type )
                {
                    case REMMINA_FTP_FILE_TYPE_FILE:
                        ret = remmina_sftp_client_thread_download_file( client, sftp, task, remote, local, &size );
                        break;
                    ...
                    case REMMINA_FTP_FILE_TYPE_DIR:
                        ...
                        if( ret )
                        {
                            for( i = 0; i < array->len; i++ )
                            {
                                ...
                                remote_file =
                                    remmina_public_combine_path( remote, (char *)g_ptr_array_index( array, i ) );
                                local_file =
                                    remmina_public_combine_path( local, (char *)g_ptr_array_index( array, i ) );
                                ret = remmina_sftp_client_thread_download_file(
                                    client, sftp, task, remote_file, local_file, &size );
                                g_free( remote_file );
                                g_free( local_file );
                                if( !ret )
                                    break;
            ...
            case REMMINA_FTP_TASK_TYPE_UPLOAD:
                ...
                {
                    case REMMINA_FTP_FILE_TYPE_FILE:
                        ret = remmina_sftp_client_thread_upload_file( client, sftp, task, remote, local, &size );
                        break;
                    ...
                    case REMMINA_FTP_FILE_TYPE_DIR:
                        ...
                        if( ret )
                        {
                            for( i = 0; i < array->len; i++ )
                            {
                                ...
                                remote_file =
                                    remmina_public_combine_path( remote, (char *)g_ptr_array_index( array, i ) );
                                local_file = g_build_filename( local, (char *)g_ptr_array_index( array, i ), NULL );
                                if( g_file_test( local_file, G_FILE_TEST_IS_DIR ) )
                                    ret = remmina_sftp_client_thread_mkdir( client, sftp, task, remote_file );
                                else
                                    ret = remmina_sftp_client_thread_upload_file(
                                        client, sftp, task, remote_file, local_file, &size );
                                g_free( remote_file );
                                g_free( local_file );
                                if( !ret )
...
static sftp_dir remmina_sftp_client_sftp_session_opendir( RemminaSFTPClient *client, const char *dir )
{
    TRACE_CALL( __func__ );
    sftp_dir sftpdir;
    GtkWidget *dialog;

    sftpdir = sftp_opendir( client->sftp->sftp_sess, (char *)dir );
    if( !sftpdir )
    {
...
static int remmina_sftp_client_sftp_session_closedir( RemminaSFTPClient *client, sftp_dir sftpdir )
{
    TRACE_CALL( __func__ );
    GtkWidget *dialog;
    ...
    if( !sftp_dir_eof( sftpdir ) )
    {
        dialog = gtk_message_dialog_new( GTK_WINDOW( gtk_widget_get_toplevel( GTK_WIDGET( client ) ) ),
                                         ...
                                         GTK_MESSAGE_ERROR,
                                         GTK_BUTTONS_OK,
                                         _( "Could not read from the folder. %s" ),
                                         ssh_get_error( REMMINA_SSH( client->sftp )->session ) );
        gtk_dialog_run( GTK_DIALOG( dialog ) );
        gtk_widget_destroy( dialog );
        return FALSE;
...
static void remmina_sftp_client_on_opendir( RemminaSFTPClient *client, const char *dir, gpointer data )
{
    TRACE_CALL( __func__ );
    sftp_dir sftpdir;
    sftp_attributes sftpattr;
...
static int remmina_sftp_client_on_canceltask( RemminaSFTPClient *client, gint taskid, gpointer data )
{
    TRACE_CALL( __func__ );
    GtkWidget *dialog;
    gint ret;
...
static int remmina_sftp_client_on_deletefile( RemminaSFTPClient *client, gint type, char *name, gpointer data )
{
    TRACE_CALL( __func__ );
    GtkWidget *dialog;
    gint ret = 0;
...
static void remmina_sftp_client_init( RemminaSFTPClient *client )
{
    ...
    g_signal_connect( G_OBJECT( client ), "destroy", G_CALLBACK( remmina_sftp_client_destroy ), NULL );
    g_signal_connect( G_OBJECT( client ), "open-dir", G_CALLBACK( remmina_sftp_client_on_opendir ), NULL );
    g_signal_connect( G_OBJECT( client ), "new-task", G_CALLBACK( remmina_sftp_client_on_newtask ), NULL );
    g_signal_connect( G_OBJECT( client ), "cancel-task", G_CALLBACK( remmina_sftp_client_on_canceltask ), NULL );
    g_signal_connect( G_OBJECT( client ), "delete-file", G_CALLBACK( remmina_sftp_client_on_deletefile ), NULL );
}
