...
#define REMMINA_TYPE_CELL_RENDERER_PIXBUF ( remmina_cell_renderer_pixbuf_get_type() )
...
static int remmina_cell_renderer_pixbuf_activate( GtkCellRenderer *renderer,
                                                  GdkEvent *event,
                                                  GtkWidget *widget,
                                                  const char *path,
...
#define BUSY_CURSOR \
    ...
    { \
        gdk_window_set_cursor( gtk_widget_get_window( GTK_WIDGET( client ) ), \
                                                          ...
                                                          gpointer user_data )
{
    ...
    gtk_tree_model_get(
        model, iter, REMMINA_FTP_TASK_COLUMN_TASKTYPE, &tasktype, REMMINA_FTP_TASK_COLUMN_STATUS, &status, -1 );
    ...
    switch( status )
    {
        ...
        case REMMINA_FTP_TASK_STATUS_RUN:
            g_object_set(
                renderer, "icon-name", ( tasktype == REMMINA_FTP_TASK_TYPE_UPLOAD ? "go-up" : "go-down" ), NULL );
            break;
...
static char *remmina_ftp_client_size_to_str( gfloat size )
{
    TRACE_CALL( __func__ );
...
static void remmina_ftp_client_open_dir( RemminaFTPClient *client, const char *dir )
{
    TRACE_CALL( __func__ );
    BUSY_CURSOR
...
static void remmina_ftp_client_set_file_action_sensitive( RemminaFTPClient *client, bool sensitive )
{
    TRACE_CALL( __func__ );
    gint i;
...
static char *remmina_ftp_client_get_download_dir( RemminaFTPClient *client )
{
    TRACE_CALL( __func__ );
    RemminaFTPClientPriv *priv = (RemminaFTPClientPriv *)client->priv;
    GtkWidget *dialog;
...
static void remmina_ftp_client_download( RemminaFTPClient *client, GtkTreeIter *piter, const char *localdir )
{
    TRACE_CALL( __func__ );
    RemminaFTPClientPriv *priv = (RemminaFTPClientPriv *)client->priv;
    ...
    gtk_tree_model_get( priv->file_list_sort,
                        piter,
...
static int remmina_ftp_client_task_list_on_query_tooltip( GtkWidget *widget,
                                                          gint x,
                                                          gint y,
                                                          bool keyboard_tip,
                                                          GtkTooltip *tooltip,
                                                          RemminaFTPClient *client )
{
    ...
    if( !gtk_tree_view_get_tooltip_context(
            GTK_TREE_VIEW( priv->task_list_view ), &x, &y, keyboard_tip, NULL, &path, &iter ) )
    {
        return FALSE;
    }
...
static void remmina_ftp_client_action_delete( GObject *object, RemminaFTPClient *client )
{
    ...
    while( list_iter )
    {
        ...
        gtk_tree_model_get(
            priv->file_list_sort, &iter, REMMINA_FTP_FILE_COLUMN_TYPE, &type, REMMINA_FTP_FILE_COLUMN_NAME, &name, -1 );
...
static void remmina_ftp_client_upload_folder_on_toggled( GtkToggleButton *togglebutton, GtkWidget *widget )
{
    ...
    gtk_file_chooser_set_action( GTK_FILE_CHOOSER( widget ),
                                 gtk_toggle_button_get_active( togglebutton ) ? GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER
...
static void remmina_ftp_client_action_upload( GObject *object, RemminaFTPClient *client )
{
    ...
    g_signal_connect(
        G_OBJECT( upload_folder_check ), "toggled", G_CALLBACK( remmina_ftp_client_upload_folder_on_toggled ), dialog );
    ...
    type = gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( upload_folder_check ) ) ? REMMINA_FTP_FILE_TYPE_DIR
...
static void
remmina_ftp_client_task_list_cell_on_activate( GtkCellRenderer *renderer, char *path, RemminaFTPClient *client )
{
    TRACE_CALL( __func__ );
    RemminaFTPClientPriv *priv = (RemminaFTPClientPriv *)client->priv;
    GtkTreeIter iter;
...
static int remmina_ftp_client_filter_visible_func( GtkTreeModel *model, GtkTreeIter *iter, RemminaFTPClient *client )
{
    TRACE_CALL( __func__ );
    char *name;
    bool result = TRUE;
...
static void remmina_ftp_client_init( RemminaFTPClient *client )
{
    ...
    gtk_tree_view_column_set_cell_data_func(
        column, renderer, remmina_ftp_client_cell_data_filetype_pixbuf, NULL, NULL );
    ...
    renderer = gtk_cell_renderer_text_new();
    column =
        gtk_tree_view_column_new_with_attributes( _( "User" ), renderer, "text", REMMINA_FTP_FILE_COLUMN_USER, NULL );
    gtk_tree_view_column_set_resizable( column, TRUE );
    ...
    renderer = gtk_cell_renderer_text_new();
    column =
        gtk_tree_view_column_new_with_attributes( _( "Group" ), renderer, "text", REMMINA_FTP_FILE_COLUMN_GROUP, NULL );
    gtk_tree_view_column_set_resizable( column, TRUE );
    ...
    column = gtk_tree_view_column_new_with_attributes(
        _( "Permission" ), renderer, "text", REMMINA_FTP_FILE_COLUMN_PERMISSION, NULL );
    gtk_tree_view_column_set_resizable( column, TRUE );
    ...
    gtk_tree_model_filter_set_visible_func( GTK_TREE_MODEL_FILTER( priv->file_list_filter ),
                                            (GtkTreeModelFilterVisibleFunc)remmina_ftp_client_filter_visible_func,
                                            client,
                                            NULL );

    priv->file_list_sort = gtk_tree_model_sort_new_with_model( priv->file_list_filter );
    gtk_tree_sortable_set_sort_column_id(
        GTK_TREE_SORTABLE( priv->file_list_sort ), REMMINA_FTP_FILE_COLUMN_NAME_SORT, GTK_SORT_ASCENDING );
    ...
    gtk_tree_view_column_pack_start( column, renderer, FALSE );
    gtk_tree_view_column_set_cell_data_func(
        column, renderer, remmina_ftp_client_cell_data_progress_pixbuf, NULL, NULL );
    renderer = gtk_cell_renderer_pixbuf_new();
    gtk_tree_view_column_pack_start( column, renderer, FALSE );
    gtk_tree_view_column_set_cell_data_func(
        column, renderer, remmina_ftp_client_cell_data_filetype_pixbuf, NULL, NULL );
    ...
    renderer = gtk_cell_renderer_text_new();
    column = gtk_tree_view_column_new_with_attributes(
        _( "Remote" ), renderer, "text", REMMINA_FTP_TASK_COLUMN_REMOTEDIR, NULL );
    gtk_tree_view_column_set_resizable( column, TRUE );
    ...
    renderer = gtk_cell_renderer_text_new();
    column = gtk_tree_view_column_new_with_attributes(
        _( "Local" ), renderer, "text", REMMINA_FTP_TASK_COLUMN_LOCALDIR, NULL );
    gtk_tree_view_column_set_resizable( column, TRUE );
    ...
    g_signal_connect(
        G_OBJECT( renderer ), "activate", G_CALLBACK( remmina_ftp_client_task_list_cell_on_activate ), client );
    ...
    g_signal_connect( G_OBJECT( gtk_bin_get_child( GTK_BIN( priv->directory_combo ) ) ),
                      "activate",
                      G_CALLBACK( remmina_ftp_client_dir_on_activate ),
                      client );
    g_signal_connect(
        G_OBJECT( priv->directory_combo ), "changed", G_CALLBACK( remmina_ftp_client_dir_on_changed ), client );
    g_signal_connect( G_OBJECT( priv->file_list_view ),
                      "button-press-event",
                      G_CALLBACK( remmina_ftp_client_file_list_on_button_press ),
                      client );
    g_signal_connect( G_OBJECT( gtk_tree_view_get_selection( GTK_TREE_VIEW( priv->file_list_view ) ) ),
                      "changed",
                      G_CALLBACK( remmina_ftp_client_file_selection_on_changed ),
                      client );
    g_signal_connect( G_OBJECT( priv->task_list_view ),
                      "query-tooltip",
                      G_CALLBACK( remmina_ftp_client_task_list_on_query_tooltip ),
                      client );
}
...
void remmina_ftp_client_add_file( RemminaFTPClient *client, ... )
{
    ...
    gtk_tree_model_get(
        GTK_TREE_MODEL( store ), &iter, REMMINA_FTP_FILE_COLUMN_TYPE, &type, REMMINA_FTP_FILE_COLUMN_NAME, &name, -1 );
...
RemminaFTPTask *remmina_ftp_client_get_waiting_task( RemminaFTPClient *client )
{
    ...
    while( 1 )
    {
        gtk_tree_model_get( priv->task_list_model,
                            &iter,
                            REMMINA_FTP_TASK_COLUMN_TYPE,
                            &task.type,
                            REMMINA_FTP_TASK_COLUMN_NAME,
