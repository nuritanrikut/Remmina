...
static PyMethodDef python_protocol_widget_type_methods[] = {
    ...
    { "panel_changed_certificate",
      (PyCFunction)protocol_widget_panel_changed_certificate,
      METH_VARARGS | METH_KEYWORDS,
      "" },
...
static PyTypeObject python_protocol_widget_type = { PyVarObject_HEAD_INIT( NULL, 0 ).tp_name = "remmina.RemminaWidget",
...
static PyObject *protocol_widget_start_reverse_tunnel( PyRemminaProtocolWidget *self, PyObject *var_local_port )
{
    ...
    if( var_local_port && PyLong_Check( var_local_port ) )
    {
        return Py_BuildValue(
            "p", remmina_protocol_widget_start_reverse_tunnel( self->gp, (gint)PyLong_AsLong( var_local_port ) ) );
    }
    else
    {
...
static PyObject *protocol_widget_panel_auth( PyRemminaProtocolWidget *self, PyObject *args )
{
    ...
    {
        if( pflags != REMMINA_MESSAGE_PANEL_FLAG_USERNAME && pflags != REMMINA_MESSAGE_PANEL_FLAG_USERNAME_READONLY
            && pflags != REMMINA_MESSAGE_PANEL_FLAG_DOMAIN && pflags != REMMINA_MESSAGE_PANEL_FLAG_SAVEPASSWORD )
        {
            g_printerr(
                "panel_auth(pflags, title, default_username, default_password, default_domain, password_prompt): "
...
static PyObject *protocol_widget_panel_changed_certificate( PyRemminaProtocolWidget *self, PyObject *args )
{
    ...
    if( PyArg_ParseTuple( args, "sss", &subject, &issuer, &new_fingerprint, &old_fingerprint ) )
    {
        remmina_protocol_widget_panel_changed_certificate(
            self->gp, subject, issuer, new_fingerprint, old_fingerprint );
    }
    else
    {
        g_printerr( "panel_changed_certificate(subject, issuer, new_fingerprint, old_fingerprint): Error parsing "
                    "arguments!\n" );
        PyErr_Print();
    }
    return Py_None;
...
static PyObject *protocol_widget_chat_open( PyRemminaProtocolWidget *self, PyObject *var_name )
{
    ...
    if( var_name, PyUnicode_Check( var_name ) )
    {
        remmina_protocol_widget_chat_open(
            self->gp, PyUnicode_AsUTF8( var_name ), _on_send_callback_wrapper, _on_destroy_callback_wrapper );
    }
    else
    {
