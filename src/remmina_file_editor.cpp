...
static const char *server_tips = N_( "<big>"
                                     "Supported formats\n"
                                     "• server\n"
                                     "• server[:port]\n"
...
static const char *cmd_tips = N_( "<big>"
...
#ifdef HAVE_LIBSSH
static const char *server_tips2 = N_( "<big>"
                                      "Supported formats\n"
                                      "• server\n"
                                      "• server[:port]\n"
 ...
 */
static void
remmina_file_editor_show_validation_error_popup( RemminaFileEditor *gfe, GtkWidget *failed_widget, GError *err )
{
    if( !err )
    {
    ...
    char *markup = g_strdup_printf( "<span size='large'>%s</span>", err->message );
    ...
    alert_icon = gtk_image_new_from_icon_name( "dialog-warning-symbolic", GTK_ICON_SIZE_DND );
    ...
    g_signal_connect( G_OBJECT( err_window ), "focus-out-event", G_CALLBACK( gtk_window_close ), NULL );
    g_signal_connect( G_OBJECT( err_window ), "button-press-event", G_CALLBACK( gtk_window_close ), NULL );
}
...
static void remmina_file_editor_browse_avahi( GtkWidget *button, RemminaFileEditor *gfe )
{
    ...
    aui_service_dialog_set_browse_service_types( AUI_SERVICE_DIALOG( dialog ), gfe->priv->avahi_service_type, NULL );
...
static void remmina_file_editor_create_notebook_container( RemminaFileEditor *gfe )
{
    ...
    gtk_scrolled_window_set_policy(
        GTK_SCROLLED_WINDOW( gfe->priv->config_scrollable ), GTK_POLICY_AUTOMATIC, GTK_POLICY_ALWAYS );
                                                                           ...
                                                                           RemminaFileEditor *gfe )
{
    ...
    gtk_widget_set_sensitive(
        GTK_WIDGET( gfe->priv->ssh_tunnel_server_entry ),
                                               ...
                                               gint row )
{
    ...
    if( setting->opt1 )
    {
        gfe->priv->avahi_service_type = (const char *)setting->opt1;
...
static void remmina_file_editor_update_resolution( GtkWidget *widget, RemminaFileEditor *gfe )
{
    ...
    remmina_public_load_combo_text_d( gfe->priv->resolution_custom_combo, remmina_pref.resolutions, res_str, NULL );
    g_free( res_str );
}
                                                   ...
                                                   gint row )
{
    ...
    res_mode = static_cast<RemminaProtocolWidgetResolutionMode>(
        remmina_file_get_int( gfe->priv->remmina_file, "resolution_mode", RES_INVALID ) );
    ...
    widget = gtk_radio_button_new_with_label_from_widget( GTK_RADIO_BUTTON( gfe->priv->resolution_iws_radio ),
                                                          _( "Use client resolution" ) );
    gtk_widget_show( widget );
    gtk_box_pack_start( GTK_BOX( hbox ), widget, TRUE, TRUE, 0 );
    ...
    g_signal_connect( G_OBJECT( gfe->priv->resolution_custom_radio ),
                      "toggled",
                      G_CALLBACK( remmina_file_editor_button_on_toggled ),
                      gfe->priv->resolution_custom_combo );
...
{
    TRACE_CALL( __func__ );
    return remmina_file_editor_create_text2( gfe, grid, row, col, label, value, 0, 40, setting_name );
}
                                                      ...
                                                      const char *setting_name )
{
    ...
    widget = gtk_file_chooser_button_new( label, static_cast<GtkFileChooserAction>( type ) );
    if( setting_name )
        gtk_widget_set_name( widget, setting_name );
    gtk_widget_show( widget );
    if( value )
                                                  ...
                                                  char *setting_name )
{
    ...
    g_signal_connect( G_OBJECT( widget ), "insert-text", G_CALLBACK( remmina_file_editor_int_setting_filter ), NULL );
                                                     ...
                                                     char *setting_name )
{
    ...
    g_signal_connect(
        G_OBJECT( widget ), "insert-text", G_CALLBACK( remmina_file_editor_double_setting_filter ), NULL );

    return widget;
}

gdouble remmina_file_get_double( RemminaFile *remminafile, const char *setting, gdouble default_value );

static void
remmina_file_editor_create_settings( RemminaFileEditor *gfe, GtkWidget *grid, const RemminaProtocolSetting *settings )
{
    TRACE_CALL( __func__ );
    RemminaFileEditorPriv *priv = gfe->priv;
    GtkWidget *widget;
    ...
    {
        setting_name = (char *)( remmina_plugin_manager_get_canonical_setting_name( settings ) );
        switch( settings->type )
        {
            ...
            case REMMINA_PROTOCOL_SETTING_TYPE_PASSWORD:
                widget =
                    remmina_file_editor_create_password( gfe,
                                                         grid,
                                                         grid_row,
                                                         0,
            ...
            case REMMINA_PROTOCOL_SETTING_TYPE_KEYMAP:
                ...
                priv->keymap_combo =
                    remmina_file_editor_create_select( gfe,
                                                       grid,
                                                       grid_row + 1,
                                                       0,
                                                       _( "Keyboard mapping" ),
            ...
            case REMMINA_PROTOCOL_SETTING_TYPE_TEXT:
                widget = remmina_file_editor_create_text( gfe,
                                                          grid,
                                                          grid_row,
                                                          0,
                                                          g_dgettext( priv->plugin->domain, settings->label ),
                ...
                g_hash_table_insert( priv->setting_widgets, setting_name, widget );
                if( settings->opt2 )
                    gtk_widget_set_tooltip_text( widget, _( (const char *)settings->opt2 ) );
                grid_row++;
                break;
            ...
            case REMMINA_PROTOCOL_SETTING_TYPE_TEXTAREA:
                ...
                widget = remmina_file_editor_create_textarea( gfe,
                                                              grid,
                                                              grid_row,
                                                              0,
                                                              g_dgettext( priv->plugin->domain, settings->label ),
            ...
            case REMMINA_PROTOCOL_SETTING_TYPE_SELECT:
                widget = remmina_file_editor_create_select( gfe,
                                                            grid,
                                                            grid_row,
                                                            0,
                                                            g_dgettext( priv->plugin->domain, settings->label ),
            ...
            case REMMINA_PROTOCOL_SETTING_TYPE_COMBO:
                widget = remmina_file_editor_create_combo( gfe,
                                                           grid,
                                                           grid_row,
                                                           0,
                                                           g_dgettext( priv->plugin->domain, settings->label ),
                                                           (const char *)settings->opt1,
                                                           remmina_file_get_string( priv->remmina_file, setting_name ),
                                                           setting_name );
                g_hash_table_insert( priv->setting_widgets, setting_name, widget );
                if( settings->opt2 )
            ...
            case REMMINA_PROTOCOL_SETTING_TYPE_CHECK:
                widget =
                    remmina_file_editor_create_check( gfe,
                                                      grid,
                                                      grid_row,
                                                      grid_column,
                                                      g_dgettext( priv->plugin->domain, settings->label ),
            ...
            case REMMINA_PROTOCOL_SETTING_TYPE_INT:
                widget = remmina_file_editor_create_int( gfe,
                                                         grid,
                                                         grid_row,
                                                         0,
                                                         g_dgettext( priv->plugin->domain, settings->label ),
            ...
            case REMMINA_PROTOCOL_SETTING_TYPE_DOUBLE:
                widget = remmina_file_editor_create_double(
                    gfe,
                    grid,
                    grid_row,
                    0,
...
static void remmina_file_editor_create_behavior_tab( RemminaFileEditor *gfe )
{
    ...
    priv->behavior_autostart_check =
        remmina_file_editor_create_check( gfe,
                                          grid,
                                          6,
                                          1,
                                          _( "Auto-start this profile" ),
                                          remmina_file_get_int( priv->remmina_file, "enable-autostart", FALSE ),
                                          "enable-autostart" );
}

static const char *ssh_tunnel_auth_list[] = { "0",
                                              N_( "Password" ),
                                              "1",
...
static void remmina_file_editor_create_ssh_tunnel_tab( RemminaFileEditor *gfe, RemminaProtocolSSHSetting ssh_setting )
{
    ...
    grid = remmina_file_editor_create_notebook_tab( gfe, NULL, _( "SSH Tunnel" ), 9, 3 );
    widget = gtk_toggle_button_new_with_label( _( "Enable SSH tunnel" ) );
    gtk_widget_set_halign( widget, GTK_ALIGN_START );
    gtk_grid_attach( GTK_GRID( grid ), widget, 0, row, 1, 1 );
    g_signal_connect(
        G_OBJECT( widget ), "toggled", G_CALLBACK( remmina_file_editor_ssh_tunnel_enabled_check_on_toggled ), gfe );
    ...
    switch( ssh_setting )
    {
        case REMMINA_PROTOCOL_SSH_SETTING_TUNNEL:
            ...
            g_signal_connect( G_OBJECT( widget ),
                              "toggled",
                              G_CALLBACK( remmina_file_editor_ssh_tunnel_server_custom_radio_on_toggled ),
                              gfe );
        ...
        case REMMINA_PROTOCOL_SSH_SETTING_REVERSE_TUNNEL:
            ...
            priv->ssh_tunnel_server_entry =
                remmina_file_editor_create_text( gfe, grid, 1, 0, _( "Server" ), NULL, "ssh_reverse_tunnel_server" );
    ...
    {
        widget = remmina_file_editor_create_text( gfe, grid, row, 1, _( "Start-up path" ), NULL, "start-up-path" );
        cs = remmina_file_get_string( priv->remmina_file, "execpath" );
        gtk_entry_set_text( GTK_ENTRY( widget ), cs ? cs : "" );
        g_hash_table_insert( priv->setting_widgets, static_cast<void*>(const_cast<char*>("execpath")), widget );
        // 2
        row++;
    }
        ...
        || ssh_setting == REMMINA_PROTOCOL_SSH_SETTING_REVERSE_TUNNEL )
    {
        priv->ssh_tunnel_username_entry =
            remmina_file_editor_create_text( gfe, grid, row, 0, _( "Username" ), NULL, "ssh_tunnel_username" );
        // 5
        row++;
    }

    widget = remmina_file_editor_create_password( gfe,
                                                  grid,
                                                  row,
                                                  0,
                                                  _( "Password" ),
    ...
    if( ssh_setting == REMMINA_PROTOCOL_SSH_SETTING_TUNNEL )
    {
        ...
        gtk_toggle_button_set_active(
            GTK_TOGGLE_BUTTON( cs ? priv->ssh_tunnel_server_custom_radio : priv->ssh_tunnel_server_default_radio ),
            TRUE );
        gtk_entry_set_text( GTK_ENTRY( priv->ssh_tunnel_server_entry ), cs ? cs : "" );
    }
...
static void remmina_file_editor_protocol_combo_on_changed( GtkComboBox *combo, RemminaFileEditor *gfe )
{
    ...
    {
        priv->plugin =
            (RemminaProtocolPlugin *)remmina_plugin_manager_get_plugin( REMMINA_PLUGIN_TYPE_PROTOCOL, protocol );
        g_free( protocol );
...
static void remmina_file_editor_save_behavior_tab( RemminaFileEditor *gfe )
{
    ...
    remmina_file_set_string(
        priv->remmina_file, "precommand", gtk_entry_get_text( GTK_ENTRY( priv->behavior_precommand_entry ) ) );
    remmina_file_set_string(
        priv->remmina_file, "postcommand", gtk_entry_get_text( GTK_ENTRY( priv->behavior_postcommand_entry ) ) );

    autostart_enabled = ( priv->behavior_autostart_check
                              ? gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( priv->behavior_autostart_check ) )
                              : FALSE );
...
static void remmina_file_editor_save_ssh_tunnel_tab( RemminaFileEditor *gfe )
{
    ...
    ssh_tunnel_enabled = ( priv->ssh_tunnel_enabled_check
                               ? gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( priv->ssh_tunnel_enabled_check ) )
                               : FALSE );
    ...
    remmina_file_set_string_ref( priv->remmina_file,
                                 "ssh_tunnel_auth",
    ...
    remmina_file_set_string(
        ...
        ( ssh_tunnel_enabled && priv->ssh_tunnel_server_entry
                  && ( priv->ssh_tunnel_server_custom_radio == NULL
                       || gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( priv->ssh_tunnel_server_custom_radio ) ) )
              ? gtk_entry_get_text( GTK_ENTRY( priv->ssh_tunnel_server_entry ) )
              : NULL ) );

    ssh_tunnel_auth = gtk_combo_box_get_active( GTK_COMBO_BOX( priv->ssh_tunnel_auth_combo ) );

    remmina_file_set_int( priv->remmina_file, "ssh_tunnel_auth", ssh_tunnel_auth );
    remmina_file_set_string( priv->remmina_file,
                             "ssh_tunnel_privatekey",
                             ( priv->ssh_tunnel_privatekey_chooser ? gtk_file_chooser_get_filename(
                                   GTK_FILE_CHOOSER( priv->ssh_tunnel_privatekey_chooser ) )
                                                                   : NULL ) );

    remmina_file_set_string( priv->remmina_file,
                             "ssh_tunnel_certfile",
                             ( priv->ssh_tunnel_certfile_chooser ? gtk_file_chooser_get_filename(
                                   GTK_FILE_CHOOSER( priv->ssh_tunnel_certfile_chooser ) )
                                                                 : NULL ) );

    remmina_file_set_string( priv->remmina_file,
                             "ssh_tunnel_password",
                             ( ssh_tunnel_enabled && ( ssh_tunnel_auth == SSH_AUTH_PASSWORD ) )
                                 ? gtk_entry_get_text( GTK_ENTRY( priv->ssh_tunnel_auth_password ) )
                                 : NULL );
    ...
    remmina_file_set_string(
        ...
        ( ssh_tunnel_enabled
          && ( ssh_tunnel_auth == SSH_AUTH_PUBLICKEY || ssh_tunnel_auth == SSH_AUTH_AUTO_PUBLICKEY ) )
            ? gtk_entry_get_text( GTK_ENTRY( priv->ssh_tunnel_passphrase ) )
            : NULL );
}

static int remmina_file_editor_validate_settings( RemminaFileEditor *gfe,
                                                   char *setting_name_to_validate,
                                                   gconstpointer value,
                                                   GError **err )
{
    ...
    if( setting_iter )
    {
        ...
        while( setting_iter->type != REMMINA_PROTOCOL_SETTING_TYPE_END )
        {
            ...
            else if( (char *)setting_name_to_validate )
            {
                if( strcmp( (char *)setting_name_to_validate, setting_iter->name ) == 0 )
                {
                    // found = TRUE;
                    ...
                    if( validator != NULL )
                    {
                        ...
                        err_ret = ( ( GError * (*)( gpointer, gconstpointer, gpointer ) )
                                        validator )( setting_name_to_validate, value, validator_data );
                    }
...
static GError *remmina_file_editor_update_settings( RemminaFileEditor *gfe, GtkWidget **failed_widget )
{
    ...
    *failed_widget = NULL;
    ...
    while( g_hash_table_iter_next( &iter, &key, reinterpret_cast<void**>(&widget) ) )
    {
        // We don't want to save or validate grayed-out settings.
        if( !gtk_widget_get_sensitive( GTK_WIDGET( widget ) ) )
        {
            g_debug( "Grayed-out setting-widget '%s' will not be saved.", gtk_widget_get_name( widget ) );
            continue;
        }
        ...
        if( GTK_IS_ENTRY( widget ) )
        {
            ...
            if( !remmina_file_editor_validate_settings( gfe, (char *)key, value, &err ) )
            {
            ...
            remmina_file_set_string( priv->remmina_file, (char *)key, value );
        }
        else if( GTK_IS_TEXT_VIEW( widget ) )
        {
            ...
            if( !remmina_file_editor_validate_settings( gfe, (char *)key, escaped, &err ) )
            {
                // Error while validating!
                // err should be set now.
                *failed_widget = widget;
            ...
            remmina_file_set_string( priv->remmina_file, (char *)key, escaped );
            g_free( escaped );
        }
        else if( GTK_IS_COMBO_BOX( widget ) )
        {
            char *value = remmina_public_combo_get_active_text( GTK_COMBO_BOX( widget ) );
            ...
            remmina_file_set_string_ref( priv->remmina_file, (char *)key, value );
        }
        else if( GTK_IS_FILE_CHOOSER( widget ) )
        {
            char *value = gtk_widget_get_sensitive( GTK_WIDGET( widget ) )
        ...
        {
            bool value = gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON( widget ) );
            ...
            remmina_file_set_int( priv->remmina_file, (char *)key, value );
        }
    }
...
static GError *remmina_file_editor_update( RemminaFileEditor *gfe, GtkWidget **failed_widget )
{
    ...
    remmina_file_set_string_ref(
        priv->remmina_file, "protocol", remmina_public_combo_get_active_text( GTK_COMBO_BOX( priv->protocol_combo ) ) );
    ...
    if( priv->keymap_combo )
        remmina_file_set_string_ref(
            priv->remmina_file, "keymap", remmina_public_combo_get_active_text( GTK_COMBO_BOX( priv->keymap_combo ) ) );
...
static void remmina_file_editor_on_default( GtkWidget *button, RemminaFileEditor *gfe )
{
    ...
    dialog = gtk_message_dialog_new(
        GTK_WINDOW( gfe ), GTK_DIALOG_MODAL, GTK_MESSAGE_INFO, GTK_BUTTONS_OK, _( "Default settings saved." ) );
...
static void remmina_file_editor_init( RemminaFileEditor *gfe )
{
    ...
    gtk_widget_set_tooltip_text( GTK_WIDGET( widget ),
                                 _( "Use the current settings as the default for all new connection profiles" ) );
...
static int remmina_file_editor_iterate_protocol( char *protocol, RemminaPlugin *plugin, gpointer data )
{
    TRACE_CALL( __func__ );
    ...
    gtk_list_store_set( store,
                        ...
                        2,
                        ( (RemminaProtocolPlugin *)plugin )->icon_name,
                        -1 );
...
GtkWidget *remmina_file_editor_new_from_file( RemminaFile *remminafile )
{
    ...
    priv->group_combo =
        remmina_public_create_combo_entry( groups, remmina_file_get_string( remminafile, "group" ), FALSE );
    g_free( groups );
